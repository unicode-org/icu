...
NormalizationTransliterator::NormalizationTransliterator(
    ...
    switch(mode) {
    case UNORM_NFD:
        skippable = &SKIPPABLES[D];
        break;
    case UNORM_NFC:
        skippable = &SKIPPABLES[C];
        break;
    case UNORM_NFKD:
        skippable = &SKIPPABLES[KD];
        break;
    case UNORM_NFKC:
        skippable = &SKIPPABLES[KC];
        break;
    }
}
...
void NormalizationTransliterator::handleTransliterate(Replaceable& text, UTransPosition& offsets,
    ...
    while(start < limit) {
        ...
        if(isIncremental && limit == iter.limit) {
            ...
            if (!skippable->containsAll(output)) {
                break;
            }
        }
...
void NormalizationTransliterator::initStatics() {
    if (SKIPPABLES == NULL) {
        ...
        if (SKIPPABLES == NULL) {
            ...
            "D1BB-\\U0001D1C0\\U0002F800-\\U0002FA1D]", ""), ec);
 ...
 */
void NormalizationTransliterator::cleanup() {
    if (SKIPPABLES != NULL) {
        delete[] SKIPPABLES;
        SKIPPABLES = NULL;
        umtx_destroy(&MUTEX);
