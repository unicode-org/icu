...
UnicodeSetIterator::UnicodeSetIterator(UnicodeSet& set, UBool abb) {
    reset(set, abb);
}
...
void UnicodeSetIterator::reset(UnicodeSet& set, UBool abb) {
     abbreviated = abb;
     this->set = set;
...
void UnicodeSetIterator::reset(UnicodeSet& set) {
    reset(set, FALSE);
}
...
void RTTest::test(const UnicodeString& sourceRangeVal,
    ...
    if (roundtripExclusions != NULL && strlen(roundtripExclusions) > 0) {
        this->roundtripExclusions.applyPattern(roundtripExclusions, status);
        if (U_FAILURE(status)) {
...
void RTTest::test2(UBool quick) {
    ...
    while (TRUE) {
        ...
        if (isSame(cs, reverse) == FALSE && 
            roundtripExclusions.contains(c) == FALSE) {
    ...
    while (TRUE) {
        ...
        while (TRUE) {
            ...
            if (isSame(cs, reverse) == FALSE && 
                roundtripExclusions.contains(c) == FALSE&&
                roundtripExclusions.contains(d) == FALSE) {
                logRoundTripFailure(cs, targ, reverse);
                continue;
            } 
