...
public class BasicTimeZoneExample {
    ...
    public static void nextTransitionExample() {
        ...
        BasicTimeZone btz =
                (BasicTimeZone) TimeZone.getTimeZone("America/Los_Angeles", TimeZone.TIMEZONE_ICU);
        ...
        for (int i = 0; i < 5; i++) { // Up to 5 transitions
            TimeZoneTransition trans =
                    btz.getNextTransition(start, false /* not including start time */);

            // Display the transition time and offset information
            long transTime = trans.getTime();
            System.out.println(
                    wallTimeFmt.format(new Date(transTime - 1))
                            + " -> "
                            + wallTimeFmt.format(new Date(transTime)));
            System.out.println(
                    " - Before (Offset/Save): "
                            + trans.getFrom().getRawOffset()
                            + "/"
                            + trans.getFrom().getDSTSavings());
            System.out.println(
                    " - After  (Offset/Save): "
                            + trans.getTo().getRawOffset()
                            + "/"
                            + trans.getTo().getDSTSavings());
    ...
    public static void previousTransitionExample() {
        ...
        for (int i = 0; i < 5; i++) { // Up to 5 transitions
            TimeZoneTransition trans =
                    btz.getPreviousTransition(start, false /* not including start time */);
    ...
    public static void timeZoneRulesExample() {
        ...
        System.out.println(
                "### Extracts time zone rules used by America/Los_Angeles since year 2005");
    ...
    public static void equivalentTransitionsExample() {
        ...
        // A TimeZone instance created by getTimeZone with TIMEZONE_ICU is always a BasicTimeZone
        BasicTimeZone tzNewYork =
                (BasicTimeZone) TimeZone.getTimeZone("America/New_York", TimeZone.TIMEZONE_ICU);
        BasicTimeZone tzDetroit =
                (BasicTimeZone) TimeZone.getTimeZone("America/Detroit", TimeZone.TIMEZONE_ICU);
        ...
        for (int startYear = 1970; startYear <= 2000; startYear += 10) {
            ...
            // Check if these two zones have equivalent time zone transitions for the given time
            // range
