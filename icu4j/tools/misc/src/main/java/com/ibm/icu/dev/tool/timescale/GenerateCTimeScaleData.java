...
/**
 * This class prints out the initializers needed to initialize the time scale data in the C version
 * of <code>UniversalTimeScale</code>.
 *
 * <p>It just calls <code>getTimeScaleValue()</code> for all fields and prints the initializers.
 * Because some C compilers can't compile a literal constant for the minimum and / or maximum values
 * of an <code>int64_t</code>, this code will print <code>U_INT64_MIN</code> or <code>U_INT64_MAX
    ...
    public static void main(String[] args) {
        ...
        for (int scale = 0; scale < UniversalTimeScale.MAX_SCALE; scale += 1) {
            ...
            cargs[2] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.FROM_MIN_VALUE));
            cargs[3] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.FROM_MAX_VALUE));
            cargs[4] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.TO_MIN_VALUE));
            cargs[5] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.TO_MAX_VALUE));
            cargs[6] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.EPOCH_OFFSET_PLUS_1_VALUE));
            cargs[7] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.EPOCH_OFFSET_MINUS_1_VALUE));
            cargs[8] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.UNITS_ROUND_VALUE));
            cargs[9] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.MIN_ROUND_VALUE));
            cargs[10] =
                    minMaxFilter(
                            UniversalTimeScale.getTimeScaleValue(
                                    scale, UniversalTimeScale.MAX_ROUND_VALUE));
