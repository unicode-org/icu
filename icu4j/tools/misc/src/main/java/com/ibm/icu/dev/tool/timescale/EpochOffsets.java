...
public class EpochOffsets {
    ...
    /**
     * The <code>main()</code> method calculates the epoch offsets used by the <code>
     * UniversalTimeScale</code> class.
     *
     * <p>The calculations are done using an ICU <code>Calendar</code> object. The first step is to
     * calculate the Universal Time Scale's epoch date. Then the epoch offsets are calculated by
     * calculating each epoch date, subtracting the universal epoch date from it, and converting
     * that value to ticks.
     *
