...
public class ScriptData extends TagValueData {
    public static class Record {
        ...
        public String toString() {
            return "["
                    + Utility.hex(startChar, 6)
                    + ".."
                    + Utility.hex(endChar, 6)
                    + ", "
                    + UScript.getShortName(scriptCode).toLowerCase()
                    + "ScriptCode]";
        }
    }
    ...
    private ScriptData.TagException exceptions[] = {
        ...
        new ScriptData.TagException(
                "nkoo", "nko "), // New code from ISO 15924, not sure this will be OT tag
        new ScriptData.TagException(
                "vaii", "vai "), // New code from ISO 15924, not sure this will be OT tag
        ...
        for (int script = fMinScript; script <= fMaxScript; script += 1) {
            ...
            if (script != commonScript) {
                UnicodeSet scriptSet =
                        new UnicodeSet("\\p{" + fScriptTags[script - fMinScript] + "}");
