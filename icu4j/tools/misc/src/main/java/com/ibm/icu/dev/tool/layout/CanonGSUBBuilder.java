 ...
 * @author Eric Mader
 *     <p>Notes:
 *     <p>The property \p{Decomposition_Type=Canonical} will match all characters with a canonical
 *     decomposition.
 *     <p>So "[[\\p{Latin}\\p{Greek}\\p{Cyrillic}] & [\\p{Decomposition_Type=Canonical}]]" will
 *     match all Latin, Greek and Cyrillic characters with a canonical decomposition.
 *     <p>Are these three scripts enough? Do we want to collect them all at once and distribute by
 *     script, or process them one script at a time. It's probably a good idea to build a single
 *     table for however many scripts there are.
 *     <p>It might be better to collect all the characters that have a canonical decomposition and
 *     just sort them into however many scripts there are... unless we'll get characters in
 *     COMMON???
 */
public class CanonGSUBBuilder {
    public static String convertArabicString(
            int type, int ligature, String decomp, ClassTable isolClassTable) {
    ...
    static void buildArabicContextualForms(
            ArabicCharacterData data,
            ClassTable initClassTable,
            ClassTable mediClassTable,
            ClassTable finaClassTable,
            ClassTable isolClassTable) {
    ...
    static LigatureTree buildArabicLigatureTree(
            ArabicCharacterData data, ClassTable isolClassTable) {
        ...
        for (int i = 0; i < data.countRecords(); i += 1) {
            ...
            if (decomposition != null && decomposition.length() > 1) {
                ...
                switch (decompType) {
                    ...
                    case UCharacter.DecompositionType.MEDIAL:
                    case UCharacter.DecompositionType.ISOLATED:
                        contextualTree.insert(
                                convertArabicString(
                                        decompType, ligature, decomposition, isolClassTable));
                        ligatureCount += 1;
                        break;
    ...
    private static void buildArabicTables(
        ...
        UnicodeSet arabicBlock =
                new UnicodeSet("[[\\p{block=Arabic}] & [[:Cf:][:Po:][:So:][:Mn:][:Nd:][:Lm:]]]");
        UnicodeSet oddLigatures = new UnicodeSet("[\\uFC5E-\\uFC63\\uFCF2-\\uFCF4\\uFE70-\\uFE7F]");
        UnicodeSet arabicLetters = new UnicodeSet("[\\p{Arabic}]");
        ArabicCharacterData arabicData =
                ArabicCharacterData.factory(
                        arabicLetters.addAll(arabicBlock).removeAll(oddLigatures));
        ...
        buildArabicContextualForms(
                arabicData, initClassTable, mediClassTable, finaClassTable, isolClassTable);
    ...
    public static void buildLigatureTree(
            CanonicalCharacterData data, int script, LigatureTree ligatureTree) {
        int ligatureCount = 0;

        System.out.print(
                "building composition ligature tree for " + UScript.getName(script) + "... ");
    ...
    public static DecompTable[] buildDecompTables(CanonicalCharacterData data, int script) {
        ...
        System.out.print(
                "Building decompositon tables for "
                        + UScript.getName(script)
                        + "... total decompositions: "
                        + data.countRecords(script)
                        + ", max: "
                        + maxDecompCount
                        + "...");
    ...
    public static int[] buildLookups(
            CanonicalCharacterData data, LookupList lookupList, int script) {
    ...
    public static void buildDecompTables(String fileName) {
        // F900 - FAFF are compatibility ideographs. They all decompose to a single other character,
        // and can be ignored.
        // UnicodeSet decompSet = new UnicodeSet("[[[\\P{Hangul}] &
        // [\\p{DecompositionType=Canonical}]] - [\uF900-\uFAFF]]");
        UnicodeSet decompSet =
                new UnicodeSet(
                        "[[\\p{DecompositionType=Canonical}] & [\\P{FullCompositionExclusion}] & [\\P{Hangul}]]");
