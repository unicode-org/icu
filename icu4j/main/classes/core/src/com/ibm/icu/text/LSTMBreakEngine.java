...
class LSTMBreakEngine extends DictionaryBreakEngine {
    ...
    private Vectorizer makeVectorizer(LSTMData data) {
        switch(data.fType) {
            case CODE_POINTS:
              System.out.println("CodePointsVectorizer()");
                return new CodePointsVectorizer(data.fDict);
            case GRAPHEME_CLUSTER:
              System.out.println("GraphemeClusterVectorizer()");
    ...
    public LSTMBreakEngine(String name, String setExpr, int script) {
        UnicodeSet set = new UnicodeSet();
        set.applyPattern(setExpr);
        set.compact();
        setCharacters(set);
        this.fScript = script;
        this.fData = new LSTMData(name);
    ...
    static private void dotProductTo(final float [] a, float[][] b, float[] result) {
    ...
    private float[] compute(final float[][] W, final float[][] U, final float[] B,
        ...
        // ifco = x * W + h * U + b
        float[] ifco = new float[B.length];
        dotProductTo(x, W, ifco);
        ...
        float[] ic = Arrays.copyOf(ifco, hunits);
        hadamardProductTo(Arrays.copyOfRange(ifco, 2*hunits, 3*hunits), ic);
        addTo(ic, c);
    ...
    public int divideUpDictionaryRange(CharacterIterator fIter, int rangeStart, int rangeEnd,
        ...
        for (int i = 0 ; i < inputSeqLength; i++) {
            ...
            dotProductTo(both, this.fData.fOutputW, logp);
            addTo(this.fData.fOutputB, logp);

            int current = maxIndex(logp);

            // BIES logic.
            if (breakOnNext || current == LSTMClass.BEGIN.ordinal() ||
                current == LSTMClass.SINGLE.ordinal()) {
                if (i != 0) {
                    foundBreaks.push(offsets.get(i));
                }
            }
            breakOnNext = (current == LSTMClass.END.ordinal() ||
                current == LSTMClass.SINGLE.ordinal());
        }
