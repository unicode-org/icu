...
public class JapaneseCalendar extends GregorianCalendar {
    ...
    protected int getDefaultMonthInYear(int extendedYear) {
        ...
        RuleDate eraStart = ERA_RULES.getStartDate(era);
        if (extendedYear == eraStart.getYear()) {
            return eraStart.getMonth()  // month..
    ...
    protected int getDefaultDayInMonth(int extendedYear, int month) {
        ...
        if (extendedYear == eraStart.getYear()) {       // if it is year 1..
            if (month == (eraStart.getMonth() - 1)) {   // if it is the emperor's first month..
                return eraStart.getDay();               // return the D_O_M of accession
            }
        }
    ...
    protected void handleComputeFields(int julianDay) {
        ...
        int eraIdx = getEraIndex(year, internalGet(MONTH) + 1 /* 1-base */, internalGet(DAY_OF_MONTH));

        internalSet(ERA, eraIdx);
        internalSet(YEAR, year - ERA_RULES.getStartYear(eraIdx) + 1);
    }

    private int getEraIndex(int year, int month1 /* 1-based */, int day) {
        int high = ERA_RULES.getNumberOfEras(); // last index + 1
        int low;

        // Short circuit for recent years.  Most modern computations will
        // occur in the last few eras.
        if (ERA_RULES.getStartDate(CURRENT_ERA).compareTo(year, month1, day) <= 0) {
            low = CURRENT_ERA;
        } else {
            low = 0;
        }
        ...
        while (low < high - 1) {
            int i = (low + high) / 2;
            RuleDate eraStart = ERA_RULES.getStartDate(i);
            if (eraStart.compareTo(year, month1, day) <= 0) {
                low = i;
            } else {
                high = i;
            }
...
//    };
    ...
    static {
        ...
        Grego.timeToFields(currentTime, fields);

        int eraIdx = ERA_RULES.getNumberOfEras() - 1;   // last era
        while (eraIdx != 0) {
            RuleDate start = ERA_RULES.getStartDate(eraIdx);
            if (start.compareTo(fields[0], fields[1], fields[2]) <= 0) {
                break;
            }
            eraIdx--;
        }
    ...
    public int getActualMaximum(int field) {
        if (field == YEAR) {
            ...
            if (era == CURRENT_ERA) {
                ...
                RuleDate eraStart = ERA_RULES.getStartDate(era + 1);
                int nextEraYear = eraStart.getYear();
                int nextEraMonth = eraStart.getMonth();
                int nextEraDate = eraStart.getDay();
