 ...
 *
 * <p>This is a port of ucnvseltst.c from ICU4C
 *
 * <p>Tests related to serialization are not ported in this version. In addition, the
 * TestConversionUTF8 method is not going to be ported, as UTF8 is seldom used in Java.
 *
 * @author Shaopeng Jia
 */
...
public class TestSelection extends TestFmwk {
    ...
    public void TestConversionUTF16() {
        ...
        for (UnicodeSet excluded : excludedSets) {
            CharsetSelector sel =
                    new CharsetSelector(testEncodings, excluded, CharsetICU.ROUNDTRIP_SET);
            for (int i = 0; i < texts.length; i++) {
                List<String> result = sel.selectForString(texts[i]);
                verifyResultUTF16(
                        texts[i], testEncodings, result, excluded, CharsetICU.ROUNDTRIP_SET);
            }
        }
    }

    private void verifyResultUTF16(
            String s,
        ...
        for (String enc : encodings) {
            ...
            if (selectResult != manualResult) {
                errln(
                        "failure in charset selector! Charset "
                                + enc
    ...
    public void TestCharsetSelectorCodeCoverage() {
        ...
        try {
            ...
            errln(
                    "IllegalArgumentException should have been thrown by CharsetSelector when given a bogus mappingType, but it wasn't.");
        } catch (IllegalArgumentException e) {
            logln(
                    "IllegalArgumentException was thrown by CharsetSelector with a bogus mappingType.");
        }
    }
