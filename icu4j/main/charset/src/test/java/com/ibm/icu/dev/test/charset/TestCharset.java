...
public class TestCharset extends TestFmwk {
    ...
    public void TestUTF8Converter() {
        ...
        for (; i < size[0]; i++) {
            ...
            try {
                ...
                errln("Incorrect result in " + converter + " for 0x" + Integer.toHexString(i));
                break;
            }
        }
    ...
    public void TestHZ() {
        ...
        try {
            ...
            if (!equals(start, finish)) {
                ...
                for (int i = 0; i < finishArray.length; i++) finishArray[i] = finish.get(i);
        ...
        char charin[] = {(char) 0x0041, (char) 0x0042, (char) 0x3000};
        ...
        for (int i = 0; i < testLoopSize; i++) {
            ...
            if (i < 3) {
                if (!result.isOverflow()) {
                    errln(
                            "Overflow buffer error should have occurred while encoding HZ ("
                                    + i
                                    + ")");
                }
            } else {
    ...
    public void TestUTF8Surrogates() {
        ...
        byte[][] in2 =
                new byte[][] {
                    ...
                    {
                        (byte) 0xc0,
                        (byte) 0x80, /* illegal non-shortest form */
                        (byte) 0xe0,
                        (byte) 0x80,
                        (byte) 0x80, /* illegal non-shortest form */
                        (byte) 0xf0,
                        ...
                        (byte) 0x80, /* illegal non-shortest form */
                        (byte) 0xc0,
                        (byte) 0xc0, /* illegal trail byte */
                        (byte) 0xf4,
                        (byte) 0x90,
                        (byte) 0x80,
                        (byte) 0x80, /* 0x110000 out of range */
                        (byte) 0xf8,
                        (byte) 0x80,
                        (byte) 0x80,
                        (byte) 0x80,
        ...
        try {
            for (i = 0; i < in.length; i++) {
                ...
                smBufDecode(decoder, converter, source, expected, true, false, true);
                smBufDecode(decoder, converter, source, expected, true, false, false);
            }
        } catch (Exception ex) {
    ...
    public void TestSurrogateBehavior() {
        ...
        outer:
        for (int conv = 0; conv < converters.length; conv++) {
            String converter = (String) converters[conv];
            if (converter.equals("x-IMAP-mailbox-name")
                    || converter.equals("UTF-7")
                    || converter.equals("CESU-8")
                    || converter.equals("BOCU-1")
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (!(equals(cr, results[i])
                        || (results[i].isUnmappable()
                                && cr.isUnderflow()) // mappability depends on the converter
                )) {
                    if (!currentlybad) {
                        currentlybad = true;
                        badcount++;
            ...
            if (!(equals(target, expected, expected.limit())
                    && target.position() == expected.limit())) {
                if (!currentlybad) {
                    currentlybad = true;
                    badcount++;
                    logln("");
        ...
        logln("");
        log("unmap\t");
        for (int i = 0; i < n; i++) log(unmapindices[i] + "\t");
        logln("");
        log("malf \t");
        for (int i = 0; i < n; i++) log(malfindices[i] + "\t");
        logln("");
        log("pos  \t");
        for (int i = 0; i < n; i++) log(badposindices[i] + "\t");
        logln("");
        log("uhoh \t");
        for (int i = 0; i < n; i++) log(uhohindices[i] + "\t");
        logln("");
        logln("");
        log("The few that passed: ");
        for (int i = 0; i < pass.size(); i++) log(pass.get(i) + ", ");
        logln("");
        log("The few that are exempt: ");
        for (int i = 0; i < exempt.size(); i++) log(exempt.get(i) + ", ");
        logln("");
    }
    ...
    //
    //            // get relevant fields
    //            Object SUB_STOP_ON_ILLEGAL = getFieldValue(CharsetCallback, "SUB_STOP_ON_ILLEGAL",
    // null);
    //
    ...
    // invalidCharLength);
    //            setFieldValue(CharsetICU.class, "subChar1", ((CharsetICU) decoder.charset()),
    // subChar1);
    //
    //            // run cbFromUWriteSub
    //            Method cbFromUWriteSub =
    // CharsetEncoderICU.class.getDeclaredMethod("cbFromUWriteSub", new Class[] {
    // CharsetEncoderICU.class, CharBuffer.class, ByteBuffer.class, IntBuffer.class});
    //            cbFromUWriteSub.setAccessible(true);
    //            CoderResult encoderResultExpected = (CoderResult)cbFromUWriteSub.invoke(encoder,
    // new Object[] {encoder, encoderCharsExpected, encoderBytesExpected, encoderOffsetsExpected});
    //
    ...
    // CharsetDecoderICU.class, char[].class, int.class, int.class, CharBuffer.class,
    // IntBuffer.class, int.class});
    //            toUWriteUChars.setAccessible(true);
    //            CoderResult decoderResultExpected1 = (CoderResult)toUWriteUChars.invoke(decoder,
    // new Object[] {decoder, new char[] {0xFFFD}, Integer.valueOf(0), Integer.valueOf(1),
    // decoderCharsExpected1, decoderOffsetsExpected1, Integer.valueOf(bytes.position())});
    //
    ...
    //            CoderResult decoderResultExpected2 = (CoderResult)toUWriteUChars.invoke(decoder,
    // new Object[] {decoder, new char[] {0x1A}, Integer.valueOf(0), Integer.valueOf(1),
    // decoderCharsExpected2, decoderOffsetsExpected2, Integer.valueOf(bytes.position())});
    //
    ...
    // encoder, null, chars, bytes, offsets, buffer, length, cp, null }, under, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SKIP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, malf }, malf,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SKIP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, unmap }, under,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SKIP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL + "xx", chars, bytes, offsets, buffer, length, cp, null }, null,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //
    //            // create tests for TO_U_CALLBACK_SKIP    4
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SKIP", new Object[] { decoder,
    // null, bytes, chars, offsets, buffer, length, null }, under, charsBackup, bytesBackup,
    // offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SKIP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL, bytes, chars, offsets, buffer, length, malf }, malf, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SKIP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL, bytes, chars, offsets, buffer, length, unmap }, under, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SKIP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL + "xx", bytes, chars, offsets, buffer, length, null }, null, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //
    //            // create tests for FROM_U_CALLBACK_STOP   8
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_STOP", new Object[] {
    // encoder, null, chars, bytes, offsets, buffer, length, cp, null }, null, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_STOP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, malf }, malf,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_STOP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, unmap }, unmap,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_STOP", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL + "xx", chars, bytes, offsets, buffer, length, cp, null }, null,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    ...
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_STOP", new Object[] { decoder,
    // null, bytes, chars, offsets, buffer, length, null }, null, charsBackup, bytesBackup,
    // offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_STOP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL, bytes, chars, offsets, buffer, length, malf }, malf, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_STOP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL, bytes, chars, offsets, buffer, length, unmap }, unmap, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_STOP", new Object[] { decoder,
    // SUB_STOP_ON_ILLEGAL + "xx", bytes, chars, offsets, buffer, length, null }, null, charsBackup,
    // bytesBackup, offsetsBackup, new Object[] { invalidCharLength, subChar1 }});
    ...
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SUBSTITUTE", new Object[] {
    // encoder, null, chars, bytes, offsets, buffer, length, cp, null }, encoderResultExpected,
    // encoderCharsExpected, encoderBytesExpected, encoderOffsetsExpected, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SUBSTITUTE", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, malf }, malf,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SUBSTITUTE", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL, chars, bytes, offsets, buffer, length, cp, unmap },
    // encoderResultExpected, encoderCharsExpected, encoderBytesExpected, encoderOffsetsExpected,
    // new Object[] { }});
    //            tests.add(new Object[] {encoderCall, "FROM_U_CALLBACK_SUBSTITUTE", new Object[] {
    // encoder, SUB_STOP_ON_ILLEGAL + "xx", chars, bytes, offsets, buffer, length, cp, null }, null,
    // charsBackup, bytesBackup, offsetsBackup, new Object[] { }});
    ...
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SUBSTITUTE", new Object[] {
    // decoder, null, bytes, chars, offsets, buffer, length, null }, decoderResultExpected1,
    // decoderCharsExpected1, bytesBackup, decoderOffsetsExpected1, new Object[] {
    // invalidCharLength, subChar1 }});
    //            tests.add(new Object[] {decoderCall, "TO_U_CALLBACK_SUBSTITUTE", new Object[] {
    // decoder, null, bytes, chars, offsets, buffer, length, null }, decoderResultExpected2,
    // decoderCharsExpected2, bytesBackup, decoderOffsetsExpected2, new Object[] {
    // invalidCharLength, subChar1_alternate }});
    //
    ...
    //                        errln(fieldName + " failed to return the correct result for case #" +
    // i + ".");
    //                    }
    ...
    //                        // case #i refers to the index in the arraylist tests
    //                        errln(fieldName + " did not perform the correct operation on the
    // buffers for case #" + i + ".");
    //                    }
    ...
    //    }
    //    private void setFieldValue(Class c, String name, Object instance, Object value) throws
    // Exception {
    //        Field field = c.getDeclaredField(name);
    //        field.setAccessible(true);
    ...
    public boolean equals(CharBuffer buf, CharBuffer str) {
        if (buf.limit() != str.limit()) return false;
        int limit = buf.limit();
        for (int i = 0; i < limit; i++) if (buf.get(i) != str.get(i)) return false;
        return true;
    }

    public boolean equals(CharBuffer buf, CharBuffer str, int limit) {
        if (limit > buf.limit() || limit > str.limit()) return false;
        for (int i = 0; i < limit; i++) if (buf.get(i) != str.get(i)) return false;
        return true;
    }
    ...
    public boolean equals(ByteBuffer buf, ByteBuffer compareTo) {
        if (buf.limit() != compareTo.limit()) return false;
        int limit = buf.limit();
        for (int i = 0; i < limit; i++) if (buf.get(i) != compareTo.get(i)) return false;
        return true;
    }

    public boolean equals(ByteBuffer buf, ByteBuffer compareTo, int limit) {
        if (limit > buf.limit() || limit > compareTo.limit()) return false;
        for (int i = 0; i < limit; i++) if (buf.get(i) != compareTo.get(i)) return false;
        return true;
    }
    ...
    */
    ...
    public void TestAvailableCharsets() {
        ...
        if (mapSize < charsets.length) {
            errln(
                    "Charset.availableCharsets() returned a number less than the number returned by icu. ICU: "
                            + charsets.length
                            + " JDK: "
                            + mapSize);
        }
    ...
    public void TestWindows936() {
        ...
        if (!canonicalName.equals("GBK")) {
            errln(
                    "Did not get the expected canonical name. Got: "
    ...
    public void TestICUAvailableCharsets() {
        ...
        for (int i = 0; i < charsets.length; i++) {
            ...
            try {
                ...
                errln(
                        "Could not instantiate encoder for "
                                + charsets[i]
                                + ". Error: "
                                + ex.toString());
            }
            try {
                ...
                errln(
                        "Could not instantiate decoder for "
                                + charsets[i]
                                + ". Error: "
                                + ex.toString());
            }
    ...
    public void TestEncoderCreation() {
            ...
            if (enc != null) {
                ...
                if (!(enc instanceof CharsetEncoderICU)) {
                    errln(
                            "Expected "
                                    + encoding
                                    + " to be implemented by ICU but got an instance of "
                                    + enc.getClass());
                }
            } else {
        ...
        try {
            ...
            if (enc != null) {
                logln(
                        "Successfully created an encoder for "
                                + encoding
                                + ": "
                                + enc
                                + " which is implemented by ICU? "
                                + (enc instanceof CharsetEncoderICU));
            } else {
                errln("Error creating charset encoder for " + encoding);
            }
        } catch (Exception e) {
    ...
    public void TestSubBytes() {
        try {
            ...
            if (!expected.equals(new String(out.array()))) {
                errln(
                        "Did not get the expected result for substitution chars. Got: "
    ...
    */
    ...
    public void TestUTF8() throws CharacterCodingException {
        try {
            CharsetEncoder encoderICU =
                    new CharsetProviderICU().charsetForName("utf-8").newEncoder();
    ...
    public void TestUTF16Bom() {
        ...
        char[] in =
                new char[] {0x1122, 0x2211, 0x3344, 0x4433, 0x5566, 0x6655, 0x7788, 0x8877, 0x9900};
    ...
    private void smBufDecode(
        ...
        for (int i = source.position(); i < source.limit(); i++) mySource.put(i, source.get(i));
        ...
        }
        {
            ...
            while (true) {
                ...
                if (result.isUnderflow()) {
                    if (mySource.limit() < source.limit()) mySource.limit(mySource.limit() + 1);
                } else if (result.isOverflow()) {
                    if (myTarget.limit() < target.limit()) myTarget.limit(myTarget.limit() + 1);
                    else break;
    ...
    */
    ...
    public void TestMBCS() {
        {
            ...
            CharsetEncoder encoderICU =
                    new CharsetProviderICU().charsetForName("ibm-971").newEncoder();
            ByteBuffer out = ByteBuffer.allocate(6);
            encoderICU.onUnmappableCharacter(CodingErrorAction.REPLACE);
            CoderResult result =
                    encoderICU.encode(CharBuffer.wrap("\u0131\u0061\u00a1"), out, true);
            if (!result.isError()) {
                byte[] expected = {
                    (byte) 0xA9, (byte) 0xA5, (byte) 0xAF, (byte) 0xFE, (byte) 0xA2, (byte) 0xAE
                };
                if (!equals(expected, out.array())) {
                    errln(
                            "Did not get the expected result for substitution bytes. Got: "
                                    + hex(out.array()));
                }
        ...
        }
        {
            ...
            CharsetDecoder decoderICU =
                    new CharsetProviderICU().charsetForName("ibm-971").newDecoder();
    ...
    private void execDecoder(Charset cs) {
        ...
        CoderResult result =
                decoder.decode(ByteBuffer.wrap(new byte[] {-1, -2, 32, 0, 98}), out, false);
    ...
    private void execEncoder(Charset cs) {
        ...
        CoderResult result =
                encoder.encode(
                        CharBuffer.wrap(new char[] {'\uFFFF', '\u2345', 32, 98}), out, false);
        logln(cs.getClass().toString() + ":" + result.toString());
    ...
    public void TestDecodeMalformed() {
        ...
        if (ir != jr) {
            errln(
                    "ICU's decoder did not return the same result as Sun. ICU: "
                            + ir.toString()
                            + " Sun: "
                            + jr.toString());
        }
    ...
    public void TestUTF32BOM() {
        ...
        encoder.encode(inBuf, outBuf, true);
        outBuf.rewind();
        if (outBuf.get(0) != (byte) 0x00
                && outBuf.get(1) != (byte) 0x00
                && outBuf.get(2) != (byte) 0xFF
                && outBuf.get(3) != (byte) 0xFE) {
     ...
     */
    private void smBufDecode(
            CharsetDecoder decoder, String encoding, ByteBuffer source, CharBuffer target) {
    ...
    private void smBufEncode(
            CharsetEncoder encoder, String encoding, CharBuffer source, ByteBuffer target) {
    ...
    public void TestCharsetICUNullCharsetName() {
        ...
        try {
            ...
            errln(
                    "CharsetICU.forName should have thown an exception after getting a null charsetName.");
        } catch (Exception ex) {
        }
    }
    ...
    public void TestCharsetASCIIOverFlow() {
        ...
        // For better code coverage
        /* For better code coverage */
        byte byteout[] = {(byte) 0x01};
        char charin[] = {(char) 0x0001, (char) 0x0002};
        ByteBuffer bb = ByteBuffer.wrap(byteout);
        CharBuffer cb = CharBuffer.wrap(charin);
    ...
    public void TestCharsetUTF7() {
        ...
        /* Testing UTF-7 toUnicode with substitute callbacks */
        {
            ...
            char[] unicodeTestErrorConsumption = {
                0x61, 0xfffd, 0x61, 0xfffd, 0xfffd, 0x61, 0xfffd, 0xfffd, 0x61, 0xfffd, 0x61,
                0xfffd, 0x2e, 0x61, 0xfffd, 0x2e
            };
        ...
        // test for overflow buffer error
        encoder.reset();
        ccus.put((char) 0x3980);
        ccus.put((char) 0x2715);
        ccbs.put((byte) 0x2b);
        ccbs.put((byte) 0x4f);
        ccbs.put((byte) 0x59);
        ccbs.put((byte) 0x2d);
    ...
    public void TestBug12956() {
        ...
        if (!Arrays.equals(bytes, expected_utf7)) {
            errln(
                    "Incorrect UTF-7 conversion. Got "
                            + new String(bytes)
                            + " but expect "
        ...
        if (!Arrays.equals(bytes, expected_imap)) {
            errln(
                    "Incorrect IMAP-mailbox-name conversion. Got "
                            + new String(bytes)
                            + " but expect "
                            + new String(expected_imap));
        }
    }
    ...
    public void TestCharsetUTF16LE() {
        ...
        char u_pts1[] = {
            (char) 0xD805, (char) 0xDC01, (char) 0xDC02, (char) 0xDC03, (char) 0xD901, (char) 0xD902
        };
        byte b_pts1[] = {
            (byte) 0x00,
            (byte) 0x00,
    ...
    public void TestCharsetCallbacks() {
        ...
        String context1[] = {"J", "C", "D", null};
        char chararray[] = {(char) 0xd122};
        ...
        ((CharsetEncoderICU) encoder)
                .setFromUCallback(
                        ...
                        CharsetCallback.FROM_U_CALLBACK_ESCAPE,
                        null); // This callback is not valid.
        for (int i = 0; i < context1.length; i++) {
            ...
            ((CharsetEncoderICU) encoder)
                    .setFromUCallback(
                            CoderResult.unmappableForLength(1),
                            CharsetCallback.FROM_U_CALLBACK_ESCAPE,
                            context1[i]); // This callback is valid.
        ...
        for (int i = 0; i < context3.length; i++) {
            ...
            ((CharsetEncoderICU) encoder)
                    .setFromUCallback(
                            CoderResult.unmappableForLength(1),
                            CharsetCallback.FROM_U_CALLBACK_SKIP,
                            context3[i]);
        ...
        for (int i = 0; i < context3.length; i++) {
            ...
            ((CharsetEncoderICU) encoder)
                    .setFromUCallback(
                            CoderResult.unmappableForLength(1),
                            CharsetCallback.FROM_U_CALLBACK_SUBSTITUTE,
                            context3[i]);
        ...
        String context2[] = {"X", "C", "D", null};
        byte bytearray[] = {(byte) 0x1b, (byte) 0x2e, (byte) 0x43};
        ...
        ((CharsetDecoderICU) decoder)
                .setToUCallback(
                        CoderResult.OVERFLOW,
                        CharsetCallback.TO_U_CALLBACK_ESCAPE,
                        null); // This callback is not valid.
        for (int i = 0; i < context2.length; i++) {
            ...
            ((CharsetDecoderICU) decoder)
                    .setToUCallback(
                            CoderResult.malformedForLength(1),
                            CharsetCallback.TO_U_CALLBACK_ESCAPE,
                            context2[i]); // This callback is valid.
        ...
        for (int i = 0; i < context3.length; i++) {
            ...
            ((CharsetDecoderICU) decoder)
                    .setToUCallback(
                            CoderResult.malformedForLength(1),
                            CharsetCallback.TO_U_CALLBACK_SKIP,
                            context3[i]);
            result = decoder.decode(bb, cb, true);
            if (!result.isError()) {
                errln(
                        "Error occurred while testing of callbacks for ISO-2022-JP decoder should have occurred.");
            }
        }
    }
    ...
    public void TestInvalidInput() {
        ...
        try {
            ...
            // and wrapped by CoderMalfunctionError. This block is required because
            // CoderMalfunctionError
            // is not an Exception.
        } catch (Exception ex) {
            // IllegalArgumentException is thrown by encodeLoop(CharBuffer,ByteBuffer)
            // implementation
    ...
    public void TestCharsetTestData() {
        ...
        ((CharsetDecoderICU) decoder)
                .setToUCallback(CoderResult.OVERFLOW, CharsetCallback.TO_U_CALLBACK_STOP, null);
    ...
    public void TestSCSUConverter() {
        ...
        byte germanSCSU[] = {(byte) 0xd6, 0x6c, 0x20, 0x66, 0x6c, 0x69, 0x65, (byte) 0xdf, 0x74};

        char russianUTF16[] = {0x041c, 0x043e, 0x0441, 0x043a, 0x0432, 0x0430};
        ...
        char japaneseUTF16[] = {
            0x3000, 0x266a, 0x30ea, 0x30f3, 0x30b4, 0x53ef, 0x611b, 0x3044, 0x3084, 0x53ef, 0x611b,
            0x3044, 0x3084, 0x30ea, 0x30f3, 0x30b4, 0x3002, 0x534a, 0x4e16, 0x7d00, 0x3082, 0x524d,
            0x306b, 0x6d41, 0x884c, 0x3057, 0x305f, 0x300c, 0x30ea, 0x30f3, 0x30b4, 0x306e, 0x6b4c,
            0x300d, 0x304c, 0x3074, 0x3063, 0x305f, 0x308a, 0x3059, 0x308b, 0x304b, 0x3082, 0x3057,
            0x308c, 0x306a, 0x3044, 0x3002, 0x7c73, 0x30a2, 0x30c3, 0x30d7, 0x30eb, 0x30b3, 0x30f3,
            0x30d4, 0x30e5, 0x30fc, 0x30bf, 0x793e, 0x306e, 0x30d1, 0x30bd, 0x30b3, 0x30f3, 0x300c,
            0x30de, 0x30c3, 0x30af, 0xff08, 0x30de, 0x30c3, 0x30ad, 0x30f3, 0x30c8, 0x30c3, 0x30b7,
            0x30e5, 0xff09, 0x300d, 0x3092, 0x3001, 0x3053, 0x3088, 0x306a, 0x304f, 0x611b, 0x3059,
            0x308b, 0x4eba, 0x305f, 0x3061, 0x306e, 0x3053, 0x3068, 0x3060, 0x3002, 0x300c, 0x30a2,
            0x30c3, 0x30d7, 0x30eb, 0x4fe1, 0x8005, 0x300d, 0x306a, 0x3093, 0x3066, 0x8a00, 0x3044,
            0x65b9, 0x307e, 0x3067, 0x3042, 0x308b, 0x3002
        };

        // SCSUEncoder produces a slightly longer result (179B vs. 178B) because of one different
        // choice:
        ...
        decode.reset();
        illegalResult =
                decode.decode(ByteBuffer.wrap(illegalDecoderSrc1), illegalDecoderTrgt, true);
        ...
        // Test malformed
        {
            char[] malformedSequence = {0xD899, 0xDC7F, 0xDC88, 0xDC88, 0xD888, 0xDDF9};
            encode.reset();
    ...
    public void TestICUCanonicalNameConsistency() {
        String[] alias = {"KSC_5601"};
        String[] expected = {"windows-949-2000"};

        for (int i = 0; i < alias.length; i++) {
            String name = CharsetProviderICU.getICUCanonicalName(alias[i]);
            if (!name.equals(expected[i])) {
                errln(
                        "The ICU canonical name in ICU4J does not match that in ICU4C. Result: "
                                + name
                                + "Expected: "
                                + expected[i]);
            }
    ...
    public void TestCharsetICUCodeCoverage() {
        ...
        try {
            ...
            errln(
                    "UnsupportedCharsetException should be thrown for charset \"bogus\" - but got charset "
                            + testCharset.name());
        } catch (UnsupportedCharsetException ex) {
    ...
    public void TestIsFixedWidth() {
        String[] fixedWidth = {"US-ASCII", "UTF32", "ibm-5478_P100-1995"};

        String[] notFixedWidth = {"GB18030", "UTF8", "windows-949-2000", "UTF16"};
        CharsetProvider provider = new CharsetProviderICU();
        Charset charset;
    ...
    public void TestBytesLengthForString() {
        ...
        int[] expected = {
            40, 20, 80, /* changed from 60 to 80 to reflect the updates by #9205 */ 80, 160
        };
    ...
    public void TestCharsetASCII8859BufferHandling() {
        String firstLine =
                "C077693790=|MEMO=|00=|022=|Blanche st and the driveway grate was fault and rotated under my car=|\r\n";
        String secondLine =
                "C077693790=|MEMO=|00=|023=|puncturing the fuel tank. I spoke to the store operator (Ram Reddi –=|\r\n";

        String charsetNames[] = {"ASCII", "ISO-8859-1"};
            ...
            try {
                ...
                if (!result.isUnmappable()) {
                    errln(
                            "Result of encoding "
                                    + charsetNames[i]
                                    + " should be: \"Unmappable\". Instead got: "
                                    + result);
                }
            } catch (IllegalArgumentException ex) {
                errln(
                        "IllegalArgumentException should not have been thrown when encoding: "
                                + charsetNames[i]);
            }
        }
    }
    ...
    public void TestDefaultIgnorableCallback() {
        ...
        String pattern_not_ignorable =
                "[[:^Default_Ignorable_Code_Point:]"
                        +
                        // For test performance, skip large ranges that will likely remain
                        // unassigned
        ...
        while (iter.next()) {
            ...
            try {
                if (encoder.encode(CharBuffer.wrap(Character.toChars(c))).limit() > 0) {
                    errln(
                            "Callback should have ignore default ignorable: U+"
