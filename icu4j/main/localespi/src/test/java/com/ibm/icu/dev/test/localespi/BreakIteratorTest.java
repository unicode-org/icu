...
public class BreakIteratorTest extends TestFmwk {
        ...
        if (TestUtil.isICUExtendedLocale(loc)) {
            ...
            if (isIcuImpl) {
                if (!brkitr.equals(brkitrIcu)) {
                    ...
                    // consistent.
                    //                        logln("INFO: " + method + " returned ICU BreakIterator
                    // for locale " + loc
    ...
    private void compareBreaks(
            String text, BreakIterator jdkBrk, com.ibm.icu.text.BreakIterator icuBrk) {
        jdkBrk.setText(text);
        icuBrk.setText(text);
        ...
        if (jidx != iidx) {
            errln(
                    "FAIL: Different first boundaries (jdk="
                            + jidx
                            + ",icu="
                            + iidx
                            + ") for text:\n"
        ...
        while (true) {
            ...
            if (jidx != iidx) {
                errln(
                        "FAIL: Different boundaries (jdk="
                                + jidx
                                + ",icu="
                                + iidx
                                + "direction=forward) for text:\n"
                                + text);
            }
        ...
        if (jidx != iidx) {
            errln(
                    "FAIL: Different last boundaries (jdk="
                            + jidx
                            + ",icu="
                            + iidx
                            + ") for text:\n"
