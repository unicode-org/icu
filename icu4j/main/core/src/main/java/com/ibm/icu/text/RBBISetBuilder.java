...
class RBBISetBuilder {
    static class RangeDescriptor {
        ...
        /**
         * Test whether this range includes characters from the original Unicode Set named
         * "dictionary".
         *
         * <p>This function looks through the Unicode Sets that the range includes, checking for one
         * named "dictionary"
         */
    ...
    /**
     * The number of the first dictionary char category. If there are no Dictionary categories, set
     * to the last category + 1.
     */
    ...
    void buildRanges() {
        ...
        for (rlRange = fRangeList; rlRange != null; rlRange = rlRange.fNext) {
            for (rlSearchRange = fRangeList;
                    rlSearchRange != rlRange;
    ...
    /**
     * Merge two character categories that have been identified as having equivalent behavior. The
     * ranges belonging to the second category (table column) will be added to the first.
     *
    ...
    void mergeCategories(IntPair categories) {
        ...
        assert ((categories.first < fDictCategoriesStart
                        && categories.second < fDictCategoriesStart)
                || (categories.first >= fDictCategoriesStart
                        && categories.second >= fDictCategoriesStart));
