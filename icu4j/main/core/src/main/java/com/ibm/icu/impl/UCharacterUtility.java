...
/**
 * ****************************************************************************** Copyright (C)
 * 1996-2004, International Business Machines Corporation and * others. All Rights Reserved. *
 * ******************************************************************************
 */
 ...
 * Does not have an analog in the JDK.
 *
 * @author Syn Wee Quek
 * @since sep2900
 */
public final class UCharacterUtility {
    ...
    /**
     * Retrieves a null terminated substring from an array of bytes. Substring is a set of non-zero
     * bytes starting from argument start to the next zero byte. If the first byte is a zero, the
     * next byte will be taken as the first byte.
     *
     * @param str stringbuffer to store data in, data will be store with each byte as a char
    ...
    /**
     * Compares a null terminated substring from an array of bytes. Substring is a set of non-zero
     * bytes starting from argument start to the next zero byte. if the first byte is a zero, the
     * next byte will be taken as the first byte.
     *
     ...
     * @return the end position of the substring within str if matches otherwise a -1
     */
    ...
    /**
     * Skip null terminated substrings from an array of bytes. Substring is a set of non-zero bytes
     * starting from argument start to the next zero byte. If the first byte is a zero, the next
     * byte will be taken as the first byte.
     *
     * @param array byte array
     ...
     * substring is a set of non-zero bytes starting from argument start to the byte of the argument
     * value. skips up to a max number of characters
     *
    ...
    /**
     * Minimum suffix value that indicates if a character is non character. Unicode 3.0 non
     * characters
     */
