...
/**
 * A collection of rules used by a RuleBasedNumberFormat to format and parse numbers. It is the
 * responsibility of a RuleSet to select an appropriate rule for formatting a particular number and
 * dispatch control to it, and to arbitrate between different rules when parsing a number.
 */
final class NFRuleSet {
    ...
    /**
     * These are a pile of fraction rules in declared order. They may have alternate ways to
     * represent fractions.
     */
    ...
    /**
     * True if the rule set is a fraction rule set. A fraction rule set is a rule set that is used
     * to format the fractional part of a number. It is called from a >> substitution in another
     * rule set's fraction rule, and is only called upon to format values between 0 and 1. A
     * fraction rule set has different rule-selection behavior than a regular rule set.
     */
     ...
     * @param descriptions An array of Strings representing rule set descriptions. On exit, this
     *     rule set's entry in the array will have been stripped of its rule set name and any
     *     trailing whitespace.
     * @param index The index into "descriptions" of the description for the rule to be constructed
     */
        ...
        if (description.charAt(0) == '%') {
            ...
            if (pos < 2) {
                ...
                if (!this.isParseable) {
                    ruleName =
                            ruleName.substring(
                                    0, ruleName.length() - 8); // Remove the @noparse from the name
                }
                this.name = ruleName;

                //noinspection StatementWithEmptyBody
                while (pos < description.length()
                        && PatternProps.isWhiteSpace(description.charAt(++pos))) {}
    ...
    /**
     * Construct the subordinate data structures used by this object. This function is called by the
     * RuleBasedNumberFormat constructor after all the rule sets have been created to actually parse
     * the description and build rules from it. Since any rule set can refer to any other rule set,
     * we have to have created all of them before we can create anything else.
     *
    ...
    /**
     * Flags this rule set as a fraction rule set. This function is called during the construction
     * process once we know this rule set is a fraction rule set. We don't know a rule set is a
     * fraction rule set until we see it used somewhere. This function is not ad must not be called
     * at any time other than during construction of a RuleBasedNumberFormat.
     */
     ...
     *
     * @return A textual representation of a rule set. This won't necessarily be the same
     *     description that the rule set was constructed with, but it will produce the same results.
     */
    @Override
     ...
     * @param pos The position in toInsertInto where the result of this operation is to be inserted
     */
    ...
    /**
     * If the value passed to findRule() is a positive integer, findRule() uses this function to
     * select the appropriate rule. The result will generally be the rule with the highest base
     * value less than or equal to the number. There is one exception to this: If that rule has two
     * substitutions and a base value that is not an even multiple of its divisor, and the number
     * itself IS an even multiple of the rule's divisor, then the result will be the rule that
     * preceded the original result in the rule list. (This behavior is known as the "rollback
     * rule", and is used to handle optional text: a rule with optional text is represented
     * internally as two rules, and the rollback rule selects appropriate between them. This avoids
    ...
    private NFRule findNormalRule(long number) {
        ...
        if (hi > 0) {
            ...
            if (hi == 0) { // bad rule set
                throw new IllegalStateException(
                        "The rule set " + name + " cannot format the value " + number);
            }
            ...
            if (result.shouldRollBack(number)) {
                if (hi == 1) { // bad rule set
                    throw new IllegalStateException(
                            "The rule set "
                                    + name
                                    + " cannot roll back from the rule '"
                                    + result
                                    + "'");
                }
                result = rules[hi - 2];
    ...
    /**
     * If this rule is a fraction rule set, this function is used by findRule() to select the most
     * appropriate rule for formatting the number. Basically, the base value of each rule in the
     * rule set is treated as the denominator of a fraction. Whichever denominator can produce the
     * fraction closest in value to the number passed in is the result. If there's a tie, the
     * earlier one in the list wins. (If there are two rules in a row with the same base value, the
     * first one is used when the numerator of the fraction would be 1, and the second rule is used
     * the rest of the time.
     *
     * @param number The number being formatted (which will always be a number between 0 and 1)
     * @return The rule to use to format this number
     */
    ...
    /**
     * Parses a string. Matches the string to be parsed against each of its rules (with a base value
     * less than upperBound) and returns the value produced by the rule that matched the most
     * characters in the source string.
     *
     * @param text The string to parse
     * @param parsePosition The initial position is ignored and assumed to be 0. On exit, this
     *     object has been updated to point to the first character position this rule set didn't
     *     consume.
     * @param upperBound Limits the rules that can be allowed to match. Only rules whose base values
     *     are strictly less than upperBound are considered.
     * @return The numerical result of parsing this string. This will be the matching rule's base
     *     value, composed appropriately with the results of matching any of its substitutions. The
     *     object will be an instance of Long if it's an integral value; otherwise, it will be an
     *     instance of Double. This function always returns a valid object: If nothing matched the
     *     input string at all, this function returns Long.valueOf(0), and the parse position is
     *     left unchanged.
     */
    public Number parse(
            String text,
        ...
        for (int nonNumericalRuleIdx = 0;
                nonNumericalRuleIdx < nonNumericalRules.length;
                nonNumericalRuleIdx++) {
            NFRule nonNumericalRule = nonNumericalRules[nonNumericalRuleIdx];
            if (nonNumericalRule != null
                    && ((nonNumericalExecutedRuleMask >> nonNumericalRuleIdx) & 1) == 0) {
    ...
    public void setDecimalFormatSymbols(DecimalFormatSymbols newSymbols) {
        ...
        if (fractionRules != null) {
            for (int nonNumericalIdx = IMPROPER_FRACTION_RULE_INDEX;
                    nonNumericalIdx <= DEFAULT_RULE_INDEX;
                    nonNumericalIdx++) {
                if (nonNumericalRules[nonNumericalIdx] != null) {
                    for (NFRule rule : fractionRules) {
                        if (nonNumericalRules[nonNumericalIdx].getBaseValue()
                                == rule.getBaseValue()) {
