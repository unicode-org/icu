...
abstract class FieldModifierImpl {
    ...
    public static FieldModifierImpl forName(
            PersonName.FieldModifier modifierID, PersonNameFormatterImpl formatterImpl) {
        switch (modifierID) {
            case INFORMAL:
            ...
            case INITIAL:
                return new InitialModifier(
                        formatterImpl.getLocale(),
                        formatterImpl.getInitialPattern(),
                        formatterImpl.getInitialSequencePattern());
            case RETAIN:
            ...
            case GENITIVE:
                // no built-in implementation for deriving genitive from nominative; PersonName
                // object must supply
                return NOOP_MODIFIER;
            case VOCATIVE:
                // no built-in implementation for deriving vocative from nominative; PersonName
                // object must supply
                return NOOP_MODIFIER;
            default:
    ...
    /**
     * A field modifier that just returns the field value unmodified. This is used to implement the
     * default behavior of the "informal" and "core" modifiers ("real" informal or core variants
     * have to be supplied or calculated by the PersonName object).
     */
    private static final FieldModifierImpl NOOP_MODIFIER =
            new FieldModifierImpl() {
                @Override
    ...
    /**
     * A field modifier that just returns the empty string. This is used to implement the default
     * behavior of the "prefix" modifier ("real" prefix variants have to be supplied to calculated
     * by the PersonName object).
     */
    ...
    /**
     * A field modifier that returns the field value converted to ALL CAPS. This is the default
     * behavior for the "allCaps" modifier.
     */
     ...
     * This is the default behavior of the "initialCap" modifier.
     */
    ...
    /**
     * A field modifier that returns the field value converted into one or more initials. This is
     * the first grapheme cluster of each word in the field value, modified using the
     * initialPattern/initial resource value from the locale data, and strung together using the
     * initialPattern/initialSequence resource value from the locale data. (In English, these
     * patterns put periods after each initial and connect them with spaces.) This is default
     * behavior of the "initial" modifier.
     */
    static class InitialModifier extends FieldModifierImpl {
        ...
        public String modifyField(String fieldValue) {
            ...
            for (int wordEnd = bi.next();
                    wordEnd != BreakIterator.DONE;
                    wordStart = wordEnd, wordEnd = bi.next()) {
    ...
    /**
     * A field modifier that simply returns the first grapheme cluster in the field value. This is
     * the default implementation of the "monogram" modifier.
     */
    ...
    private static String getFirstGrapheme(String s) {
        ...
        // (currently, no locale overrides the grapheme-break rules, so we just use "root" instead
        // of passing in the locale)
