 ...
 *
 * <p>In ICU4J, the {@code MessageFormatter} class is the next iteration of {@link
 * com.ibm.icu.text.MessageFormat}. This new version will build on the lessons learned from using
 * MessageFormat for 25 years in various environments, when used directly or as a base for other
 * public APIs.
 *
 * <p>The effort to design a succesor to {@code MessageFormat} will result in a specification
 * referred to as MessageFormat 2.0. The reasoning for this effort is shared in the <a
 * target="github"
 * href="https://github.com/unicode-org/message-format-wg/blob/main/docs/why_mf_next.md">“Why
 * MessageFormat needs a successor”</a> document.
 *
 * <p>MessageFormat 2.0 will be more modular and easier to port and backport. It will also provide
 * extension points via interfaces to allow users to supply new formatters and selectors without
 * having to modify the specification. ICU will eventually include support for new formatters, such
 * as intervals, relative time, lists, measurement units, personal names, and more, as well as the
 * ability for users to supply their own custom implementations. These will potentially support use
 * cases like grammatical gender, inflection, markup regimes (such as those require for
 * text-to-speech), and other complex message management needs.
 *
 * <p>The MessageFormat Working Group, which develops the new data model, semantics, and syntax, is
 * hosted on <a target="github" href="https://github.com/unicode-org/message-format-wg">GitHub</a>.
 * The current specification for the syntax and data model can be found <a target="github"
 * href="https://github.com/unicode-org/message-format-wg/blob/main/spec/syntax.md">here</a>.
 *
 * <p>This technical preview implements enough functions for {@code MessageFormatter} to be useful
 * in many situations, but the final set of functions and the parameters accepted by those functions
 ...
 *
 * <p>The tech preview implementation comes with formatters for numbers ({@code :number}), date /
 * time ({@code :datetime}, {@code :date}, {@code :time}), plural selectors ({@code :number} with
 * options for {@code plural} and {@code ordinal} selection), and general selector ({@code
 * :string}), very similar to what {@code MessageFormat} offers.
 *
 * <p>The <a target="github"
 * href="https://github.com/unicode-org/icu/tree/main/icu4j/main/core/src/test/java/com/ibm/icu/dev/test/message2">ICU
 * test code</a> covers most features, and has examples of how to make custom placeholder
 * formatters; you can look for classes that implement {@code com.ibm.icu.message2.FormatterFactory}
 * (they are named {@code Custom*Test.java}).
 *
 * <p>The complete list of valid options for each function, and how they infulence the results, can
...
public class MessageFormatter {
    ...
    private MessageFormatter(Builder builder) {
        ...
        modelFormatter =
                new MFDataModelFormatter(
                        dataModel, locale, errorHandlingBehavior, bidiIsolation, functionRegistry);
    }
    ...
    /**
     * Get the locale to use for all the formatting and selections in the current {@code
     * MessageFormatter}.
     *
     * @return the locale.
     * @internal ICU 72 technology preview
    ...
    /**
     * Get the {@link ErrorHandlingBehavior} to use when encountering errors in the current {@code
     * MessageFormatter}.
     *
    ...
    /**
     * Get the {@link BidiIsolation} algorithm to use when formatting mixed message parts with mixed
     * direction.
     *
    ...
    /**
     * Get the pattern (the serialized message in MessageFormat 2 syntax) of the current {@code
     * MessageFormatter}.
     *
     * <p>If the {@code MessageFormatter} was created from an {@link MFDataModel} the this string is
     ...
     *
     * <p>This data model is similar to the functionality we have today in {@link
     * com.ibm.icu.text.MessagePatternUtil} maybe even a bit more higher level.
     *
    ...
    /**
     * Formats a map of objects by iterating over the MessageFormat's pattern, with the plain text
     * “as is” and the arguments replaced by the formatted objects.
     *
     ...
     * @throws IllegalArgumentException when something goes wrong (for example wrong argument type,
    ...
    /**
     * Not yet implemented: formats a map of objects by iterating over the MessageFormat's pattern,
     ...
     *
     * <p>Parsing errors and data model errors always throw and will not be affected by this
     * setting.<br>
     * But resolution errors and formatting errors will either try to fallback (if possible) or
     * throw, depending on this setting.
     *
     * <p>Used in conjunction with the {@link
     ...
     *
     * <p>They can be either ignored, or will implement the default algorithm described in the
     * MessageFormat 2 specification.
     *
     * <p>Used in conjunction with the {@link
     * MessageFormatter.Builder#setBidiIsolation(MessageFormatter.BidiIsolation)} method.
     *
     * @internal ICU 77 technology preview
     * @deprecated This API is for technology preview only.
     */
    ...
    public static enum BidiIsolation {
        /**
         * Ignore any text direction mixture, don't introduce bidi control characters in the
         * formatted result.
         *
         * @internal ICU 77 technology preview
         * @deprecated This API is for technology preview only.
        ...
        NONE,
        /**
         * Wrap direction mixtures in bidi control characters as described in the MessageFormat 2
         * specification.
         *
         * @internal ICU 77 technology preview
         * @deprecated This API is for technology preview only.
    ...
    public static class Builder {
        ...
        /**
         * Sets the {@link ErrorHandlingBehavior} to use when encountering errors at formatting
         * time.
         *
         * <p>The default value is {@code ErrorHandlingBehavior.BEST_EFFORT}, trying to fallback.
         *
        ...
        /**
         * Sets the {@link BidiIsolation} to introduce bidi control characters / tags as described
         * in the MessageFormat 2 specification.
         *
         * <p>The default value is {@code BidiIsolation.NONE}.
         *
        ...
        /**
         * Sets an instance of {@link MFFunctionRegistry} that should register any custom functions
         * used by the message.
         *
         * <p>There is no need to do this in order to use standard functions (for example date /
         * time / number formatting, plural / ordinal / literal selection).<br>
         * The exact set of standard functions, with the types they format and the options they
         * accept is still TBD.
         *
