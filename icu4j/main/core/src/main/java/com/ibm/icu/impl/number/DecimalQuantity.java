...
public interface DecimalQuantity extends PluralRules.IFixedDecimal {
    /**
     * Sets the minimum integer digits that this {@link DecimalQuantity} should generate. This
    ...
    /**
     * Sets the minimum fraction digits that this {@link DecimalQuantity} should generate. This
     * method does not perform rounding.
     *
    ...
    /**
     * Truncates digits from the upper magnitude of the number in order to satisfy the specified
    ...
    /**
     * Rounds the number to the nearest multiple of 5 at the specified magnitude. For example, when
     * magnitude == -2, this performs rounding to the nearest 0.05.
     *
     * @param magnitude The magnitude at which the digit should become either 0 or 5.
     * @param mathContext Rounding strategy.
     */
    ...
    /**
     * Scales the number by a power of ten. For example, if the value is currently "1234.56",
     * calling this method with delta=-3 will change the value to "1.23456".
     *
    ...
    /**
     * @return The value of the (suppressed) exponent after the number has been put into a notation
     *     with exponents (ex: compact, scientific). Ex: given the number 1000 as "1K" / "1E3", the
     *     return value will be 3 (positive).
     */
    public int getExponent();

    /**
     * Adjusts the value for the (suppressed) exponent stored when using notation with exponents
     * (ex: compact, scientific).
     *
     * <p>Adjusting the exponent is decoupled from {@link #adjustMagnitude} in order to allow
     * flexibility for {@link StandardPlural} to be selected in formatting (ex: for compact
     * notation) either with or without the exponent applied in the value of the number.
     *
     * @param delta The value to adjust the exponent by.
     */
    ...
    /**
     * @return Whether the value represented by this {@link DecimalQuantity} is zero, infinity, or
     *     NaN.
     */
    ...
    /**
     * Returns a long approximating the decimal quantity. A long can only represent the integral
     * part of the number. Note: this method incorporates the value of {@code getExponent} (for
     * cases such as compact notation) to return the proper long value represented by the result.
     *
     * @param truncateIfOverflow if false and the number does NOT fit, fails with an error. See
     *     comment about call site guards in DecimalQuantity_AbstractBCD.java
    ...
    /**
     * Gets the smallest power of ten that needs to be displayed. The value returned by this
     * function will be bounded between -minFrac and -maxFrac.
     *
    ...
    /**
     * Returns the string using ASCII digits and using exponential notation for non-zero exponents,
     * following the UTS 35 specification for plural rule samples.
     */
