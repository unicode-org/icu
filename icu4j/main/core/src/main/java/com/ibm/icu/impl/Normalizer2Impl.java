...
/**
 * Low-level implementation of the Unicode Normalization Algorithm. For the data structure and
 * details see the documentation at the end of C++ normalizer2impl.h and in the design doc at
...
public final class Normalizer2Impl {
    public static final class Hangul {
        ...
        /**
         * Decomposes c, which must be a Hangul syllable, into buffer and returns the length of the
         * decomposition (2 or 3).
         */
        ...
        /**
         * Decomposes c, which must be a Hangul syllable, into buffer. This is the raw, not
         * recursive, decomposition. Its length is always 2.
         */
    ...
    /**
     * Writable buffer that takes care of canonical ordering. Its Appendable methods behave like the
     * C++ implementation's appendZeroCC() methods.
     *
     * <p>If dest is a StringBuilder, then the buffer writes directly to it. Otherwise, the buffer
     * maintains a StringBuilder for intermediate text segments until no further changes are
     * necessary and whole segments are appended. append() methods that take combining-class values
     * always write to the StringBuilder. Other append() methods flush and append to the Appendable.
     */
    public static final class ReorderingBuffer implements Appendable {
        ...
        public void append(
                CharSequence s, int start, int limit, boolean isNFD, int leadCC, int trailCC) {
            if (start == limit) {
                return;
            }
        ...
        /**
         * Flushes from the intermediate StringBuilder to the Appendable, if they are different
         * objects. Used after recomposition. Must be called at the end when writing to a
         * non-StringBuilder Appendable.
         */
        public void flush() {
        ...
        /**
         * Flushes from the intermediate StringBuilder to the Appendable, if they are different
         * objects. Then appends the new text to the Appendable or StringBuilder. Normally used
         * after quick check loops find a non-empty sequence.
         */
    ...
    public static final class UTF16Plus {
        ...
        public static boolean equal(
                CharSequence s1, int start1, int limit1, CharSequence s2, int start2, int limit2) {
    ...
    public Normalizer2Impl load(ByteBuffer bytes) {
        try {
            ...
            ICUBinary.skipBytes(
                    bytes, (nextOffset - offset) - trieLength); // skip padding after trie bytes
    ...
    public void addLcccChars(UnicodeSet set) {
        ...
        while (normTrie.getRange(
                start, CodePointMap.RangeOption.FIXED_LEAD_SURROGATES, INERT, null, range)) {
    ...
    /**
     * Builds the canonical-iterator data for this instance. This is required before any of {@link
     * #isCanonSegmentStarter(int)} or {@link #getCanonStartSet(int, UnicodeSet)} are called, or
     * else they crash.
     *
     * @return this
     */
                                ...
                                if (norm16_2 >= minNoNo) {
                                    while ((mapping += Character.charCount(c2)) < limit) {
                                        ...
                                        if ((c2Value & CANON_NOT_SEGMENT_STARTER) == 0) {
                                            mutableTrie.set(
                                                    c2, c2Value | CANON_NOT_SEGMENT_STARTER);
                                        }
                                    }
                                }
                            }
     ...
     *
     * @param c code point
     * @return c's raw decomposition, if it has one; returns null if it does not have a
     *     decomposition
     */
    ...
    /**
     * Returns true if code point c starts a canonical-iterator string segment. <b>{@link
     * #ensureCanonIterData()} must have been called before this method, or else this method will
     * crash.</b>
     *
     * @param c A Unicode code point.
    ...
    /**
     * Returns true if there are characters whose decomposition starts with c. If so, then the set
     * is cleared and then filled with those characters. <b>{@link #ensureCanonIterData()} must have
     * been called before this method, or else this method will crash.</b>
     *
     * @param c A Unicode code point.
     * @param set A UnicodeSet to receive the characters whose decompositions start with c, if there
     *     are any.
    ...
    /**
     * Decomposes s[src, limit[ and writes the result to dest. limit can be NULL if src is
     * NUL-terminated. destLengthEstimate is the initial dest buffer capacity and can be -1.
     */
    public void decompose(
            CharSequence s, int src, int limit, StringBuilder dest, int destLengthEstimate) {
    ...
    public boolean compose(
        ...
        for (; ; ) {
            // Fast path: Scan over a sequence of characters below the minimum "no or maybe" code
            // point,
            ...
            if (norm16 < minMaybeNo) { // minNoNo <= norm16 < minMaybeNo
                ...
                if (c < Hangul.JAMO_T_BASE) {
                    ...
                    if (l < Hangul.JAMO_L_COUNT) {
                        ...
                        // (which would complicate the code and may not work with strange custom
                        // data)
            ...
            decomposeShort(
                    s, prevSrc, src, false /* !stopAtCompBoundary */, onlyContiguous, buffer);
            // Decompose until the next boundary.
            src =
                    decomposeShort(
                            s, src, limit, true /* stopAtCompBoundary */, onlyContiguous, buffer);
    ...
    /**
     * Very similar to compose(): Make the same changes in both places if relevant. doSpan:
     * spanQuickCheckYes (ignore bit 0 of the return value) !doSpan: quickCheck
     *
     * @return bits 31..1: spanQuickCheckYes (==s.length() if "yes") and bit 0: set if "maybe";
     *     otherwise, if the span length&lt;s.length() then the quick check result is "no"
     */
    public int composeQuickCheck(
            CharSequence s, int src, int limit, boolean onlyContiguous, boolean doSpan) {
        int qcResult = 0;
        int prevBoundary = src;
        int minNoMaybeCP = minCompNoMaybeCP;
                    ...
                    for (; ; ) {
                        if (norm16 < MIN_YES_YES_WITH_CC) {
                            if (!doSpan) {
                                ...
                                return prevBoundary
                                        << 1; // spanYes does not care to know it's "maybe"
                            }
                        }
                        if (src == limit) {
                            return (src << 1) | qcResult; // "yes" or "maybe"
    ...
    public void composeAndAppend(
            CharSequence s, boolean doCompose, boolean onlyContiguous, ReorderingBuffer buffer) {
            ...
            if (0 != firstStarterInSrc) {
                int lastStarterInDest =
                        findPreviousCompBoundary(
                                buffer.getStringBuilder(), buffer.length(), onlyContiguous);
        ...
        for (; ; ) {
            ...
            for (prevSrc = src; src != limit; ) {
                if ((c = s.charAt(src)) < minLcccCP) {
                    ...
                    if (UTF16Plus.isLeadSurrogate(c)) {
                        char c2;
                        if ((src + 1) != limit
                                && Character.isLowSurrogate(c2 = s.charAt(src + 1))) {
                            c = Character.toCodePoint((char) c, c2);
                        }
                    }
            ...
            if (src != prevSrc) {
                ...
                if (prevFCD16 < 0) {
                    ...
                    int p = src - 1;
                    if (Character.isLowSurrogate(s.charAt(p))
                            && prevSrc < p
                        ...
                        prevFCD16 =
                                getFCD16FromNormData(
                                        Character.toCodePoint(s.charAt(p), s.charAt(p + 1)));
        ...
        if (!buffer.isEmpty()) {
            ...
            if (0 != firstBoundaryInSrc) {
                int lastBoundaryInDest =
                        findPreviousFCDBoundary(buffer.getStringBuilder(), buffer.length());
                StringBuilder middle =
                        new StringBuilder(
                                (buffer.length() - lastBoundaryInDest) + firstBoundaryInSrc + 16);
    ...
    public boolean hasDecompBoundaryBefore(int c) {
        return c < minLcccCP
                || (c <= 0xffff && !singleLeadMightHaveNonZeroFCD16(c))
    ...
    public boolean norm16HasDecompBoundaryBefore(int norm16) {
        ...
        return (firstUnit & MAPPING_HAS_CCC_LCCC_WORD) == 0
                || (extraData.charAt(mapping - 1) & 0xff00) == 0;
    }
    ...
    private boolean isMaybe(int norm16) {
        return minMaybeNo <= norm16 && norm16 <= JAMO_VT;
    }
    ...
    /**
     * A little faster and simpler than isDecompYesAndZeroCC() but does not include the MaybeYes
     * which combine-forward and have ccc=0.
     */
    ...
    private int getCompositionsListForComposite(int norm16) {
        ...
        return list
                + // mapping in maybeYesCompositions
                1
                + // +1 to skip the first unit with the mapping length
    ...
    /**
     * Finds the recomposition result for a forward-combining "lead" character, specified with a
     * pointer to its compositions list, and a backward-combining "trail" character.
     *
     * <p>If the lead and trail characters combine, then this function returns the following
     ...
     *
     * <p>The compositions list has (trail, compositeAndFwd) pair entries, encoded as either pairs
     * or triples of 16-bit units. The last entry has the high bit of its first unit set.
     *
     * <p>The list is sorted by ascending trail characters (there are no duplicates). A linear
     * search is used.
     *
     * <p>See normalizer2impl.h for a more detailed description of the compositions list format.
     */
            ...
            for (; ; ) {
                if (key1 > (firstUnit = extraData.charAt(list))) {
                    ...
                    if (key2 > (secondUnit = extraData.charAt(list + 1))) {
                        ...
                        return ((secondUnit & ~COMP_2_TRAIL_MASK) << 16)
    ...
    private void addComposites(int list, UnicodeSet set) {
        ...
        do {
            ...
            if ((firstUnit & COMP_1_TRIPLE) == 0) {
                ...
                compositeAndFwd =
                        ((extraData.charAt(list + 1) & ~COMP_2_TRAIL_MASK) << 16)
                                | extraData.charAt(list + 2);
                list += 3;
            }
                ...
                if (isJamoVT(norm16)) {
                    ...
                    if (c < Hangul.JAMO_T_BASE) {
                        ...
                        if (prev < Hangul.JAMO_L_COUNT) {
                            ...
                            char t;
                            if (p != sb.length()
                                    && (t = (char) (sb.charAt(p) - Hangul.JAMO_T_BASE))
                                            < Hangul.JAMO_T_COUNT) {
                                ++p;
            ...
            if (isJamoL(norm16)) {
                ...
                if (norm16 > minYesNo) { // composite 'a' has both mapping & compositions list
                    list += // mapping pointer
                            1
                                    + // +1 to skip the first unit with the mapping length
                                    (extraData.charAt(list)
                                            & MAPPING_LENGTH_MASK); // + mapping length
                }
            }
    ...
    /**
     * Does c have a composition boundary before it? True if its decomposition begins with a
     * character that has ccc=0 && NFC_QC=Yes (isCompYesAndZeroCC()). As a shortcut, this is true if
     * c itself has ccc=0 && NFC_QC=Yes (isCompYesAndZeroCC()) so we need not decompose.
     */
    ...
    private void addToStartSet(MutableCodePointTrie mutableTrie, int origin, int decompLead) {
        ...
        if ((canonValue & (CANON_HAS_SET | CANON_VALUE_MASK)) == 0 && origin != 0) {
            ...
            if ((canonValue & CANON_HAS_SET) == 0) {
                ...
                canonValue =
                        (canonValue & ~CANON_VALUE_MASK) | CANON_HAS_SET | canonStartSets.size();
