...
/**
 * Provides an immutable list of languages/locales in priority order. The string format is based on
 * the Accept-Language format (<a href="https://tools.ietf.org/html/rfc2616#section-14.4">RFC 2616
 * Section 14.4</a>), such as "af, en, fr;q=0.9". Syntactically it is slightly more lenient, in
 * allowing extra whitespace between elements, extra commas, and more than 3 decimals (on input).
 * The qvalues must be between 0 and 1.
 *
 * <p>In theory, Accept-Language indicates the relative 'quality' of each item, but in practice, all
 * of the browsers just take an ordered list, like "en, fr, de", and synthesize arbitrary quality
 * values that put these in the right order, like: "en, fr;q=0.7, de;q=0.3". The quality values in
 * these de facto semantics thus have <b>nothing</b> to do with the relative qualities of the
 * original. Accept-Language also doesn't specify the interpretation of multiple instances, eg what
 * "en, fr, en;q=.5" means.
 *
 * <p>There are various ways to build a LocalePriorityList, such as using the following equivalent
 * patterns:
 *
 * <pre>
 ...
 * </pre>
 *
 * When the list is built, the internal values are sorted in descending order by weight, and then by
 * input order. That is, if two languages/locales have the same weight, the first one in the
 * original order comes first. If exactly the same language tag appears multiple times, the last one
 * wins.
 *
 * <p>There are two options when building. If preserveWeights are on, then "de;q=0.3, ja;q=0.3, en,
 * fr;q=0.7, de " would result in the following:
 *
 * <pre> en;q=1.0
 * de;q=1.0
 * fr;q=0.7
 * ja;q=0.3</pre>
 *
 * If it is off (the default), then all weights are reset to 1.0 after reordering. This is to match
 * the effect of the Accept-Language semantics as used in browsers, and results in the following:
 *
 * <pre> en;q=1.0
 * de;q=1.0
 * fr;q=1.0
...
public class LocalePriorityList implements Iterable<ULocale> {
    ...
    /**
     * Creates a Builder and adds a locale with a specified weight. A zero or negative weight leads
     * to removing the locale. A weight greater than 1 is pinned to 1.
     *
    ...
    /**
     * Returns the weight for a given language/locale, or null if there is none. Note that the
     * weights may be adjusted from those used to build the list.
     *
    ...
    /**
     * Returns the locales as an immutable Set view. The set has the same iteration order as this
     * object itself.
     *
     * @return the locales
     * @stable ICU 65
    ...
    public static class Builder {
        /**
         * These store the input languages and weights, in chronological order, where later
         * additions override previous ones.
         */
        private Map<ULocale, Double> languageToWeight;

        /**
         * The builder is reusable but rarely reused. Avoid cloning the map when not needed. Exactly
         * one of languageToWeight and built is null.
         */
        ...
        /**
         * Creates a LocalePriorityList. This is equivalent to {@link Builder#build(boolean)
         * Builder.build(false)}.
         *
        ...
        /**
         * Adds a locale with a specified weight. Overrides any previous weight for the locale. A
         * zero or negative weight leads to removing the locale. A weight greater than 1 is pinned
         * to 1.
         *
