...
public final class UResource {
    /**
     * Represents a resource bundle item's key string. Avoids object creations as much as possible.
     * Mutable, not thread-safe. For permanent storage, use clone() or toString().
     */
    public static final class Key implements CharSequence, Cloneable, Comparable<Key> {
        ...
        /**
         * Mutates this key for a new NUL-terminated resource key string. The corresponding
         * ASCII-character bytes are not copied and must not be changed during the lifetime of this
         * key (or until the next setBytes() call) and lifetimes of subSequences created from this
         * key.
         *
        ...
        /** {@inheritDoc} Does not clone the byte array. */
        @Override
        public Key clone() {
            try {
        ...
        /**
         * @return true if the substring of this key starting from the offset contains the same
         *     characters as the other sequence.
         */
    ...
    /**
     * Interface for iterating over a resource bundle array resource. Does not use Java Iterator to
    ...
    /**
     * Interface for iterating over a resource bundle table resource. Does not use Java Iterator to
     * reduce object creations.
     */
    ...
    /**
     * Represents a resource bundle item's value. Avoids object creations as much as possible.
    ...
    public abstract static class Value {
        ...
        /**
         * @return ICU resource type like {@link UResourceBundle#getType()}, for example, {@link
        ...
        /**
         * Is this a no-fallback/no-inheritance marker string? Such a marker is used for CLDR
         * no-fallback data values of "∅∅∅" when enumerating tables with fallback from the specific
         * resource bundle to root.
         *
    ...
    public abstract static class Sink {
        /**
         * Called once for each bundle (child-parent-...-root). The value is normally an array or
         * table resource, and implementations of this method normally iterate over the tree of
         * resource items stored there.
         *
         * @param key Initially the key string of the enumeration-start resource. Empty if the
         *     enumeration starts at the top level of the bundle. Reuse for output values from Array
         *     and Table getters.
         * @param value Call getArray() or getTable() as appropriate. Then reuse for output values
         *     from Array and Table getters.
         * @param noFallback true if the bundle has no parent; that is, its top-level table has the
         *     nofallback attribute, or it is the root bundle of a locale tree.
         */
