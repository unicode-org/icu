...
/**
 * <code>TimeZoneNames</code> is an abstract class representing the time zone display name data
 ...
 * names. A meta zone can be shared by multiple time zones. Also a time zone may have multiple meta
 ...
 * <p><b>Note:</b>
 *
 * <p>{@link TimeZoneFormat} assumes an instance of <code>TimeZoneNames</code> is immutable. If you
 * want to provide your own <code>TimeZoneNames</code> implementation and use it with {@link
 * TimeZoneFormat}, you must follow the contract.
 *
 ...
 *
 * <p>{@link #getTimeZoneDisplayName(String, NameType)} is designed for accessing a name only used
 * by a single time zone. But is not necessarily mean that a subclass implementation use the same
 * model with CLDR. A subclass implementation may provide time zone names only through {@link
 * #getTimeZoneDisplayName(String, NameType)}, or only through {@link
 * #getMetaZoneDisplayName(String, NameType)}, or both.
 *
 * <p>The default <code>TimeZoneNames</code> implementation returned by {@link
 * #getInstance(ULocale)} uses the locale data imported from CLDR. In CLDR, set of meta zone IDs and
 * mappings between zone IDs and meta zone IDs are shared by all locales. Therefore, the behavior of
 * {@link #getAvailableMetaZoneIDs()}, {@link #getAvailableMetaZoneIDs(String)}, {@link
 * #getMetaZoneID(String, long)}, and {@link #getReferenceZoneID(String, String)} won't be changed
...
public abstract class TimeZoneNames implements Serializable {
    ...
    /**
     * Returns an instance of <code>TimeZoneNames</code> for the specified {@link java.util.Locale}.
     *
    ...
    /**
     * Returns an instance of <code>TimeZoneNames</code> containing only short specific zone names
     * ({@link NameType#SHORT_STANDARD} and {@link NameType#SHORT_DAYLIGHT}), compatible with the
     * IANA tz database's zone abbreviations (not localized). <br>
     * Note: The input locale is used for resolving ambiguous names (e.g. "IST" is parsed as Israel
     * Standard Time for Israel, while it is parsed as India Standard Time for all other regions).
     ...
     *     not have a corresponding meta zone at the given date or the implementation does not
     *     support meta zones, null is returned.
     * @stable ICU 49
     */
     ...
     * @return The reference zone ID ("golden zone" in the LDML specification) for the given time
     *     zone ID for the region. If the meta zone is unknown or the implementation does not
     *     support meta zones, null is returned.
     * @stable ICU 49
     */
     ...
     *     name for the given meta zone with the specified type or the implementation does not
     *     provide any display names associated with meta zones, null is returned.
     * @stable ICU 49
     */
     ...
     *
     * <p><b>Note:</b> This method calls the subclass's {@link #getTimeZoneDisplayName(String,
     * NameType)} first. When the result is null, this method calls {@link #getMetaZoneID(String,
     * long)} to get the meta zone ID mapped from the time zone, then calls {@link
     * #getMetaZoneDisplayName(String, NameType)}.
     *
     * @param tzID The canonical time zone ID.
     * @param type The display name type. See {@link TimeZoneNames.NameType}.
     * @param date The date
     * @return The display name for the time zone at the given date. When this object does not have
     *     a localized display name for the time zone with the specified type and date, null is
     *     returned.
     * @stable ICU 49
     */
    ...
    /**
     * Returns the display name of the time zone. Unlike {@link #getDisplayName(String, NameType,
     * long)}, this method does not get a name from a meta zone used by the time zone.
     *
     * @param tzID The canonical time zone ID.
     * @param type The display name type. See {@link TimeZoneNames.NameType}.
     * @return The display name for the time zone. When this object does not have a localized
     *     display name for the given time zone with the specified type, null is returned.
     * @stable ICU 49
     */
    public abstract String getTimeZoneDisplayName(String tzID, NameType type);

    /**
     * Returns the exemplar location name for the given time zone. When this object does not have a
    ...
    /**
     * Finds time zone name prefix matches for the input text at the given offset and returns a
    ...
    /**
     * A <code>MatchInfo</code> represents a time zone name match used by {@link
    ...
    public static class MatchInfo {
         ...
         * @throws IllegalArgumentException when 1) <code>nameType</code> is <code>null</code>, or
         *     2) both <code>tzID</code> and <code>mzID</code> are <code>null</code>, or 3) <code>
         ...
         *
         * <p><b>Note</b>: A <code>MatchInfo</code> must have either a time zone ID or a meta zone
         * ID.
         *
    ...
    /// CLOVER:OFF
    /**
     * The default implementation of <code>TimeZoneNames</code> used by {@link
     * TimeZoneNames#getInstance(ULocale)} when the ICU4J tznamedata component is not available.
     */
    private static class DefaultTimeZoneNames extends TimeZoneNames {
         ...
         */
        @Override
        public Collection<MatchInfo> find(
                CharSequence text, int start, EnumSet<NameType> nameTypes) {
            return Collections.emptyList();
        }

        /**
         * The default <code>TimeZoneNames</code> factory called from {@link
         * TimeZoneNames#getInstance(ULocale)} when the ICU4J tznamedata component is not available.
         */
