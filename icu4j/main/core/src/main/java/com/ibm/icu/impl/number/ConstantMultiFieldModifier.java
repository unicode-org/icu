 ...
 * modifier. Constructed based on the contents of two {@link FormattedStringBuilder} instances (one
 * for the prefix, one for the suffix).
 */
public class ConstantMultiFieldModifier implements Modifier {

    // NOTE: In Java, these are stored as array pointers. In C++, the FormattedStringBuilder is
    // stored by
    ...
    public boolean strictEquals(Modifier other) {
        ...
        return Arrays.equals(prefixChars, _other.prefixChars)
                && Arrays.equals(prefixFields, _other.prefixFields)
                && Arrays.equals(suffixChars, _other.suffixChars)
                && Arrays.equals(suffixFields, _other.suffixFields)
                && overwrite == _other.overwrite
    ...
    public String toString() {
        ...
        return String.format(
                "<ConstantMultiFieldModifier prefix:'%s' suffix:'%s'>",
                temp.subSequence(0, prefixLength), temp.subSequence(prefixLength, temp.length()));
    }
}
