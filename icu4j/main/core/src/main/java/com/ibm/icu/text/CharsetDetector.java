 ...
 *
 * <p>Character set detection is at best an imprecise operation. The detection process will attempt
 * to identify the charset that best matches the characteristics of the byte data, but the process
 * is partly statistical in nature, and the results can not be guaranteed to always be correct.
 *
 * <p>For best accuracy in charset detection, the input data should be primarily in a single
 * language, and a minimum of a few hundred bytes worth of plain text in the language are needed.
...
public class CharsetDetector {
     ...
     *
     * <p>A declared encoding that is incompatible with the input data being analyzed will not be
     * added to the list of possible encodings.
     *
     ...
     * Set the input text (byte) data whose charset is to be detected.
     *
     * <p>The input stream that supplies the character data must have markSupported() == true; the
     ...
     *
     * <p>Note though, that because the detection only looks at the start of the input data, there
     * is a possibility that the returned charset will fail to handle the full set of input data.
     *
     * <p>Raise an exception if
     *
     * <ul>
     *   <li>no charset appears to match the data.
     *   <li>no input text has been provided
     * </ul>
     *
     ...
     *
     * <ul>
     *   <li>no charsets appear to match the input data.
     *   <li>no input text has been provided
     * </ul>
     *
     ...
     * input data.
     *
     * <p>This is a convenience method that is equivalent to <code>
     * this.setDeclaredEncoding(declaredEncoding).setText(in).detect().getReader();</code>
     *
     * <p>For the input stream that supplies the character data, markSupported() must be true; the
     ...
     *
     * <p>Raise an exception if no charsets appear to match the input data.
     *
     ...
     *
     * <p>This is a convenience method that is equivalent to <code>
     * this.setDeclaredEncoding(declaredEncoding).setText(in).detect().getString();</code>
     *
     * <p>Raise an exception if no charsets appear to match the input data.
     *
     * @param in The source of the byte data in the unknown charset.
     ...
     *
     * <p><b>Note:</b> Multiple different charset encodings in a same family may use a single shared
    ...
    static {
        ...
        list.add(
                new CSRecognizerInfo(
                        new CharsetRecog_mbcs.CharsetRecog_euc.CharsetRecog_gb_18030(), true));
        list.add(
                new CSRecognizerInfo(
                        new CharsetRecog_mbcs.CharsetRecog_euc.CharsetRecog_euc_jp(), true));
        list.add(
                new CSRecognizerInfo(
                        new CharsetRecog_mbcs.CharsetRecog_euc.CharsetRecog_euc_kr(), true));
