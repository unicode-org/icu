 ...
 *
 * <p>The UCharacter class provides extensions to the {@link java.lang.Character} class. These
 * extensions provide support for more Unicode properties. Each ICU release supports the latest
 * version of Unicode available at that time.
 *
 * <p>For some time before Java 5 added support for supplementary Unicode code points, The ICU
 * UCharacter class and many other ICU classes already supported them. Some UCharacter methods and
 * constants were widened slightly differently than how the Character class methods and constants
 * were widened later. In particular, {@link Character#MAX_VALUE} is still a char with the value
 * U+FFFF, while the {@link UCharacter#MAX_VALUE} is an int with the value U+10FFFF.
 *
 * <p>Code points are represented in these API using ints. While it would be more convenient in Java
 * to have a separate primitive datatype for them, ints suffice in the meantime.
 *
 * <p>To use this class please add the jar file name icu4j.jar to the class path, since it contains
 * data files which supply the information used by this file.<br>
 * E.g. In Windows <br>
 * <code>set CLASSPATH=%CLASSPATH%;$JAR_FILE_PATH/ucharacter.jar</code>.<br>
 * Otherwise, another method would be to copy the files uprops.dat and unames.icu from the icu4j
 * source subdirectory <i>$ICU4J_SRC/src/com.ibm.icu.impl.data</i> to your class directory
 * <i>$ICU4J_CLASS/com.ibm.icu.impl.data</i>.
 *
 * <p>Aside from the additions for UTF-16 support, and the updated Unicode properties, the main
 * differences between UCharacter and Character are:
 *
 * <ul>
 *   <li>UCharacter is not designed to be a char wrapper and does not have APIs to which involves
 *       management of that single char.<br>
 *       These include:
 *       <ul>
 *         <li>char charValue(),
 *         <li>int compareTo(java.lang.Character, java.lang.Character), etc.
 *       </ul>
 *   <li>UCharacter does not include Character APIs that are deprecated, nor does it include the
 *       Java-specific character information, such as boolean isJavaIdentifierPart(char ch).
 *   <li>Character maps characters 'A' - 'Z' and 'a' - 'z' to the numeric values '10' - '35'.
 *       UCharacter also does this in digit and getNumericValue, to adhere to the java semantics of
 *       these methods. New methods unicodeDigit, and getUnicodeNumericValue do not treat the above
 *       code points as having numeric values. This is a semantic change from ICU4J 1.3.1.
 * </ul>
 *
 * <p>Further detail on differences can be determined using the program <a href=
 ...
 *
 * <p>In addition to Java compatibility functions, which calculate derived properties, this API
 * provides low-level access to the Unicode Character Database.
 *
 * <p>Unicode assigns each code point (not just assigned character) values for many properties. Most
 * of them are simple boolean flags, or constants from a small enumerated list. For some properties,
 * values are strings or other relatively more complex types.
 *
 * <p>For more information see <a href="http://www.unicode/org/ucd/">"About the Unicode Character
 * Database"</a> (http://www.unicode.org/ucd/) and the <a
 * href="https://unicode-org.github.io/icu/userguide/strings/properties">ICU User Guide chapter on
 * Properties</a> (https://unicode-org.github.io/icu/userguide/strings/properties).
 *
 * <p>There are also functions that provide easy migration from C/POSIX functions like isblank().
 * Their use is generally discouraged because the C/POSIX standards do not define their semantics
 * beyond the ASCII range, which means that different implementations exhibit very different
 * behavior. Instead, Unicode properties should be used directly.
 *
 * <p>There are also only a few, broad C/POSIX character classes, and they tend to be used for
 * conflicting purposes. For example, the "isalpha()" class is sometimes used to determine word
 * boundaries, while a more sophisticated approach would at least distinguish initial letters from
 * continuation characters (the latter including combining marks). (In ICU, BreakIterator is the
 * most sophisticated API for word boundaries.) Another example: There is no "istitle()" class for
 * titlecase characters.
 *
 * <p>ICU 3.4 and later provides API access for all twelve C/POSIX character classes. ICU implements
 * them according to the Standard Recommendations in Annex C: Compatibility Properties of UTS #18
 * Unicode Regular Expressions (http://www.unicode.org/reports/tr18/#Compatibility_Properties).
 *
 * <p>API access for C/POSIX character classes is as follows:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The C/POSIX character classes are also available in UnicodeSet patterns, using patterns like
 * [:graph:] or \p{graph}.
 *
 * <p>{@icunote} There are several ICU (and Java) whitespace functions. Comparison:
 *
 * <ul>
 *   <li>isUWhiteSpace=UCHAR_WHITE_SPACE: Unicode White_Space property; most of general categories
 *       "Z" (separators) + most whitespace ISO controls (including no-break spaces, but excluding
 *       IS1..IS4)
...
public final class UCharacter implements ECharacterCategory, ECharacterDirection {
     ...
     *
     * <p>A family of character subsets representing the character blocks in the Unicode
     * specification, generated from Unicode Data file Blocks.txt. Character blocks generally define
     * characters used for a specific script or purpose. A character is contained by at most one
     * Unicode block.
     *
     * <p>{@icunote} All fields named XXX_ID are specific to ICU.
     *
     * @stable ICU 2.4
     */
    public static final class UnicodeBlock extends Character.Subset {
        ...
        /**
         * Same as public static final int PRIVATE_USE. Until Unicode 3.1.1; the corresponding block
         * name was "Private Use"; and multiple code point ranges had this block. Unicode 3.2
         * renames the block for the BMP PUA to "Private Use Area" and adds separate blocks for the
         * supplementary PUAs.
         *
         * @stable ICU 2.4
         */
        ...
        /**
         * Same as public static final int PRIVATE_USE_AREA. Until Unicode 3.1.1; the corresponding
         * block name was "Private Use"; and multiple code point ranges had this block. Unicode 3.2
         * renames the block for the BMP PUA to "Private Use Area" and adds separate blocks for the
         * supplementary PUAs.
         *
         * @stable ICU 2.4
         ...
         * @stable ICU 70
         */
        public static final int UNIFIED_CANADIAN_ABORIGINAL_SYLLABICS_EXTENDED_A_ID =
                318; /*[11AB0]*/
        ...
        public static final UnicodeBlock ENCLOSED_CJK_LETTERS_AND_MONTHS =
                new UnicodeBlock(
                        "ENCLOSED_CJK_LETTERS_AND_MONTHS", ENCLOSED_CJK_LETTERS_AND_MONTHS_ID);
        ...
        public static final UnicodeBlock MATHEMATICAL_ALPHANUMERIC_SYMBOLS =
                new UnicodeBlock(
                        "MATHEMATICAL_ALPHANUMERIC_SYMBOLS", MATHEMATICAL_ALPHANUMERIC_SYMBOLS_ID);
        ...
        public static final UnicodeBlock SUPPLEMENTARY_PRIVATE_USE_AREA_A =
                new UnicodeBlock(
                        "SUPPLEMENTARY_PRIVATE_USE_AREA_A", SUPPLEMENTARY_PRIVATE_USE_AREA_A_ID);
        ...
        public static final UnicodeBlock SUPPLEMENTARY_PRIVATE_USE_AREA_B =
                new UnicodeBlock(
                        "SUPPLEMENTARY_PRIVATE_USE_AREA_B", SUPPLEMENTARY_PRIVATE_USE_AREA_B_ID);
        ...
        public static final UnicodeBlock MISCELLANEOUS_SYMBOLS_AND_ARROWS =
                new UnicodeBlock(
                        "MISCELLANEOUS_SYMBOLS_AND_ARROWS", MISCELLANEOUS_SYMBOLS_AND_ARROWS_ID);
        ...
        public static final UnicodeBlock VARIATION_SELECTORS_SUPPLEMENT =
                new UnicodeBlock(
                        "VARIATION_SELECTORS_SUPPLEMENT", VARIATION_SELECTORS_SUPPLEMENT_ID);
        ...
        public static final UnicodeBlock SUPPLEMENTAL_PUNCTUATION =
                new UnicodeBlock(
                        "SUPPLEMENTAL_PUNCTUATION", SUPPLEMENTAL_PUNCTUATION_ID); /*[2E00]*/
        ...
        public static final UnicodeBlock COMMON_INDIC_NUMBER_FORMS =
                new UnicodeBlock(
                        "COMMON_INDIC_NUMBER_FORMS", COMMON_INDIC_NUMBER_FORMS_ID); /*[A830]*/
        ...
        public static final UnicodeBlock TRANSPORT_AND_MAP_SYMBOLS =
                new UnicodeBlock(
                        "TRANSPORT_AND_MAP_SYMBOLS", TRANSPORT_AND_MAP_SYMBOLS_ID); /*[1F680]*/
        ...
        public static final UnicodeBlock ARABIC_MATHEMATICAL_ALPHABETIC_SYMBOLS =
                new UnicodeBlock(
                        ...
                        ARABIC_MATHEMATICAL_ALPHABETIC_SYMBOLS_ID); /*[1EE00]*/
        ...
        public static final UnicodeBlock SHARADA =
                new UnicodeBlock("SHARADA", SHARADA_ID); /*[11180]*/
        ...
        public static final UnicodeBlock BASSA_VAH =
                new UnicodeBlock("BASSA_VAH", BASSA_VAH_ID); /*[16AD0]*/
        ...
        public static final UnicodeBlock COMBINING_DIACRITICAL_MARKS_EXTENDED =
                new UnicodeBlock(
                        ...
                        COMBINING_DIACRITICAL_MARKS_EXTENDED_ID); /*[1AB0]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock DUPLOYAN =
                new UnicodeBlock("DUPLOYAN", DUPLOYAN_ID); /*[1BC00]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock ELBASAN =
                new UnicodeBlock("ELBASAN", ELBASAN_ID); /*[10500]*/
        ...
        public static final UnicodeBlock GEOMETRIC_SHAPES_EXTENDED =
                new UnicodeBlock(
                        "GEOMETRIC_SHAPES_EXTENDED", GEOMETRIC_SHAPES_EXTENDED_ID); /*[1F780]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock GRANTHA =
                new UnicodeBlock("GRANTHA", GRANTHA_ID); /*[11300]*/
        ...
        public static final UnicodeBlock KHUDAWADI =
                new UnicodeBlock("KHUDAWADI", KHUDAWADI_ID); /*[112B0]*/
        ...
        public static final UnicodeBlock LINEAR_A =
                new UnicodeBlock("LINEAR_A", LINEAR_A_ID); /*[10600]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock MAHAJANI =
                new UnicodeBlock("MAHAJANI", MAHAJANI_ID); /*[11150]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock MANICHAEAN =
                new UnicodeBlock("MANICHAEAN", MANICHAEAN_ID); /*[10AC0]*/
        ...
        public static final UnicodeBlock NABATAEAN =
                new UnicodeBlock("NABATAEAN", NABATAEAN_ID); /*[10880]*/
        ...
        public static final UnicodeBlock OLD_PERMIC =
                new UnicodeBlock("OLD_PERMIC", OLD_PERMIC_ID); /*[10350]*/
        ...
        public static final UnicodeBlock PAHAWH_HMONG =
                new UnicodeBlock("PAHAWH_HMONG", PAHAWH_HMONG_ID); /*[16B00]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock PALMYRENE =
                new UnicodeBlock("PALMYRENE", PALMYRENE_ID); /*[10860]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock PAU_CIN_HAU =
                new UnicodeBlock("PAU_CIN_HAU", PAU_CIN_HAU_ID); /*[11AC0]*/
        ...
        public static final UnicodeBlock SHORTHAND_FORMAT_CONTROLS =
                new UnicodeBlock(
                        "SHORTHAND_FORMAT_CONTROLS", SHORTHAND_FORMAT_CONTROLS_ID); /*[1BCA0]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock SIDDHAM =
                new UnicodeBlock("SIDDHAM", SIDDHAM_ID); /*[11580]*/
        ...
        public static final UnicodeBlock TIRHUTA =
                new UnicodeBlock("TIRHUTA", TIRHUTA_ID); /*[11480]*/

        /**
         * @stable ICU 54
         */
        public static final UnicodeBlock WARANG_CITI =
                new UnicodeBlock("WARANG_CITI", WARANG_CITI_ID); /*[118A0]*/
        ...
        public static final UnicodeBlock EARLY_DYNASTIC_CUNEIFORM =
                new UnicodeBlock(
                        "EARLY_DYNASTIC_CUNEIFORM", EARLY_DYNASTIC_CUNEIFORM_ID); /*[12480]*/
        ...
        public static final UnicodeBlock MULTANI =
                new UnicodeBlock("MULTANI", MULTANI_ID); /*[11280]*/
        ...
        public static final UnicodeBlock BHAIKSUKI =
                new UnicodeBlock("BHAIKSUKI", BHAIKSUKI_ID); /*[11C00]*/
        ...
        public static final UnicodeBlock IDEOGRAPHIC_SYMBOLS_AND_PUNCTUATION =
                new UnicodeBlock(
                        ...
                        IDEOGRAPHIC_SYMBOLS_AND_PUNCTUATION_ID); /*[16FE0]*/

        /**
         * @stable ICU 58
         */
        public static final UnicodeBlock MARCHEN =
                new UnicodeBlock("MARCHEN", MARCHEN_ID); /*[11C70]*/
        ...
        public static final UnicodeBlock CJK_UNIFIED_IDEOGRAPHS_EXTENSION_F =
                new UnicodeBlock(
                        ...
                        CJK_UNIFIED_IDEOGRAPHS_EXTENSION_F_ID); /*[2CEB0]*/
        ...
        public static final UnicodeBlock SOYOMBO =
                new UnicodeBlock("SOYOMBO", SOYOMBO_ID); /*[11A50]*/
        ...
        public static final UnicodeBlock MAKASAR =
                new UnicodeBlock("MAKASAR", MAKASAR_ID); /*[11EE0]*/
        ...
        public static final UnicodeBlock SOGDIAN =
                new UnicodeBlock("SOGDIAN", SOGDIAN_ID); /*[10F30]*/
        ...
        public static final UnicodeBlock EGYPTIAN_HIEROGLYPH_FORMAT_CONTROLS =
                new UnicodeBlock(
                        ...
                        EGYPTIAN_HIEROGLYPH_FORMAT_CONTROLS_ID); /*[13430]*/

        /**
         * @stable ICU 64
         */
        public static final UnicodeBlock ELYMAIC =
                new UnicodeBlock("ELYMAIC", ELYMAIC_ID); /*[10FE0]*/
        ...
        public static final UnicodeBlock SYMBOLS_AND_PICTOGRAPHS_EXTENDED_A =
                new UnicodeBlock(
                        ...
                        SYMBOLS_AND_PICTOGRAPHS_EXTENDED_A_ID); /*[1FA70]*/
        ...
        public static final UnicodeBlock SYMBOLS_FOR_LEGACY_COMPUTING =
                new UnicodeBlock(
                        ...
                        SYMBOLS_FOR_LEGACY_COMPUTING_ID); /*[1FB00]*/
        ...
        public static final UnicodeBlock ZNAMENNY_MUSICAL_NOTATION =
                new UnicodeBlock(
                        "ZNAMENNY_MUSICAL_NOTATION", ZNAMENNY_MUSICAL_NOTATION_ID); /*[1CF00]*/
        ...
        public static final UnicodeBlock EGYPTIAN_HIEROGLYPHS_EXTENDED_A =
                new UnicodeBlock(
                        "EGYPTIAN_HIEROGLYPHS_EXTENDED_A", EGYPTIAN_HIEROGLYPHS_EXTENDED_A_ID);
        ...
        public static final UnicodeBlock CJK_UNIFIED_IDEOGRAPHS_EXTENSION_J =
                new UnicodeBlock(
                        ...
                        CJK_UNIFIED_IDEOGRAPHS_EXTENSION_J_ID);
        ...
        public static final UnicodeBlock MISCELLANEOUS_SYMBOLS_SUPPLEMENT =
                new UnicodeBlock(
                        "MISCELLANEOUS_SYMBOLS_SUPPLEMENT", MISCELLANEOUS_SYMBOLS_SUPPLEMENT_ID);
        ...
        /**
         * {@icu} Returns the only instance of the UnicodeBlock with the argument ID. If no such ID
         * exists, a INVALID_CODE UnicodeBlock will be returned.
         *
         * @param id UnicodeBlock ID
         * @return the only instance of the UnicodeBlock with the argument ID if it exists,
         *     otherwise a INVALID_CODE UnicodeBlock will be returned.
         * @stable ICU 2.4
         */
        ...
        /**
         * Returns the Unicode allocation block that contains the code point, or null if the code
         * point is not a member of a defined block.
         *
         ...
         * Returns the Unicode block with the given name. {@icunote} Unlike {@link
         * java.lang.Character.UnicodeBlock#forName(String)}, this only matches against the official
                    ...
                    String name =
                            trimBlockName(
                                    getPropertyValueName(
                                            UProperty.BLOCK, b.getID(), UProperty.NameChoice.LONG));
                    m.put(name, b);
                }
    ...
    public static interface EastAsianWidth {
        ...
        /**
         * One more than the highest normal EastAsianWidth value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.EAST_ASIAN_WIDTH).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 6;
    }
    ...
    public static interface DecompositionType {
        ...
        /**
         * One more than the highest normal DecompositionType value. The highest value is available
         * via UCharacter.getIntPropertyMaxValue(UProperty.DECOMPOSITION_TYPE).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 18;
    }
    ...
    public static interface JoiningType {
        ...
        /**
         * One more than the highest normal JoiningType value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.JOINING_TYPE).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 6;
    ...
    public static interface GraphemeClusterBreak {
        ...
        /**
         * One more than the highest normal GraphemeClusterBreak value. The highest value is
         * available via UCharacter.getIntPropertyMaxValue(UProperty.GRAPHEME_CLUSTER_BREAK).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 18;
    ...
    public static interface WordBreak {
        ...
        /**
         * One more than the highest normal WordBreak value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.WORD_BREAK).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 23;
    }
    ...
    public static interface SentenceBreak {
        ...
        /**
         * One more than the highest normal SentenceBreak value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.SENTENCE_BREAK).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 15;
    }
    ...
    public static interface LineBreak {
        ...
        /**
         * One more than the highest normal LineBreak value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.LINE_BREAK).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 49;
    }
    ...
    public static interface NumericType {
        ...
        /**
         * One more than the highest normal NumericType value. The highest value is available via
         * UCharacter.getIntPropertyMaxValue(UProperty.NUMERIC_TYPE).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 4;
    }
    ...
    public static interface HangulSyllableType {
        ...
        /**
         * One more than the highest normal HangulSyllableType value. The highest value is available
         * via UCharacter.getIntPropertyMaxValue(UProperty.HANGUL_SYLLABLE_TYPE).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
        @Deprecated public static final int COUNT = 6;
    ...
    public static interface BidiPairedBracketType {
        ...
        /**
         * One more than the highest normal BidiPairedBracketType value. The highest value is
         * available via UCharacter.getIntPropertyMaxValue(UProperty.BIDI_PAIRED_BRACKET_TYPE).
         *
         * @deprecated ICU 58 The numeric value may change over time, see ICU ticket #12420.
         */
    ...
    /**
     * Indic Conjunct Break constants. See https://unicode.org/reports/tr44/#Indic_Conjunct_Break
     *
    ...
    /**
     * Identifier Status constants. See
    ...
    /**
     * The lowest Unicode code point value, constant 0. Same as {@link Character#MIN_CODE_POINT},
     * same integer value as {@link Character#MIN_VALUE}.
     *
     * @stable ICU 2.1
     */
    ...
    /**
     * The highest Unicode code point value (scalar value), constant U+10FFFF (uses 21 bits). Same
     * as {@link Character#MAX_CODE_POINT}.
     *
     * <p>Up-to-date Unicode implementation of {@link Character#MAX_VALUE} which is still a char
     * with the value U+FFFF.
     *
     * @stable ICU 2.1
     */
    ...
    /**
     * The minimum value for Supplementary code points, constant U+10000. Same as {@link
    ...
    /**
     * Unicode value used when translating into Unicode encoding form and there is no existing
     * character.
     *
     * @stable ICU 2.1
     */
    ...
    /**
     * Special value that is returned by getUnicodeNumericValue(int) when no numeric value is
     * defined for a code point.
     *
     * @stable ICU 2.4
    ...
    /**
     * Do not lowercase non-initial parts of words when titlecasing. Option bit for titlecasing APIs
     * that take an options bit set.
     *
     * <p>By default, titlecasing will titlecase the first cased character of a word and lowercase
     * all other characters. With this option, the other characters will not be modified.
     *
     * @see #toTitleCase
     * @stable ICU 3.8
     */
    ...
    /**
     * Do not adjust the titlecasing indexes from BreakIterator::next() indexes; titlecase exactly
     * the characters at breaks from the iterator. Option bit for titlecasing APIs that take an
     * options bit set.
     *
     * <p>By default, titlecasing will take each break iterator index, adjust it by looking for the
     * next cased character, and titlecase that one. Other characters are lowercased.
     *
     * <p>This follows Unicode 4 &amp; 5 section 3.13 Default Case Operations:
     *
     * <p>R3 toTitlecase(X): Find the word boundaries based on Unicode Standard Annex #29, "Text
     * Boundaries." Between each pair of word boundaries, find the first cased character F. If F
     * exists, map F to default_title(F); then map each subsequent character C to default_lower(C).
     *
     * @see #toTitleCase
     ...
     * This method observes the semantics of <code>java.lang.Character.digit()</code>. Note that
     * this will return positive values for code points for which isDigit returns false, just like
     * java.lang.Character. <br>
     * <em>Semantic Change:</em> In release 1.3.1 and prior, this did not treat the European letters
     * as having a digit value, and also treated numeric letters and other numbers as digits. This
     * has been changed to conform to the java semantics. <br>
     ...
     * @return the numeric value represented by the code point in the specified radix, or -1 if the
     *     code point is not a decimal digit or if its value is too large for the radix
     * @stable ICU 2.1
     */
    ...
    /**
     * Returnss the numeric value of a decimal digit code point. <br>
     * This is a convenience overload of <code>digit(int, int)</code> that provides a decimal radix.
     * <br>
     * <em>Semantic Change:</em> In release 1.3.1 and prior, this treated numeric letters and other
     * numbers as digits. This has been changed to conform to the java semantics.
     *
     * @param ch the code point to query
     * @return the numeric value represented by the code point, or -1 if the code point is not a
     *     decimal digit or if its value is too large for a decimal radix
     * @stable ICU 2.1
     */
     ...
     * If the code point has a numeric value that cannot be represented as a nonnegative integer
     * (for example, a fractional value), then -2 is returned.
     *
     * @param ch the code point to query
     * @return the numeric value of the code point, or -1 if it has no numeric value, or -2 if it
     *     has a numeric value that cannot be represented as a nonnegative integer
     * @stable ICU 2.1
     */
    ...
    /**
     * {@icu} Returns the numeric value for a Unicode code point as defined in the Unicode Character
     * Database.
     *
     * <p>A "double" return type is necessary because some numeric values are fractions, negative,
     * or too large for int.
     *
     * <p>For characters without any numeric values in the Unicode Character Database, this function
     * will return NO_NUMERIC_VALUE. Note: This is different from the Unicode Standard which
     * specifies NaN as the default value.
     *
     * <p><em>API Change:</em> In release 2.2 and prior, this API has a return type int and returns
     * -1 when the argument ch does not have a corresponding numeric value. This has been changed to
     * synch with ICU4C
     *
     * <p>This corresponds to the ICU4C function u_getNumericValue.
     *
    ...
    /**
     * Compatibility override of Java deprecated method. This method will always remain deprecated.
     * Same as java.lang.Character.isSpace().
     *
     * @param ch the code point
     * @return true if the code point is a space character as defined by
    ...
    /**
     * Returns a value indicating a code point's Unicode category. Up-to-date Unicode implementation
     * of java.lang.Character.getType() except for the above mentioned code points that had their
     * category changed.<br>
     * Return results are constants from the interface <a
     * href=UCharacterCategory.html>UCharacterCategory</a><br>
     * <em>NOTE:</em> the UCharacterCategory values are <em>not</em> compatible with those returned
     * by java.lang.Character.getType. UCharacterCategory values match the ones used in ICU4C, while
     * java.lang.Character type values, though similar, skip the value 17.
     *
     ...
     * supplementary code points though allocated space are not defined in Unicode yet.<br>
     ...
     * This method observes the semantics of <code>java.lang.Character.isDigit()</code>. It returns
     * true for decimal digits only. <br>
     * <em>Semantic Change:</em> In release 1.3.1 and prior, this treated numeric letters and other
     * numbers as digits. This has been changed to conform to the java semantics.
     *
    ...
    /**
     * Determines if the specified code point is an ISO control character. A code point is
     * considered to be an ISO control character if it is in the range &#92;u0000 through &#92;u001F
     * or in the range &#92;u007F through &#92;u009F.<br>
    ...
    public static boolean isISOControl(int ch) {
        return ch >= 0
                && ch <= APPLICATION_PROGRAM_COMMAND_
                && ((ch <= UNIT_SEPARATOR_) || (ch >= DELETE_));
    }

    /**
     * Determines if the specified code point is a letter. Up-to-date Unicode implementation of
    ...
    /**
     * Determines if the specified code point is a letter or digit. {@icunote} This method, unlike
     * java.lang.Character does not regard the ascii characters 'A' - 'Z' and 'a' - 'z' as digits.
     *
    ...
    /**
     * Compatibility override of Java deprecated method. This method will always remain deprecated.
     * Delegates to java.lang.Character.isJavaIdentifierStart.
     *
    ...
    /**
     * Compatibility override of Java deprecated method. This method will always remain deprecated.
     * Delegates to java.lang.Character.isJavaIdentifierPart.
     *
     * @param cp the code point
    ...
    /**
     * Determines if the specified code point is a lowercase character. UnicodeData only contains
     * case mappings for code points where they are one-to-one mappings; it also omits information
     * about context-sensitive case mappings.<br>
     * For more information about Unicode case mapping please refer to the <a
     * href=https://www.unicode.org/reports/tr21/>Technical report #21</a>.<br>
    ...
    /**
     * Determines if the specified code point is a white space character. A code point is considered
     * to be an whitespace character if and only if it satisfies one of the following criteria:
     *
     * <ul>
     ...
     * </ul>
     *
     * This API tries to sync with the semantics of Java's java.lang.Character.isWhitespace(), but
     * it may not return the exact same results because of the Unicode version difference.
     *
     * <p>Note: Unicode 4.0.1 changed U+200B ZERO WIDTH SPACE from a Space Separator (Zs) to a
     * Format Control (Cf). Since then, isWhitespace(0x200b) returns false. See
    ...
    /**
     * Determines if the specified code point is a Unicode specified space character, i.e. if code
     * point is in the category Zs, Zl and Zp. Up-to-date Unicode implementation of
    ...
    /**
     * Determines if the specified code point is a titlecase character. UnicodeData only contains
     * case mappings for code points where they are one-to-one mappings; it also omits information
     * about context-sensitive case mappings.<br>
     * For more information about Unicode case mapping please refer to the <a
     * href=https://www.unicode.org/reports/tr21/>Technical report #21</a>.<br>
    ...
    /**
     * Determines if the specified character is permissible as a non-initial character of an
     * identifier according to UAX #31 Unicode Identifier and Pattern Syntax.
     *
    ...
    /**
     * Writes code point c's Identifier_Type as a set of IdentifierType values and returns the
     * number of types. The set is cleared before c's types are added.
     *
     * <p>Used for UTS #39 General Security Profile for Identifiers
     * (https://www.unicode.org/reports/tr39/#General_Security_Profile).
     *
     * <p>Each code point maps to a <i>set</i> of IdentifierType values. There is always at least
     * one type. Only some of the types can be combined with others, and usually only a small number
     * of types occur together. Future versions might add additional types. See UTS #39 and its data
     * files for details.
     *
     * @param c code point
    ...
    /**
     * Determines if the specified code point should be regarded as an ignorable character in a Java
     * identifier. A character is Java-identifier-ignorable if it has the general category Cf
     * Formatting Control, or it is a non-Java-whitespace ISO control: U+0000..U+0008,
     * U+000E..U+001B, U+007F..U+009F.<br>
    ...
    /**
     * Determines if the specified code point is an uppercase character. UnicodeData only contains
     * case mappings for code point where they are one-to-one mappings; it also omits information
     * about context-sensitive case mappings.<br>
     * For language specific case conversion behavior, use toUpperCase(locale, str). <br>
     * For example, the case conversion for dot-less i and dotted I in Turkish, or for final sigma
     * in Greek. For more information about Unicode case mapping please refer to the <a
     * href=https://www.unicode.org/reports/tr21/>Technical report #21</a>.<br>
    ...
    /**
     * The given code point is mapped to its lowercase equivalent; if the code point has no
     * lowercase equivalent, the code point itself is returned. Up-to-date Unicode implementation of
     * java.lang.Character.toLowerCase()
     *
     * <p>This function only returns the simple, single-code point case mapping. Full case mappings
     * should be used whenever possible because they produce better results by working on whole
     * strings. They take into account the string context and the language and can map to a result
     * string with a different length as appropriate. Full case mappings are applied by the case
     * mapping functions that take String parameters rather than code points (int). See also the
     * User Guide chapter on C/POSIX migration:
     ...
     * in UTF-16 format. The result is a string whose length is 1 for BMP code points, 2 for
     * supplementary ones.
     *
    ...
    /**
     * Converts the code point argument to titlecase. If no titlecase is available, the uppercase is
     * returned. If no uppercase is available, the code point itself is returned. Up-to-date Unicode
     * implementation of java.lang.Character.toTitleCase()
     *
     * <p>This function only returns the simple, single-code point case mapping. Full case mappings
     * should be used whenever possible because they produce better results by working on whole
     * strings. They take into account the string context and the language and can map to a result
    ...
    /**
     * Converts the character argument to uppercase. If no uppercase is available, the character
     * itself is returned. Up-to-date Unicode implementation of java.lang.Character.toUpperCase()
     *
     * <p>This function only returns the simple, single-code point case mapping. Full case mappings
     * should be used whenever possible because they produce better results by working on whole
     * strings. They take into account the string context and the language and can map to a result
    ...
    /**
     * {@icu} Determines if the code point is a supplementary character. A code point is a
     * supplementary character if and only if it is greater than <a
    ...
    /**
     * {@icu} Determines whether the specified code point is a printable character according to the
     * Unicode standard.
     *
    ...
    /**
     * {@icu} Determines whether the specified code point is of base form. A code point of base form
     * does not graphically combine with preceding characters, and is neither a control nor a format
     * character.
     *
    ...
    /**
     * {@icu} Returns the Bidirection property of a code point. For example, 0x0041 (letter A) has
     * the LEFT_TO_RIGHT directional property.<br>
    ...
    /**
     * Determines whether the code point has the "mirrored" property. This property is set for
     * characters that are commonly used in Right-To-Left contexts and need to be displayed with a
     * "mirrored" glyph.
     *
    ...
    /**
     * {@icu} Maps the specified code point to a "mirror-image" code point. For code points with the
     * "mirrored" property, implementations sometimes need a "poor man's" mapping to another code
     * point such that the default glyph may serve as the mirror-image of the default glyph of the
     ...
     * @return another code point that may serve as a mirror-image substitute, or ch itself if there
     *     is no such mapping or ch does not have the "mirrored" property
     * @stable ICU 2.1
     */
    ...
    /**
     * {@icu} Maps the specified character to its paired bracket character. For
     * Bidi_Paired_Bracket_Type!=None, this is the same as getMirror(int). Otherwise c itself is
     * returned. See http://www.unicode.org/reports/tr9/
     *
     * @param c the code point to be mapped
     * @return the paired bracket code point, or c itself if there is no such mapping
    ...
    /**
     * {@icu} A string is legal iff all its code points are legal. A code point is illegal if and
     * only if
     *
     * <ul>
     *   <li>Out of bounds, less than 0 or greater than UCharacter.MAX_VALUE
    ...
    /**
     * {@icu} Returns the most current Unicode name of the argument code point, or null if the
     * character is unassigned or outside the range {@code UCharacter.MIN_VALUE} and {@code
     * UCharacter.MAX_VALUE} or does not have a name. <br>
     * Note calling any methods related to code point names, e.g. {@code getName()} incurs a
     * one-time initialization cost to construct the name tables.
     *
    ...
    /**
     * {@icu} Returns null. Used to return the Unicode_1_Name property value which was of little
     * practical value.
     *
     * @param ch the code point for which to get the name
     * @return null
    ...
    /**
     * {@icu} Returns a name for a valid codepoint. Unlike, getName(int) and getName1_0(int), this
     * method will return a name even for codepoints that are not assigned a name in
     * UnicodeData.txt.
     *
     ...
     *   <li>Extended name in the form of "&lt;codepoint_type-codepoint_hex_digits&gt;". E.g.,
     *       &lt;noncharacter-fffe&gt;
     * </ul>
     *
     * Note calling any methods related to code point names, e.g. {@code getName()} incurs a
    ...
    /**
     * {@icu} Returns the corrected name from NameAliases.txt if there is one. Returns null if the
     * character is unassigned or outside the range UCharacter.MIN_VALUE and UCharacter.MAX_VALUE or
     * does not have a name. <br>
     * Note calling any methods related to code point names, e.g. {@code getName()} incurs a
     * one-time initialization cost to construct the name tables.
     *
    ...
    /**
     * {@icu} Returns null. Used to return the ISO 10646 comment for a character. The Unicode
     * ISO_Comment property is deprecated and has no values.
     *
     * @param ch The code point for which to get the ISO comment. It must be the case that {@code 0
     *     <= ch <= 0x10ffff}.
     * @return null
     * @deprecated ICU 49
     */
     ...
     * {@icu}
     *
     * <p>Finds a Unicode code point by its most current Unicode name and return its code point
     * value. All Unicode names are in uppercase. Note calling any methods related to code point
     * names, e.g. {@code getName()} incurs a one-time initialization cost to construct the name
     * tables.
     *
     ...
     * {@icu} Returns -1.
     *
     * <p>Used to find a Unicode character by its version 1.0 Unicode name and return its code point
     * value.
     *
     ...
     * {@icu}
     *
     * <p>Find a Unicode character by either its name and return its code point value. All Unicode
     * names are in uppercase. Extended names are all lowercase except for numbers and are contained
     ...
     * {@icu}
     *
     * <p>Find a Unicode character by its corrected name alias and return its code point value. All
     * Unicode names are in uppercase. Note calling any methods related to code point names, e.g.
     * {@code getName()} incurs a one-time initialization cost to construct the name tables.
     *
    ...
    /**
     * {@icu} Return the Unicode name for a given property, as given in the Unicode database file
     * PropertyAliases.txt. Most properties have more than one name. The nameChoice determines which
     * one is returned.
     *
     * <p>In addition, this function maps the property UProperty.GENERAL_CATEGORY_MASK to the
     * synthetic names "gcm" / "General_Category_Mask". These names are not in PropertyAliases.txt.
     *
     * @param property UProperty selector.
     * @param nameChoice UProperty.NameChoice selector for which name to get. All properties have a
     *     long name. Most have a short name, but some do not. Unicode allows for additional names;
     *     if present these will be returned by UProperty.NameChoice.LONG + i, where i=1, 2,...
     * @return a name, or null if Unicode explicitly defines no name ("n/a") for a given
     *     property/nameChoice. If a given nameChoice throws an exception, then all larger values of
     *     nameChoice will throw an exception. If null is returned for a given nameChoice, then
     *     other nameChoice values may return non-null results.
     * @exception IllegalArgumentException thrown if property or nameChoice are invalid.
     * @see UProperty
    ...
    /**
     * {@icu} Return the UProperty selector for a given property name, as specified in the Unicode
     * database file PropertyAliases.txt. Short, long, and any other variants are recognized.
     *
     * <p>In addition, this function maps the synthetic names "gcm" / "General_Category_Mask" to the
     * property UProperty.GENERAL_CATEGORY_MASK. These names are not in PropertyAliases.txt.
     *
     * @param propertyAlias the property name to be matched. The name is compared using "loose
     *     matching" as described in PropertyAliases.txt.
     * @return a UProperty enum.
     * @exception IllegalArgumentException thrown if propertyAlias is not recognized.
     * @see UProperty
     * @stable ICU 2.4
     */
    ...
    /**
     * {@icu} Return the Unicode name for a given property value, as given in the Unicode database
     * file PropertyValueAliases.txt. Most values have more than one name. The nameChoice determines
     * which one is returned.
     *
     * <p>Note: Some of the names in PropertyValueAliases.txt can only be retrieved using
     * UProperty.GENERAL_CATEGORY_MASK, not UProperty.GENERAL_CATEGORY. These include: "C" /
     * "Other", "L" / "Letter", "LC" / "Cased_Letter", "M" / "Mark", "N" / "Number", "P" /
     * "Punctuation", "S" / "Symbol", and "Z" / "Separator".
     *
     * @param property UProperty selector constant. UProperty.INT_START &lt;= property &lt;
     *     UProperty.INT_LIMIT or UProperty.BINARY_START &lt;= property &lt; UProperty.BINARY_LIMIT
     *     or UProperty.MASK_START &lt; = property &lt; UProperty.MASK_LIMIT. If out of range, null
     *     is returned.
     * @param value selector for a value for the given property. In general, valid values range from
     *     0 up to some maximum. There are a few exceptions: (1.) UProperty.BLOCK values begin at
     *     the non-zero value BASIC_LATIN.getID(). (2.) UProperty.CANONICAL_COMBINING_CLASS values
     *     are not contiguous and range from 0..240. (3.) UProperty.GENERAL_CATEGORY_MASK values are
     *     mask values produced by left-shifting 1 by UCharacter.getType(). This allows grouped
     *     categories such as [:L:] to be represented. Mask values are non-contiguous.
     * @param nameChoice UProperty.NameChoice selector for which name to get. All values have a long
     *     name. Most have a short name, but some do not. Unicode allows for additional names; if
     *     present these will be returned by UProperty.NameChoice.LONG + i, where i=1, 2,...
     * @return a name, or null if Unicode explicitly defines no name ("n/a") for a given
     *     property/value/nameChoice. If a given nameChoice throws an exception, then all larger
     *     values of nameChoice will throw an exception. If null is returned for a given nameChoice,
     *     then other nameChoice values may return non-null results.
     * @exception IllegalArgumentException thrown if property, value, or nameChoice are invalid.
     * @see UProperty
     * @see UProperty.NameChoice
     * @stable ICU 2.4
     */
    ...
    /**
     * {@icu} Return the property value integer for a given value name, as specified in the Unicode
     * database file PropertyValueAliases.txt. Short, long, and any other variants are recognized.
     *
     * <p>Note: Some of the names in PropertyValueAliases.txt will only be recognized with
     * UProperty.GENERAL_CATEGORY_MASK, not UProperty.GENERAL_CATEGORY. These include: "C" /
     * "Other", "L" / "Letter", "LC" / "Cased_Letter", "M" / "Mark", "N" / "Number", "P" /
     * "Punctuation", "S" / "Symbol", and "Z" / "Separator".
     *
     * @param property UProperty selector constant. UProperty.INT_START &lt;= property &lt;
     *     UProperty.INT_LIMIT or UProperty.BINARY_START &lt;= property &lt; UProperty.BINARY_LIMIT
     *     or UProperty.MASK_START &lt; = property &lt; UProperty.MASK_LIMIT. Only these properties
     *     can be enumerated.
     * @param valueAlias the value name to be matched. The name is compared using "loose matching"
     *     as described in PropertyValueAliases.txt.
     * @return a value integer. Note: UProperty.GENERAL_CATEGORY values are mask values produced by
     *     left-shifting 1 by UCharacter.getType(). This allows grouped categories such as [:L:] to
     *     be represented.
     * @see UProperty
     * @throws IllegalArgumentException if property is not a valid UProperty selector or valueAlias
    ...
    /**
     * Same as {@link #getPropertyValueEnum(int, CharSequence)}, except doesn't throw exception.
     * Instead, returns UProperty.UNDEFINED.
     *
     ...
     *
     * @param lead the lead unit (In ICU 2.1-69 the type of both parameters was <code>char</code>.)
     * @param trail the trail unit
     * @return code point if lead and trail form a valid surrogate pair.
     * @exception IllegalArgumentException thrown when the code units do not form a valid surrogate
     *     pair
     * @stable ICU 70
     ...
     * @exception IllegalArgumentException thrown when char16 is not a valid code point
     * @stable ICU 2.1
     */
    ...
    /**
     * Returns the uppercase version of the argument string. Casing is dependent on the default
     * locale and context-sensitive.
     *
    ...
    /**
     * Returns the lowercase version of the argument string. Casing is dependent on the default
     * locale and context-sensitive
     *
     * @param str source string to be performed on
     ...
     *
     * <p>Position for titlecasing is determined by the argument break iterator, hence the user can
     * customize his break iterator for a specialized titlecasing. In this case only the forward
     * iteration needs to be implemented. If the break iterator passed in is null, the default
     * Unicode algorithm will be used to determine the titlecase positions.
     *
     * <p>Only positions returned by the break iterator will be title cased, character in between
     * the positions will all be in lower case.
     *
     * <p>Casing is dependent on the default locale and context-sensitive
     *
     * @param str source string to be performed on
     * @param breakiter break iterator to determine the positions in which the character should be
     *     title cased.
    ...
    /**
     * Returns the uppercase version of the argument string. Casing is dependent on the argument
     * locale and context-sensitive.
     *
    ...
    /**
     * Returns the lowercase version of the argument string. Casing is dependent on the argument
     * locale and context-sensitive
     *
     * @param locale which string is to be converted in
     * @param str source string to be performed on
     ...
     * @param locale which string is to be converted in
     * @param str source string to be performed on
     * @param titleIter break iterator to determine the positions in which the character should be
     *     title cased.
     * @return titlecase version of the argument string
     * @stable ICU 3.2
     */
    ...
    public static String toTitleCase(
            ULocale locale, String str, BreakIterator titleIter, int options) {
    ...
    /**
     * {@icu} The given character is mapped to its case folding equivalent according to
     * UnicodeData.txt and CaseFolding.txt; if the character has no case folding equivalent, the
     * character itself is returned.
     *
     * <p>This function only returns the simple, single-code point case mapping. Full case mappings
     * should be used whenever possible because they produce better results by working on whole
     * strings. They can map to a result string with a different length as appropriate. Full case
     * mappings are applied by the case mapping functions that take String parameters rather than
     * code points (int). See also the User Guide chapter on C/POSIX migration:
     * https://unicode-org.github.io/icu/userguide/icu/posix#case-mappings
     *
     * @param ch the character to be converted
     * @param defaultmapping Indicates whether the default mappings defined in CaseFolding.txt are
     *     to be used, otherwise the mappings for dotted I and dotless i marked with 'T' in
     *     CaseFolding.txt are included.
    ...
    /**
     * {@icu} The given string is mapped to its case folding equivalent according to UnicodeData.txt
     * and CaseFolding.txt; if any character has no case folding equivalent, the character itself is
     * returned. "Full", multiple-code point case folding mappings are returned here. For "simple"
     * single-code point mappings use the API foldCase(int ch, boolean defaultmapping).
     *
    ...
    /**
     * {@icu} Option value for case folding: Use the modified set of mappings provided in
     * CaseFolding.txt to handle dotted I and dotless i appropriately for Turkic languages (tr, az).
     *
     * <p>Before Unicode 3.2, CaseFolding.txt contains mappings marked with 'I' that are to be
     * included for default mappings and excluded for the Turkic-specific mappings.
     *
     * <p>Unicode 3.2 CaseFolding.txt instead contains mappings marked with 'T' that are to be
     ...
     *
     * <p>This returns the value of Han 'numeric' code points, including those for zero, ten,
     * hundred, thousand, ten thousand, and hundred million. This includes both the standard and
     * 'checkwriting' characters, the 'big circle' zero character, and the standard zero character.
     *
     * <p>Note: The Unicode Standard has numeric values for more Han characters recognized by this
     * method (see {@link #getNumericValue(int)} and the UCD file DerivedNumericValues.txt), and a
     * {@link com.ibm.icu.text.NumberFormat} can be used with a Chinese {@link
     ...
     * {@icu}
     *
     * <p>Returns an iterator for character types, iterating over codepoints.
     *
     * <p>Example of use:<br>
     *
     * <pre>
     ...
     * {@icu}
     *
     * <p>Returns an iterator for character names, iterating over codepoints.
     *
     * <p>This API only gets the iterator for the modern, most up-to-date Unicode names. For older
     * 1.0 Unicode names use get1_0NameIterator() or for extended names use
     ...
     * </pre>
     *
     * <p>The maximal range which the name iterator iterates is from UCharacter.MIN_VALUE to
     * UCharacter.MAX_VALUE.
     *
     * @return an iterator
     * @stable ICU 2.6
     */
    public static ValueIterator getNameIterator() {
        return new UCharacterNameIterator(
                UCharacterName.INSTANCE, UCharacterNameChoice.UNICODE_CHAR_NAME);
    }

    /**
     * {@icu} Returns an empty iterator.
     *
     * <p>Used to return an iterator for the older 1.0 Unicode character names, iterating over
     * codepoints.
     *
     ...
     * <p>Returns an iterator for character names, iterating over codepoints.
     *
     * <p>This API only gets the iterator for the extended names. For modern, most up-to-date
     * Unicode names use getNameIterator() or for older 1.0 Unicode names use get1_0NameIterator().
     *
     * <p>Example of use:<br>
     *
     * <pre>
    ...
    public static ValueIterator getExtendedNameIterator() {
        return new UCharacterNameIterator(
                UCharacterName.INSTANCE, UCharacterNameChoice.EXTENDED_CHAR_NAME);
    }

    /**
     * {@icu} Returns the "age" of the code point.
     *
     * <p>The "age" is the Unicode version when the code point was first designated (as a
     * non-character or for Private Use) or assigned a character.
     *
     * <p>This can be useful to avoid emitting code points to receiving processes that do not accept
     * newer characters.
     *
     ...
     *
     * <p>Unicode, especially in version 3.2, defines many more properties than the original set in
     * UnicodeData.txt.
     *
     * <p>This API is intended to reflect Unicode properties as defined in the Unicode Character
     * Database (UCD) and Unicode Technical Reports (UTR).
     *
     * <p>For details about the properties see <a
     ...
     *
     * <p>Important: If ICU is built with UCD files from Unicode versions below 3.2, then properties
     * marked with "new" are not or not fully available.
     *
     * @param ch code point to test.
     * @param property selector constant from com.ibm.icu.lang.UProperty, identifies which binary
     *     property to check.
     * @return true or false according to the binary Unicode property value for ch. Also false if
     *     property is out of bounds or if the Unicode version does not have data for the property
     *     at all, or not for this code point.
    ...
    /**
     * {@icu} Returns true if the property is true for the string. Same as {@link
     * #hasBinaryProperty(int, int)} if the string contains exactly one code point.
     *
     * <p>Most properties apply only to single code points. <a
     * href="https://www.unicode.org/reports/tr51/#Emoji_Sets">UTS #51 Unicode Emoji</a> defines
     * several properties of strings.
     *
     * @param s String to test.
     * @param property UProperty selector constant, identifies which binary property to check. Must
     *     be BINARY_START&lt;=which&lt;BINARY_LIMIT.
     * @return true or false according to the binary Unicode property value for the string. Also
     *     false if <code>property</code> is out of bounds or if the Unicode version does not have
     *     data for the property at all.
     * @see com.ibm.icu.lang.UProperty
     * @see CharacterProperties#getBinaryPropertySet(int)
     * @stable ICU 70
     */
    public static boolean hasBinaryProperty(CharSequence s, int property) {
        ...
        return UProperty.BASIC_EMOJI <= property
                && property <= UProperty.RGI_EMOJI
     ...
     * {@icu}
     *
     * <p>Check if a code point has the Alphabetic Unicode property.
     *
     ...
     * {@icu}
     *
     * <p>Check if a code point has the Lowercase Unicode property.
     *
     ...
     * {@icu}
     *
     * <p>Check if a code point has the Uppercase Unicode property.
     *
     ...
     * {@icu}
     *
     * <p>Check if a code point has the White_Space Unicode property.
     *
     * <p>Same as UCharacter.hasBinaryProperty(ch, UProperty.WHITE_SPACE).
     *
     * <p>This is different from both UCharacter.isSpace(ch) and UCharacter.isWhitespace(ch)!
     *
     * @param ch codepoint to be tested
     * @stable ICU 2.6
     */
    ...
    /**
     * {@icu} Returns the property value for a Unicode property type of a code point. Also returns
     * binary and mask property values.
     *
     * <p>Unicode, especially in version 3.2, defines many more properties than the original set in
     * UnicodeData.txt.
     *
     * <p>The properties APIs are intended to reflect Unicode properties as defined in the Unicode
     * Character Database (UCD) and Unicode Technical Reports (UTR). For details about the
     * properties see http://www.unicode.org/.
     *
     * <p>For names of Unicode properties see the UCD file PropertyAliases.txt.
     *
     * <pre>
     ...
     *     UProperty.BINARY_START &lt;= type &lt; UProperty.BINARY_LIMIT or UProperty.INT_START
     *     &lt;= type &lt; UProperty.INT_LIMIT or UProperty.MASK_START &lt;= type &lt;
     *     UProperty.MASK_LIMIT.
     * @return numeric value that is directly the property value or, for enumerated properties,
     *     corresponds to the numeric value of the enumerated constant of the respective property
     *     value type ({@link ECharacterCategory}, {@link ECharacterDirection}, {@link
     *     DecompositionType}, etc.). Returns 0 or 1 (for false / true) for binary Unicode
     *     properties. Returns a bit-mask for mask properties. Returns 0 if 'type' is out of bounds
     *     or if the Unicode version does not have data for the property at all, or not for this
     *     code point.
     * @see UProperty
    ...
    public static String getStringPropertyValue(int propertyEnum, int codepoint, int nameChoice) {
        if ((propertyEnum >= UProperty.BINARY_START && propertyEnum < UProperty.BINARY_LIMIT)
            ...
            return getPropertyValueName(
                    propertyEnum, getIntPropertyValue(codepoint, propertyEnum), nameChoice);
        }
     ...
     * together with UCharacter.getIntPropertyMaxValue(int) to allocate arrays of
     * com.ibm.icu.text.UnicodeSet or similar.
     *
     * @param type UProperty selector constant, identifies which binary property to check. Must be
     *     UProperty.BINARY_START &lt;= type &lt; UProperty.BINARY_LIMIT or UProperty.INT_START
     *     &lt;= type &lt; UProperty.INT_LIMIT.
     * @return Minimum value returned by UCharacter.getIntPropertyValue(int) for a Unicode property.
     *     0 if the property selector 'type' is out of range.
     * @see UProperty
     * @see #hasBinaryProperty
     * @see #getUnicodeVersion
     * @see #getIntPropertyMaxValue
    ...
    /**
     * {@icu} Returns the maximum value for an integer/binary Unicode property. Can be used together
     * with UCharacter.getIntPropertyMinValue(int) to allocate arrays of com.ibm.icu.text.UnicodeSet
     * or similar. Examples for min/max values (for Unicode 3.2):
     *
     * <ul>
     ...
     *     UProperty.BINARY_START &lt;= type &lt; UProperty.BINARY_LIMIT or UProperty.INT_START
     *     &lt;= type &lt; UProperty.INT_LIMIT.
     * @return Maximum value returned by u_getIntPropertyValue for a Unicode property. &lt;= 0 if
     *     the property selector 'type' is out of range.
     * @see UProperty
     * @see #hasBinaryProperty
     * @see #getUnicodeVersion
     * @see #getIntPropertyMaxValue
    ...
    /**
     * Is cp a Unicode code point U+0000..U+10FFFF? See <a
     * href="https://www.unicode.org/glossary/#code_point">Unicode Glossary: Code Point</a>.
    ...
    /**
     * {@icu} Is cp a Unicode scalar value, that is, a non-surrogate code point? Only scalar values
     * can be represented in well-formed UTF-8/16/32. See <a
    ...
    /**
     * {@icu} Returns an IterableOfInt over all Unicode code points U+0000..U+10FFFF. See <a
     * href="https://www.unicode.org/glossary/#code_point">Unicode Glossary: Code Point</a>.
     *
    ...
    /**
     * {@icu} Returns an IntStream over all Unicode code points U+0000..U+10FFFF. See <a
     * href="https://www.unicode.org/glossary/#code_point">Unicode Glossary: Code Point</a>.
     *
     * <p>Intended for test and builder code.
     *
    ...
    public static final IntStream allScalarValuesStream() {
        return IntStream.concat(
                IntStream.rangeClosed(0, 0xd7ff), IntStream.rangeClosed(0xe000, 0x10ffff));
    }
    ...
    /**
     * Same as {@link Character#isHighSurrogate}, except that the ICU version accepts <code>int
     * </code> for code points.
     *
     * @param codePoint the code point to check (In ICU 3.0-69 the type of this parameter was <code>
     *     char</code>.)
    ...
    /**
     * Same as {@link Character#isLowSurrogate}, except that the ICU version accepts <code>int
     * </code> for code points.
     *
     * @param codePoint the code point to check (In ICU 3.0-69 the type of this parameter was <code>
     *     char</code>.)
    ...
    /**
     * Same as {@link Character#isSurrogatePair}, except that the ICU version accepts <code>int
     * </code> for code points.
     *
     * @param high the high (lead) unit (In ICU 3.0-69 the type of both parameters was <code>char
     *     </code>.)
    ...
    /**
     * {@icu} Is cp a Unicode noncharacter? See <a
     * href="https://www.unicode.org/glossary/#noncharacter">Unicode Glossary: Noncharacter</a>.
     *
     * @param cp the code point to check
    ...
    /**
     * Same as {@link Character#charCount}. Returns the number of chars needed to represent the code
     * point (1 or 2). This does not check the code point for validity.
     *
     * @param cp the code point to check
    ...
    /**
     * Same as {@link Character#toCodePoint}, except that the ICU version accepts <code>int</code>
     * for code points. Returns the code point represented by the two surrogate code units. This
     * does not check the surrogate pair for validity.
     *
     * @param high the high (lead) surrogate (In ICU 3.0-69 the type of both parameters was <code>
     *     char</code>.)
    ...
    /**
     * Same as {@link Character#codePointAt(CharSequence, int)}. Returns the code point at index.
    ...
    /**
     * Same as {@link Character#codePointAt(char[], int)}. Returns the code point at index. This
     * examines only the characters at index and index+1.
     *
    ...
    /**
     * Same as {@link Character#codePointAt(char[], int, int)}. Returns the code point at index.
     * This examines only the characters at index and index+1.
     *
     * @param text the characters to check
     * @param index the index of the first or only char forming the code point
    ...
    /**
     * Same as {@link Character#codePointBefore(CharSequence, int)}. Return the code point before
     * index. This examines only the characters at index-1 and index-2.
     *
     * @param seq the characters to check
    ...
    /**
     * Same as {@link Character#codePointBefore(char[], int)}. Returns the code point before index.
    ...
    /**
     * Same as {@link Character#codePointBefore(char[], int, int)}. Return the code point before
     * index. This examines only the characters at index-1 and index-2.
     *
     * @param text the characters to check
     * @param index the index after the last or only char forming the code point
    ...
    /**
     * Same as {@link Character#toChars(int, char[], int)}. Writes the chars representing the code
     * point into the destination at the given index.
     *
    ...
    /**
     * Same as {@link Character#toChars(int)}. Returns a char array representing the code point.
     *
     * @param cp the code point to convert
    ...
    /**
     * Equivalent to the {@link Character#getDirectionality(char)} method, for convenience. Returns
     * a byte representing the directionality of the character.
     *
     * <p>{@icunote} Unlike {@link Character#getDirectionality(char)}, this returns
     * DIRECTIONALITY_LEFT_TO_RIGHT for undefined or out-of-bounds characters.
     *
     * <p>{@icunote} The return value must be tested using the constants defined in {@link
     * UCharacterDirection} and its interface {@link UCharacterEnums.ECharacterDirection} since the
     * values are different from the ones defined by <code>java.lang.Character</code>.
     *
     * @param cp the code point to check
    ...
    /**
     * Equivalent to the {@link Character#codePointCount(CharSequence, int, int)} method, for
    ...
    /**
     * Equivalent to the {@link Character#codePointCount(char[], int, int)} method, for convenience.
     * Counts the number of code points in the range of text.
     *
     * @param text the characters to check
     * @param start the start of the range
     * @param limit the limit of the range
    ...
    /**
     * Equivalent to the {@link Character#offsetByCodePoints(CharSequence, int, int)} method, for
     * convenience. Adjusts the char index by a code point offset.
     *
     * @param text the characters to check
    ...
    public static int offsetByCodePoints(CharSequence text, int index, int codePointOffset) {
        if (index < 0 || index > text.length()) {
            throw new IndexOutOfBoundsException(
                    "index ( " + index + ") out of range 0, " + text.length());
        }
    ...
    /**
     * Equivalent to the {@link Character#offsetByCodePoints(char[], int, int, int, int)} method,
     * for convenience. Adjusts the char index by a code point offset.
     *
     * @param text the characters to check
    ...
    public static int offsetByCodePoints(
            char[] text, int start, int count, int index, int codePointOffset) {
