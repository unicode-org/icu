...
/**
 * This class interact with TimeZoneNames and LocaleDisplayNames to format and parse time zone's
 * generic display names. It is not recommended to use this class directly, instead use
...
public class TimeZoneGenericNames implements Serializable, Freezable<TimeZoneGenericNames> {
    ...
    public enum Pattern {
        ...
        // The format pattern such as "{1} ({0})", where {1} is the metazone, and {0} is the country
        // or city.
    ...
    /**
     * Constructs a <code>TimeZoneGenericNames</code> with the given locale and the <code>
     * TimeZoneNames</code>.
     *
    ...
    /**
     * Private method initializing the instance of <code>TimeZoneGenericName</code>. This method
     * should be called from a constructor and readObject.
     */
    private void init() {
        if (_tznames == null) {
    ...
    /**
     * Constructs a <code>TimeZoneGenericNames</code> with the given locale. This constructor is
     * private and called from {@link #getInstance(ULocale)}.
     *
     * @param locale the locale
     */
    ...
    /**
     * The factory method of <code>TimeZoneGenericNames</code>. This static method returns a frozen
     * instance of cached <code>TimeZoneGenericNames</code>.
     *
     * @param locale the locale
    ...
    /**
     * Returns the display name of the time zone for the given name type at the given date, or null
     * if the display name is not available.
     *
    ...
    /**
     * Sets the pattern string for the pattern type. Note: This method is designed for CLDR ST - not
     * for common use.
     *
     ...
     *
     * <p>1. If a generic non-location string is available for the zone, return it. 2. If a generic
     * non-location string is associated with a meta zone and the zone never use daylight time
     * around the given date, use the standard string (if available). 3. If a generic non-location
     * string is associated with a meta zone and the offset at the given time is different from the
     * preferred zone for the current locale, then return the generic partial location string (if
     * available) 4. If a generic non-location string is not available, use generic location string.
     *
     * @param tz the requested time zone
     * @param date the date
     * @param type the generic name type, either LONG or SHORT
     * @return the name used for a generic name type, which could be the generic name, or the
     *     standard name (if the zone does not observes DST around the date), or the partial
     *     location name.
     */
    private String formatGenericNonLocationName(TimeZone tz, GenericNameType type, long date) {
        ...
        NameType nameType =
                (type == GenericNameType.LONG) ? NameType.LONG_GENERIC : NameType.SHORT_GENERIC;
    ...
    /**
     * Private simple pattern formatter used for formatting generic location names and partial
     * location names. We intentionally use JDK MessageFormat for performance reason.
     *
    ...
    /**
     * Private method returning LocaleDisplayNames instance for the locale of this instance. Because
     * LocaleDisplayNames is only used for generic location format and partial location format, the
     * LocaleDisplayNames is instantiated lazily.
     *
                ...
                for (NameType genNonLocType : GENERIC_NON_LOCATION_TYPES) {
                    ...
                    if (mzGenName != null) {
                        ...
                        getPartialLocationName(
                                tzCanonicalID,
                                mzID,
                                (genNonLocType == NameType.LONG_GENERIC),
                                mzGenName);
                    }
                }
            }
    ...
    /**
     * Private method for formatting partial location names. This format is used when a generic name
     * of a meta zone is available, but the given time zone is not a reference zone (golden zone) of
     * the meta zone.
     *
     ...
     */
    private String getPartialLocationName(
            String tzID, String mzID, boolean isLong, String mzDisplayName) {
    ...
    /**
     * A class used for returning the name search result used by {@link
    ...
    public static class GenericMatchInfo {
        ...
        private GenericMatchInfo(
                GenericNameType nameType, String tzID, int matchLength, TimeType timeType) {
    ...
    /**
     * A private class implementing the search callback interface in <code>TextTrieMap</code> for
    ...
    private static class GenericNameSearchHandler implements ResultHandler<NameInfo> {
        ...
        public boolean handlePrefixMatch(int matchLength, Iterator<NameInfo> values) {
            while (values.hasNext()) {
                ...
                GenericMatchInfo matchInfo =
                        new GenericMatchInfo(info.type, info.tzID, matchLength);
                if (_matches == null) {
    ...
    /**
     * Returns the best match of time zone display name for the specified types in the given text at
     * the given offset.
     *
     * @param text the text
     ...
     */
    public GenericMatchInfo findBestMatch(
            String text, int start, EnumSet<GenericNameType> genericTypes) {
        ...
        if (tznamesMatches != null) {
            ...
            if (longestMatch != null) {
                ...
                if (bestMatch.matchLength() == (text.length() - start)) {
                    ...
                    //                    if (bestMatch.nameType != GenericNameType.LONG ||
                    // bestMatch.timeType != TimeType.STANDARD) {
                    //                        return bestMatch;
                    //                    }
    ...
    /**
     * Returns a collection of time zone display name matches for the specified types in the given
     ...
     */
    public Collection<GenericMatchInfo> find(
            String text, int start, EnumSet<GenericNameType> genericTypes) {
        if (text == null || text.length() == 0 || start < 0 || start >= text.length()) {
            throw new IllegalArgumentException("bad input text or range");
        }
    ...
    private GenericMatchInfo createGenericMatchInfo(MatchInfo matchInfo) {
        ...
        switch (matchInfo.nameType()) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unexpected MatchInfo name type - " + matchInfo.nameType());
        }
        ...
        GenericMatchInfo gmatch =
                new GenericMatchInfo(nameType, tzID, matchInfo.matchLength(), timeType);
    ...
    /**
     * Returns a collection of time zone display name matches for the specified types in the given
     * text at the given offset. This method only finds matches from the TimeZoneNames used by this
     * object.
     *
     * @param text the text
     * @param start the start offset in the text
     ...
     * @return A collection of match info.
     */
    private Collection<MatchInfo> findTimeZoneNames(
            String text, int start, EnumSet<GenericNameType> types) {
    ...
    /**
     * Returns a collection of time zone display name matches for the specified types in the given
     * text at the given offset. This method only finds matches from the local trie, that contains
     * 1) generic location names and 2) long/short generic partial location names, used by this
     * object.
     *
     * @param text the text
     * @param start the start offset in the text
     ...
     * @return A collection of match info.
     */
    private synchronized Collection<GenericMatchInfo> findLocal(
            String text, int start, EnumSet<GenericNameType> types) {
