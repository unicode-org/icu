...
public class TrieIterator implements RangeValueIterator {
     ...
     *
     * <p>The next set of codepoints with the same value type will be calculated during this call
     * and returned in the argument element.
     *
    ...
    /**
     * Called by next() to extracts a 32 bit value from a trie value used for comparison. This
     * method is to be overwritten if special manipulation is to be done to retrieve a relevant
     * comparison. The default function is to return the value as it is.
     *
    ...
    /**
     * Finding the next element. This method is called just before returning the result of next().
     * We always store the next element before it is requested. In the case that we have to continue
     * calculations into the supplementary planes, a false will be returned.
     *
     * @param element return result object
     * @return true if the next range is found, false if we have to proceed to the supplementary
     *     range.
     */
    ...
    /**
     * Finds the next supplementary element. For each entry in the trie, the value to be delivered
     * is passed through extract(). We always store the next element before it is requested. Called
     * after calculateNextBMP() completes its round of BMP characters. There is a slight difference
     * in the usage of m_currentCodepoint_ here as compared to calculateNextBMP(). Though both
     * represents the lower bound of the next element, in calculateNextBMP() it gets set at the
     * start of any loop, where-else, in calculateNextSupplementary() since m_currentCodepoint_
     * already contains the lower bound of the next element (passed down from calculateNextBMP()),
     * we keep it till the end before resetting it to the new value. Note, if there are no more
     * iterations, it will never get to here. Blocked out by next().
     *
     * @param element return result object
     */
    private final void calculateNextSupplementaryElement(Element element) {
        ...
        while (nextLead < TRAIL_SURROGATE_MIN_VALUE_) {
            ...
            m_nextIndex_ =
                    m_trie_.m_dataManipulate_.getFoldingOffset(
                            m_trie_.getValue(leadBlock + (nextLead & Trie.INDEX_STAGE_3_MASK_)));
            if (m_nextIndex_ <= 0) {
    ...
    /**
     * Internal block value calculations Performs calculations on a data block to find codepoints in
     * m_nextBlock_ after the index m_nextBlockIndex_ that has the same value. Note m_*_ variables
     * at this point is the next codepoint whose value has not been calculated. But when returned
     * with false, it will be the last codepoint whose value has been calculated.
     *
     * @param currentValue the value which other codepoints are tested against
     * @return true if the whole block has the same value as currentValue or if the whole block has
     *     been calculated, false otherwise.
     */
    ...
    /**
     * Internal block value calculations Performs calculations on a data block to find codepoints in
     * m_nextBlock_ that has the same value. Will call checkBlockDetail() if highlevel check fails.
     * Note m_*_ variables at this point is the next codepoint whose value has not been calculated.
     *
    ...
    /**
     * Internal block value calculations Performs calculations on multiple data blocks for a set of
     * trail surrogates to find codepoints in m_nextBlock_ that has the same value. Will call
     * checkBlock() for internal block checks. Note m_*_ variables at this point is the next
     * codepoint whose value has not been calculated.
     *
     * @param currentValue the value which other codepoints are tested against
     * @return true if the whole block has the same value as currentValue or if the whole block has
     *     been calculated, false otherwise.
    ...
    /**
     * Checks if we are beginning at the start of a initial block. If we are then the rest of the
     * codepoints in this initial block has the same values. We increment m_nextCodepoint_ and
     * relevant data members if so. This is used only in for the supplementary codepoints because
    ...
    /**
     * Number of stage 1 indexes for supplementary calculations that maps to each lead surrogate
     * character. See second pass into getRawOffset for the trail surrogate character. 10 for
     * significant number of bits for trail surrogates, 5 for what we discard during shifting.
     */
