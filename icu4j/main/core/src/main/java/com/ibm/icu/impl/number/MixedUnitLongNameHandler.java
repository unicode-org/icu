...
public class MixedUnitLongNameHandler
    ...
    /**
     * Stores unit data for each of the individual units. For each unit, it stores ARRAY_LENGTH
     * strings, as returned by getMeasureData.
     */
    private List<String[]> fMixedUnitData;

    /**
     * A localized NumberFormatter used to format the integer-valued bigger units of Mixed Unit
     * measurements.
     */
    ...
    /**
     * Construct a localized MixedUnitLongNameHandler for the specified MeasureUnit. It must be a
     * MIXED unit.
     *
     * <p>
     *
     * @param locale The desired locale.
     * @param mixedUnit The mixed measure unit to construct a MixedUnitLongNameHandler for.
    ...
    public static MixedUnitLongNameHandler forMeasureUnit(
        ...
        for (int i = 0; i < individualUnits.size(); i++) {
            ...
            LongNameHandler.getMeasureData(
                    locale, individualUnits.get(i), width, unitDisplayCase, unitData);
        ...
        result.fListFormatter =
                ListFormatter.getInstance(locale, ListFormatter.Type.UNITS, listWidth);
    ...
    /**
     * Produces a plural-appropriate Modifier for a mixed unit: {@code quantity} is taken as the
     * final smallest unit, while the larger unit values must be provided by {@code
     * micros.mixedMeasures}, micros being the MicroProps instance returned by the parent.
     *
     * <p>This function must not be called if this instance has no parent: call
    ...
    /**
     * Produces a plural-appropriate Modifier for a mixed unit: {@code quantity} is taken as the
     * final smallest unit, while the larger unit values must be provided via {@code
    ...
    /**
     * Required for ModifierStore. And ModifierStore is required by SimpleModifier constructor's
     * last parameter. We assert his will never get called though.
     */
    @Override
    ...
    /**
     * For a mixed unit, returns a Modifier that takes only one parameter: the smallest and final
     * unit of the set. The bigger units' values and labels get baked into this Modifier, together
     * with the unit label of the final unit.
     */
    private Modifier getMixedUnitModifier(DecimalQuantity quantity, MicroProps micros) {
        ...
        for (int i = 0; i < micros.mixedMeasures.size(); i++) {
            ...
            if (i == micros.indexOfQuantity) {
                ...
                String quantitySimpleFormat =
                        LongNameHandler.getWithPlural(this.fMixedUnitData.get(i), quantityPlural);
                SimpleFormatter finalFormatter =
                        SimpleFormatter.compileMinMaxArguments(quantitySimpleFormat, 0, 1);
            ...
            DecimalQuantity fdec =
                    new DecimalQuantity_DualStorageBCD(micros.mixedMeasures.get(i).getNumber());
            ...
            String simpleFormat =
                    LongNameHandler.getWithPlural(this.fMixedUnitData.get(i), pluralForm);
            SimpleFormatter compiledFormatter =
                    SimpleFormatter.compileMinMaxArguments(simpleFormat, 0, 1);
