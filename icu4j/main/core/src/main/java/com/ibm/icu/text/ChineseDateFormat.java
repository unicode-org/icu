...
/**
 * A concrete {@link DateFormat} for {@link com.ibm.icu.util.ChineseCalendar}. This class handles a
 * <code>ChineseCalendar</code>-specific field, <code>ChineseCalendar.IS_LEAP_MONTH</code>. It also
 * redefines the handling of two fields, <code>ERA</code> and <code>YEAR</code>. The former is
 * displayed numerically, instead of symbolically, since it is the numeric cycle number in <code>
 * ChineseCalendar</code>. The latter is numeric, as before, but has no special 2-digit Y2K
 * behavior.
 *
 * <p>With regard to <code>ChineseCalendar.IS_LEAP_MONTH</code>, this class handles parsing
 * specially. If no string symbol is found at all, this is taken as equivalent to an <code>
 * IS_LEAP_MONTH</code> value of zero. This allows formats to display a special string (e.g., "*")
 * for leap months, but no string for normal months.
 *
 * <p>Summary of field changes vs. {@link SimpleDateFormat}:
 *
 * <pre>
...
public class ChineseDateFormat extends SimpleDateFormat {
    ...
    /**
     * Construct a ChineseDateFormat from a date format pattern, numbering system override and
     * locale
     *
     * @param pattern the pattern
    ...
    public ChineseDateFormat(String pattern, String override, ULocale locale) {
        super(
                pattern,
                new ChineseDateFormatSymbols(locale),
                new ChineseCalendar(TimeZone.getDefault(), locale),
                locale,
                true,
                override);
    }
    ...
    public static class Field extends DateFormat.Field {
        ...
        public static final Field IS_LEAP_MONTH =
                new Field("is leap month", ChineseCalendar.IS_LEAP_MONTH);

        /**
         * Constructs a <code>ChineseDateFormat.Field</code> with the given name and the <code>
         * ChineseCalendar</code> field which this attribute represents. Use -1 for <code>
         * calendarField</code> if this field does not have a corresponding <code>ChineseCalendar
         * </code> field.
         *
         ...
         * ChineseCalendar</code> field <code>calendarField</code>. If there is no corresponding
         * <code>Field</code> is available, null is returned.
         *
