...
/**
 * Base class for cache implementations. To use, instantiate a subclass of a concrete implementation
 * class, where the subclass implements the createInstance() method, and call get() with the key and
 * the data. The get() call will use the data only if it needs to call createInstance(), otherwise
 * the data is ignored.
 *
...
public abstract class CacheBase<K, V, D> {
    /**
     * Retrieves an instance from the cache. Calls createInstance(key, data) if the cache does not
     * already contain an instance with this key. Ignores data if the cache already contains an
     * instance with this key.
     *
