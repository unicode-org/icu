...
/**
 * A compression engine implementing the Standard Compression Scheme for Unicode (SCSU) as outlined
 * in <A HREF="https://www.unicode.org/reports/tr6">Unicode Technical Report #6</A>.
 *
 * <p>The SCSU works by using dynamically positioned <EM>windows</EM> consisting of 128 consecutive
 * characters in Unicode. During compression, characters within a window are encoded in the
 * compressed stream as the bytes {@code 0x7F - 0xFF}. The SCSU provides transparency for the
 * characters (bytes) between {@code U+0000 - U+00FF}. The SCSU approximates the storage size of
 * traditional character sets, for example 1 byte per character for ASCII or Latin-1 text, and 2
 * bytes per character for CJK ideographs.
 *
 * <p><STRONG>USAGE</STRONG>
 *
 * <p>The static methods on {@code UnicodeCompressor} may be used in a straightforward manner to
 * compress simple strings:
 *
 * <PRE>
 ...
 * usage, {@code UnicodeCompressor} offers more powerful APIs allowing iterative compression:
 *
 * <PRE>
...
public final class UnicodeCompressor implements SCSU {
     ...
     * Compress a Unicode character array into a byte array.
     *
     * <p>This function will only consume input that can be completely output.
     *
     ...
     * @param charsRead A one-element array. If not null, on return the number of characters read
     *     from charBuffer.
     * @param byteBuffer A buffer to receive the compressed data. This buffer must be at minimum
     *     four bytes in size.
     * @param byteBufferStart The starting offset to which to write compressed data.
            ...
            switch (fMode) {
                ...
                case SINGLEBYTEMODE:
                    ...
                    while (ucPos < charBufferLimit && bytePos < byteBufferLimit) {
                        ...
                        // get next char
                        if (ucPos < charBufferLimit) nextUC = charBuffer[ucPos];
                        else nextUC = INVALIDCHAR;
                        ...
                        // and subtracting the window's offset
                            ...
                            byteBuffer[bytePos++] =
                                    (byte) (curUC - fOffsets[fCurrentWindow] + COMPRESSIONOFFSET);
                        }
                        ...
                        // it or quote from it
                            ...
                            // look ahead
                            if ((ucPos + 1) < charBufferLimit) forwardUC = charBuffer[ucPos + 1];
                            else forwardUC = INVALIDCHAR;
                            ...
                            if (inDynamicWindow(nextUC, whichWindow)
                                ...
                                byteBuffer[bytePos++] =
                                        (byte) (curUC - fOffsets[whichWindow] + COMPRESSIONOFFSET);
                        ...
                        // define a new one or switch to unicode mode
                            ...
                            if ((fIndexCount[curIndex] > 1)
                                ...
                                byteBuffer[bytePos++] =
                                        (byte) (curUC - sOffsetTable[curIndex] + COMPRESSIONOFFSET);
    ...
    /**
     * Create the index value for a character. For more information on this function, refer to table
     * X-3 <A HREF="https://www.unicode.org/reports/tr6">UTR6</A>.
     *
    ...
    private static int makeIndex(int c) {
        ...
        if (c >= 0x00C0 && c < 0x0140) return LATININDEX;
        else if (c >= 0x0250 && c < 0x02D0) return IPAEXTENSIONINDEX;
        else if (c >= 0x0370 && c < 0x03F0) return GREEKINDEX;
        else if (c >= 0x0530 && c < 0x0590) return ARMENIANINDEX;
        else if (c >= 0x3040 && c < 0x30A0) return HIRAGANAINDEX;
        else if (c >= 0x30A0 && c < 0x3120) return KATAKANAINDEX;
        else if (c >= 0xFF60 && c < 0xFF9F) return HALFWIDTHKATAKANAINDEX;

        // calculate index
        else if (c >= 0x0080 && c < 0x3400) return (c / 0x80) & 0xFF;
        else if (c >= 0xE000 && c <= 0xFFFF) return ((c - 0xAC00) / 0x80) & 0xFF;
     ...
     * @return true if {@code c} will fit in {@code whichWindow}, false otherwise.
     */
     ...
     *
     * @param c The character in question
     * @return The dynamic window containing {@code c}, or INVALIDWINDOW if not defined.
     */
     ...
     *
     * @param c The character in question
     * @return The static window containing {@code c}, or INVALIDWINDOW if not defined.
     */
