...
public class BytesTrieTest extends CoreTestFmwk {
    ...
    public void Test59IteratorFromBytes() {
        ...
        checkIterator(
                BytesTrie.iterator(
                        trieBytes.array(), trieBytes.arrayOffset() + trieBytes.position(), 0),
                data);
    }
    ...
    private void checkFirst(BytesTrie trie, StringAndValue data[], int dataLength) {
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (firstResult != trie.reset().next(c)
                ...
                errln(
                        String.format(
                                "trie.first(%c)!=trie.reset().next(same) for %s", c, data[i].s));
            }
        }
        trie.reset();
    }
            ...
            if ((result == BytesTrie.Result.INTERMEDIATE_VALUE) != nextContinues) {
                errln(
                        "(trie.current()==BytesTrie.Result.INTERMEDIATE_VALUE) contradicts "
                                + "(trie.next(some UChar)!=BytesTrie.Result.NO_MATCH) after end of "
                                + data[i].s);
            }
            trie.reset();
        }
