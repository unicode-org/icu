...
public class UtilityTest extends CoreTestFmwk {
    ...
    public void TestFormat() {
        ...
        for (int i = 0; i < data.length; i++) {
            assertEquals(
                    "formatForSource(\"" + data[i] + "\")",
                    result[i],
                    Utility.formatForSource(data[i]));
        }
        for (int i = 0; i < data.length; i++) {
            assertEquals(
                    "format1ForSource(\"" + data[i] + "\")",
                    result1[i],
                    Utility.format1ForSource(data[i]));
        }
    }
    ...
    public void TestCompareUnsigned() {
        int data[] = {
            0, 1, 0x8fffffff, -1, Integer.MAX_VALUE, Integer.MIN_VALUE, 2342423, -2342423
        };
        for (int i = 0; i < data.length; i++) {
    ...
    public void TestByteArrayWrapper() {
        ...
        if (x.equals(y)) errln("FAIL: test (operator !=): Failed!");
    ...
    public void TestSourceLocation() {
        ...
        assertTrue(
                "here < there < hereAgain",
                here.compareTo(there) < 0 && there.compareTo(hereAgain) < 0);
    }
