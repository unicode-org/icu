...
/**
 * @author Markus W. Scherer BiDi conformance test, using the Unicode BidiTest.txt and
 *     BidiCharacterTest.txt files. Ported from ICU4C intltest/bidiconf.cpp .
 */
...
public class BiDiConformanceTest extends CoreTestFmwk {
    ...
    */
            ...
            while (errorCount < 20 && (line = bidiTestFile.readLine()) != null) {
                ...
                if (paraDirection == 0) {
                    ...
                    errln(
                            " on line "
                                    + lineNumber
                                    + ": Input paragraph direction incorrect at "
                                    + line);
                    continue;
                }
                ...
                if (resolvedParaLevel < 0 || resolvedParaLevel > (Bidi.MAX_EXPLICIT_LEVEL + 1)) {
                    ...
                    errln(
                            " on line "
                                    + lineNumber
                                    + ": Resolved paragraph level incorrect at "
                                    + line);
                    continue;
                }
                ...
                if (actualParaLevel != resolvedParaLevel) {
                    errln(
                            " ------------ Wrong resolved paragraph level; expected "
                                    + resolvedParaLevel
                                    + " actual "
                                    + actualParaLevel);
                    printErrorLine();
    ...
    private static final byte paraLevels[] = {
        Bidi.DIRECTION_DEFAULT_LEFT_TO_RIGHT, 0, 1, Bidi.DIRECTION_DEFAULT_RIGHT_TO_LEFT
    };
        ...
        while (skipWhitespace() && (c0 = line.charAt(lineIndex)) != ';') {
            ...
            if (c0 == 'L') {
                ...
                if (line.charAt(lineIndex + 2) == 'F') {
                    ...
                    && (lineIndex + 2) < line.length()
                    && line.charAt(lineIndex + 1) == 'S'
                    && line.charAt(lineIndex + 2) == 'M') {
                biDiClass = UCharacterDirection.DIR_NON_SPACING_MARK;
            } else if (c0 == 'F'
                    && (lineIndex + 2) < line.length()
                    && line.charAt(lineIndex + 1) == 'S'
                    && line.charAt(lineIndex + 2) == 'I') {
            ...
            if (biDiClass == UCharacterDirection.CHAR_DIRECTION_COUNT
                ...
                throw new IllegalArgumentException(
                        "BiDi class string not recognized at "
    ...
    private boolean checkLevels(byte actualLevels[]) {
        ...
        if (levelsCount != actualLevels.length) {
            errln(
                    " ------------ Wrong number of level values; expected "
                            + levelsCount
                            + " actual "
                            + actualLevels.length);
            isOk = false;
    ...
    private boolean checkOrdering(Bidi ubidi) {
        if (orderingCount < 0) return true;
        boolean isOk = true;
        ...
        for (i = visualIndex = 0; i < resultLength; ++i) {
            ...
            if (visualIndex < orderingCount && logicalIndex != ordering[visualIndex]) {
                errln(
                        " ------------ Wrong ordering value at visual index "
                                + visualIndex
                                + "; expected "
                                + ordering[visualIndex]
                                + " actual "
                                + logicalIndex);
                isOk = false;
                break;
        ...
        if (isOk && orderingCount != visualIndex) {
            errln(
                    " ------------ Wrong number of ordering values; expected "
                            + orderingCount
                            + " actual "
                            + visualIndex);
            isOk = false;
