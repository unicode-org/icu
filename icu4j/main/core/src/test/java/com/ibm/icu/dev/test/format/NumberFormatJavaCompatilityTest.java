...
public class NumberFormatJavaCompatilityTest {
    ...
    public void testParseGroupingSeparator() {
        ...
        // Test that grouping separator must not be present when the group separator is NOT
        // specified
    ...
    private void assertParsedOpenJDK(String pattern, String input, Number expected) {
        java.text.DecimalFormat df =
                new java.text.DecimalFormat(pattern, new java.text.DecimalFormatSymbols(Locale.US));
        ParsePosition ppos = new ParsePosition(0);
        Number actual = df.parse(input, ppos);
        assertEquals(
                String.format("pattern: %s input: %s", pattern, input),
