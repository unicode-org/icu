...
public class TestCompatibility extends BidiFmwk {
    ...
    void compareBidi(Bidi bidi, java.text.Bidi jbidi) {
        ...
        if (bidi.baseIsLeftToRight() != jbidi.baseIsLeftToRight()) {
            errln(
                    "Discrepancy in baseIsLeftToRight for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            ...
                            + "\n   jbidi: "
                            + jbidi.baseIsLeftToRight());
        }
        if (bidi.getBaseLevel() != jbidi.getBaseLevel()) {
            errln(
                    "Discrepancy in getBaseLevel for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            + u16ToPseudo(bidi.getTextAsString())
        ...
        if (bidi.getLength() != jbidi.getLength()) {
            errln(
                    "Discrepancy in getLength for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            + u16ToPseudo(bidi.getTextAsString())
        ...
        for (int i = 0; i < len; i++) {
            if (bidi.getLevelAt(i) != jbidi.getLevelAt(i)) {
                errln(
                        "Discrepancy in getLevelAt for offset "
                                + i
                                + " of case "
                                + "(level="
                                + paraLevel
        ...
        if (bidi.getRunCount() != jbidi.getRunCount()) {
            if (!(len == 0 && jbidi.getRunCount() == 1)) {
                errln(
                        "Discrepancy in getRunCount for case "
                                + "(level="
                                + paraLevel
                                + "): "
                                + u16ToPseudo(bidi.getTextAsString())
        ...
        for (int i = 0; i < runCount; i++) {
            if (bidi.getRunLevel(i) != jbidi.getRunLevel(i)) {
                errln(
                        "Discrepancy in getRunLevel for run "
                                + i
                                + " of case "
                                + "(level="
                                + paraLevel
            ...
            if (bidi.getRunLimit(i) != jbidi.getRunLimit(i)) {
                errln(
                        "Discrepancy in getRunLimit for run "
                                + i
                                + " of case "
                                + "(level="
                                + paraLevel
            ...
            if (bidi.getRunStart(i) != jbidi.getRunStart(i)) {
                errln(
                        "Discrepancy in getRunStart for run "
                                + i
                                + " of case "
                                + "(level="
                                + paraLevel
        ...
        if (bidi.isLeftToRight() != jbidi.isLeftToRight()) {
            errln(
                    "Discrepancy in isLeftToRight for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            + u16ToPseudo(bidi.getTextAsString())
        ...
        if (bidi.isMixed() != jbidi.isMixed()) {
            errln(
                    "Discrepancy in isMixed for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            + u16ToPseudo(bidi.getTextAsString())
        ...
        if (bidi.isRightToLeft() != jbidi.isRightToLeft()) {
            errln(
                    "Discrepancy in isRightToLeft for case "
                            + "(level="
                            + paraLevel
                            + "): "
                            + u16ToPseudo(bidi.getTextAsString())
        ...
        if (Bidi.requiresBidi(text, 0, text.length)
            ...
            errln(
                    "Discrepancy in requiresBidi for case "
                            + u16ToPseudo(bidi.getTextAsString())
                            + "\n    bidi: "
    ...
    public void testCompatibility() {
        ...
        if (javaVersion.startsWith("1.4.0")
        ...
        as.addAttribute(
                TextAttribute.NUMERIC_SHAPING, NumericShaper.getShaper(NumericShaper.ARABIC));
        ...
        jbidi =
                new java.text.Bidi(
                        text, 3, null, 0, 10, java.text.Bidi.DIRECTION_DEFAULT_LEFT_TO_RIGHT);
        compareBidi(bidi, jbidi);
        out = bidi.writeReordered(0);
        ...
        jbidi =
                new java.text.Bidi(
                        text, 3, null, 0, 10, java.text.Bidi.DIRECTION_DEFAULT_RIGHT_TO_LEFT);
        compareBidi(bidi, jbidi);
        out = bidi.writeReordered(0);
        ...
        jbidi =
                new java.text.Bidi(
                        text, 3, levels, 0, 10, java.text.Bidi.DIRECTION_DEFAULT_RIGHT_TO_LEFT);
        compareBidi(bidi, jbidi);
        out = bidi.writeReordered(0);
        ...
        if (!strjbidi.equals(strbidi)) {
            errln(
                    "Discrepancy in reorderVisually "
                            + "\n      bidi: "
                            + strbidi
                            + "\n     jbidi: "
                            + strjbidi);
            ...
            logln(
                    "Correct match in reorderVisually "
                            + "\n      bidi: "
                            + strbidi
                            + "\n     jbidi: "
                            + strjbidi);
