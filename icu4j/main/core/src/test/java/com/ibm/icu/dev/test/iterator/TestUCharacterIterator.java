...
public class TestUCharacterIterator extends CoreTestFmwk {
        ...
        while (ch != UCharacterIterator.DONE) {
            ...
            if (index != ITERATION_SUPPLEMENTARY_INDEX) {
                ...
                if (UTF16.getLeadSurrogate(ch) != iterator.next()
                    ...
                    errln(
                            "Error mismatch in next and nextCodePoint for "
        ...
        while (ch != UCharacterIterator.DONE) {
            ...
            if (index != ITERATION_SUPPLEMENTARY_INDEX) {
                ...
                if (UTF16.getLeadSurrogate(ch) != iterator.previous()
                    ...
                    errln(
                            "Error mismatch in previous and "
    ...
    public void TestIterationUChar32() {
        ...
        int i;
        {
            ...
            if (!iterText.equals(text)) errln("iter.getText() failed");
            ...
            if (c != UTF16.charAt(text, 1) || i != 1)
                errln(
                        "moveCodePointIndex(1) didn't work correctly expected "
                                + hex(c)
                                + " got "
                                + hex(UTF16.charAt(text, 1))
                                + " i= "
                                + i);
            ...
            if (c != UTF16.charAt(text, 4) || i != 4)
                errln(
                        "moveCodePointIndex(2) didn't work correctly expected "
                                + hex(c)
                                + " got "
                                + hex(UTF16.charAt(text, 4))
                                + " i= "
                                + i);
            ...
            if (c != UTF16.charAt(text, (text.length() - 3)) || i != (text.length() - 3))
                errln(
                        "moveCodePointIndex(-2) didn't work correctly expected "
                                + hex(c)
                                + " got "
                                + hex(UTF16.charAt(text, (text.length() - 3)))
                                + " i= "
                                + i);
            ...
            if (c != UTF16.charAt(text, i))
                errln(
                        "first32PostInc failed.  Expected->"
                                + hex(UTF16.charAt(text, i))
                                + " Got-> "
                                + hex(c));
            ...
            iter.setToStart();
            i = 0;
            if (iter.getIndex() != 0) errln("setToStart failed");

            logln("Testing forward iteration...");
            do {
                if (c != UCharacterIterator.DONE) c = iter.nextCodePoint();
    ...
    public void TestPreviousNext() {
        ...
        // iterators
        UCharacterIterator iter1 =
                UCharacterIterator.getInstance(new ReplaceableString(new String(src)));
        UCharacterIterator iter2 = UCharacterIterator.getInstance(src /*char array*/);
        UCharacterIterator iter3 =
                UCharacterIterator.getInstance(new StringCharacterIterator(new String(src)));
        UCharacterIterator iter4 =
                UCharacterIterator.getInstance(new StringBuffer(new String(src)));
        previousNext(iter1);
        previousNext(iter2);
        previousNext(iter3);
        previousNext(iter4);
    ...
    public void previousNext(UCharacterIterator iter) {
        ...
        int expect[] = {0x2f999, 0x1d15f, 0xc4, 0x1ed0};

        // expected src indexes corresponding to expect indexes
        int expectIndex[] = {
            0, 0, 1, 1, 2, 3, 4 // needed
        };
    ...
    public void TestUCharacterIteratorWrapper() {
        ...
        if (ci.first() != wrap_ci.first()) {
            errln(
                    "CharacterIteratorWrapper.first() failed. expected: "
                            + ci.first()
                            + " got: "
                            + wrap_ci.first());
        }
        if (ci.last() != wrap_ci.last()) {
            errln(
                    "CharacterIteratorWrapper.last() failed expected: "
                            + ci.last()
                            + " got: "
                            + wrap_ci.last());
        }
        if (ci.getBeginIndex() != wrap_ci.getBeginIndex()) {
            errln(
                    "CharacterIteratorWrapper.getBeginIndex() failed expected: "
                            + ci.getBeginIndex()
                            + " got: "
                            + wrap_ci.getBeginIndex());
        }
        if (ci.getEndIndex() != wrap_ci.getEndIndex()) {
            errln(
                    "CharacterIteratorWrapper.getEndIndex() failed expected: "
                            + ci.getEndIndex()
                            + " got: "
                            + wrap_ci.getEndIndex());
        }
        try {
            ...
            if (wrap_ci.getIndex() != cloneWCI.getIndex()) {
                errln(
                        "CharacterIteratorWrapper.clone() failed expected: "
                                + wrap_ci.getIndex()
                                + " got: "
                                + cloneWCI.getIndex());
            }
    ...
    private static final String ITERATION_STRING_ = "Testing 1 2 3 \ud800\udc00 456";
