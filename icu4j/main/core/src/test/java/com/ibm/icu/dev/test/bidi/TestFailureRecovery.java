...
public class TestFailureRecovery extends BidiFmwk {
    ...
    public void testFailureRecovery() {
        ...
        try {
            ...
            logln(
                    "OK: Got exception for bidi.setPara(..., -1, ...)"
                            + " as expected: "
                            + e.getMessage());
        }
        try {
            ...
            logln("OK: Got exception for Bidi.writeReverse(null) as expected: " + e.getMessage());
        }
        bidi = new Bidi();
        try {
            bidi.setLine(0, 1);
            ...
            logln("OK: Got exception for Bidi.setLine(0, 1) as expected: " + e.getMessage());
        }
        try {
            bidi.getDirection();
            errln("bidi.getDirection did not fail when called before valid setPara()");
        } catch (IllegalStateException e) {
            logln("OK: Got exception for Bidi.getDirection() as expected: " + e.getMessage());
        }
        ...
        try {
            ...
            logln("OK: Got exception for Bidi.getLevelAt(3) as expected: " + e.getMessage());
        }
        try {
            bidi = new Bidi(-1, 0);
            errln("Bidi constructor did not fail when called with bad argument");
        } catch (IllegalArgumentException e) {
            logln("OK: Got exception for Bidi(-1,0) as expected: " + e.getMessage());
        }
        bidi = new Bidi(2, 1);
        try {
            bidi.setPara("abc", Bidi.LTR, null);
            ...
            logln("OK: Got exception for setPara(\"abc\") as expected: " + e.getMessage());
        }
        try {
            ...
            logln("OK: Got exception for countRuns as expected: " + e.getMessage());
        }
        ...
        try {
            ...
            logln("OK: Got exception for setPara(..., levels) as expected: " + e.getMessage());
        }
