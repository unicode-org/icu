...
public class Mf2IcuTest extends CoreTestFmwk {
    ...
    public void testStaticFormat() {
        ...
        assertEquals(
                ...
                MessageFormatter.builder()
                        .setPattern(
                                "At {$when :datetime timeStyle=medium} on {$when :datetime dateStyle=medium}, "
    ...
    public void testMessageFormatDateTimeOptions() {
        ...
        doTheRealDateTimeSkeletonTesting(
                date,
                "{$when :date month=long day=numeric}",
                Locale.forLanguageTag("en"),
                "November 23");
        doTheRealDateTimeSkeletonTesting(
                date,
                "{$when :datetime year=numeric month=long day=numeric hour=numeric minute=numeric}",
                Locale.forLanguageTag("en"),
                "November 23, 2021 at 4:42\u202FPM");
        doTheRealDateTimeSkeletonTesting(
                date,
                "{$when :date year=numeric month=short day=numeric}",
                Locale.forLanguageTag("en"),
                "Nov 23, 2021");
        doTheRealDateTimeSkeletonTesting(
                date,
                "{$when :datetime year=numeric month=long day=numeric}",
                Locale.forLanguageTag("fr"),
                "23 novembre 2021");
        doTheRealDateTimeSkeletonTesting(
                date,
                "Expiration: {$when :datetime year=numeric month=short}!",
                Locale.forLanguageTag("en"),
                "Expiration: Nov 2021!");
    }
