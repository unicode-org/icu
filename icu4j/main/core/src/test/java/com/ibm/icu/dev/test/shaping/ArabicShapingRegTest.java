...
public class ArabicShapingRegTest extends CoreTestFmwk {
  ...
  public void TestEquals() {
    ArabicShaping as1 =
        new ArabicShaping(LETTERS_SHAPE | TEXT_DIRECTION_VISUAL_LTR | LENGTH_FIXED_SPACES_NEAR);
    ArabicShaping as2 =
        new ArabicShaping(LETTERS_SHAPE | TEXT_DIRECTION_VISUAL_LTR | LENGTH_FIXED_SPACES_NEAR);
    ArabicShaping as3 =
        new ArabicShaping(
  ...
  public void TestShape() {
    ...
    for (int i = 0; i < negNum.length; i++) {
      try {
        ...
        errln(
            "ArabicShaping.shape(char[],int,int,char[],int,int) was "
    ...
    try {
      ...
      errln(
          "ArabicShaping.shape(char[],int,int,char[],int,int) was suppose to return an exception"
              + " when 'sourceStart + sourceLength > source.length'.");
    } catch (Exception e) {
    }
    try {
    ...
    // Checks when
    // if ((destSize != 0) && (destStart < 0 || destSize < 0 || destStart + destSize >
    // dest.length))
    for (int i = 0; i < negNum.length; i++) {
      try {
        ...
        errln(
            "ArabicShaping.shape(char[],int,int,char[],int,int) was suppose to return an exception"
                + " when (destSize != 0) && (destStart < 0 || destSize < 0 || destStart + destSize"
                + " > dest.length).");
      } catch (Exception e) {
      }
  ...
  private boolean getStaticCharacterHelperFunctionValue(String methodName, char testValue)
      throws Exception {
  ...
  /*
   * Regression test for ICU-23395: verify that expandCompositCharAtNear()
   * does not access out-of-bounds indices. The Java implementation iterates
   * backward and is not affected, but this test guards against regressions.
   * Input: N spaces followed by U+FEF5 (lam-alef ligature).
   */
  @Test
  public void TestExpandCompositCharAtNearBounds() {
    int[] lengths = {300, 301};
    ...
    for (int len : lengths) {
      ...
      java.util.Arrays.fill(source, 0, len - 1, (char) 0x0020);
      source[len - 1] = 0xFEF5;
      char[] dest = new char[len + 1];
      try {
        shaper.shape(source, 0, source.length, dest, 0, dest.length);
      } catch (ArabicShapingException e) {
        // Acceptable if expansion cannot find a free space slot.
      }
    }
  }
}
