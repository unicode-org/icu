...
public class CharsTrieTest extends CoreTestFmwk {
    ...
    public void Test10Empty() {
        final StringAndValue[] data = {new StringAndValue("", 0)};
        checkData(data);
    }

    @Test
    public void Test11_a() {
        final StringAndValue[] data = {new StringAndValue("a", 1)};
        checkData(data);
    }

    @Test
    public void Test12_a_ab() {
        final StringAndValue[] data = {new StringAndValue("a", 1), new StringAndValue("ab", 100)};
        checkData(data);
    }
    ...
    public void Test20ShortestBranch() {
        final StringAndValue[] data = {
            new StringAndValue("a", 1000), new StringAndValue("b", 2000)
        };
        checkData(data);
    }
    ...
    public void Test32NextForCodePoint() {
        ...
        if ((result = trie.nextForCodePoint(0x4dff)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x10000)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x9999)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x20000)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0xdfff)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x10ffff)) != BytesTrie.Result.FINAL_VALUE
                || result != trie.current()
                || trie.getValue() != 2000000000) {
            errln("CharsTrie.nextForCodePoint() fails for " + data[0].s);
        }
        if ((result = trie.firstForCodePoint(0x4dff)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x10000)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x9999)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x20002)) != BytesTrie.Result.FINAL_VALUE
                || result != trie.current()
                || trie.getValue() != 44444) {
            errln("CharsTrie.nextForCodePoint() fails for " + data[1].s);
        }
        if ((result = trie.reset().nextForCodePoint(0x4dff)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x10000)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x9999)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x20222)) != BytesTrie.Result.NO_MATCH
        ...
        if ((result = trie.reset().nextForCodePoint(0x4dff)) != BytesTrie.Result.NO_VALUE
                || result != trie.current()
                || (result = trie.nextForCodePoint(0x103ff)) != BytesTrie.Result.FINAL_VALUE
                || result != trie.current()
                || trie.getValue() != 99999) {
        ...
        while (gen.countUniqueFirstChars() < 1111) {
            ...
            if (x.length() == 0) {
                ...
                if (trie.first(x.charAt(0)) == BytesTrie.Result.NO_MATCH) {
                    errln(
                            String.format(
                                    "first(first char U+%04x)=BytesTrie.Result.NO_MATCH for string %d\n",
    ...
    public void Test51IteratorFromLinearMatch() {
        ...
        // Expected data: Same as in buildMonthsTrie(), except only the suffixes
        // following "janua".
        final StringAndValue[] data = {new StringAndValue("r", 1), new StringAndValue("ry", 1)};
        checkIterator(iter, data);
        // Reset, and we should get the same result.
        logln("after iter.reset()");
        checkIterator(iter.reset(), data);
    ...
    public void Test53TruncatingIteratorFromLinearMatchShort() {
        ...
        final StringAndValue[] expected = {new StringAndValue("cd", -1)};
    ...
    private void checkData(
            StringAndValue[] data, int dataLength, StringTrieBuilder.Option buildOption) {
    ...
    private void checkFirst(CharsTrie trie, StringAndValue[] data, int dataLength) {
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (firstResult != trie.reset().next(c)
                ...
                errln(
                        String.format(
                                "trie.first(U+%04X)!=trie.reset().next(same) for %s",
                                c, data[i].s));
            }
            ...
            if (firstResult != trie.reset().nextForCodePoint(c)
                ...
                errln(
                        String.format(
                                "trie.firstForCodePoint(U+%04X)!=trie.reset().nextForCodePoint(same) for %s",
                                c, data[i].s));
            }
        }
        trie.reset();
    ...
    private void checkNext(CharsTrie trie, StringAndValue[] data, int dataLength) {
        ...
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (!(result = trie.next(expectedString, 0, stringLength)).hasValue()
                ...
                errln(
                        String.format(
                                "trie value for %s is %d=0x%x instead of expected %d=0x%x",
                                data[i].s,
                                trie.getValue(),
                                trie.getValue(),
                                data[i].value,
                ...
                errln(
                        "trie value for "
                                + data[i].s
                                + " changes when repeating current()/getValue()");
            }
            trie.reset();
            result = trie.current();
            for (int j = 0; j < stringLength; ++j) {
                if (!result.hasNext()) {
                    errln(
                            String.format(
                                    "trie.current()!=hasNext before end of %s (at index %d)",
                                    data[i].s, j));
                    break;
                }
                if (result == BytesTrie.Result.INTERMEDIATE_VALUE) {
                    ...
                    if (trie.current() != BytesTrie.Result.INTERMEDIATE_VALUE) {
                        errln(
                                String.format(
                                        "trie.getValue().current()!=BytesTrie.Result.INTERMEDIATE_VALUE "
                                                + "before end of %s (at index %d)",
                                        data[i].s, j));
                        break;
                    }
                }
                ...
                if (!result.matches()) {
                    errln(
                            String.format(
                                    "trie.next()=BytesTrie.Result.NO_MATCH "
                                            + "before end of %s (at index %d)",
                                    data[i].s, j));
                    break;
                }
            ...
            if ((result == BytesTrie.Result.INTERMEDIATE_VALUE) != nextContinues) {
                errln(
                        "(trie.current()==BytesTrie.Result.INTERMEDIATE_VALUE) contradicts "
                                + "(trie.next(some char)!=BytesTrie.Result.NO_MATCH) after end of "
                                + data[i].s);
            }
            trie.reset();
        }
        ...
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (resultAtState != trie.resetToState(state).current()
                ...
                errln(
                        "trie.next(part of "
                                + data[i].s
                                + ") changes current()/getValue() after "
                        ...
                        "trie.next(rest of "
                                ...
                                + "saveState/next(0)/resetToState");
                                    ...
                                    trie.resetToState(state)
                                            .next(expectedString, partialLength, stringLength))
                            .hasValue()
                    || result != trie.current()) {
                errln(
                        "trie does not seem to contain "
                                + data[i].s
    ...
    private void checkNextString(CharsTrie trie, StringAndValue[] data, int dataLength) {
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (!trie.next(expectedString, 0, stringLength / 2).matches()) {
                errln(
                        "trie.next(up to middle of string)=BytesTrie.Result.NO_MATCH for "
                                + data[i].s);
                continue;
            }
    ...
    private void checkIterator(CharsTrie.Iterator iter, StringAndValue[] data, int dataLength) {
        for (int i = 0; i < dataLength; ++i) {
            ...
            if (!expectedString.contentEquals(entry.chars)) {
                errln(
                        String.format(
                                "trie iterator next().getString()=%s but expected %s for item %d",
                                entry.chars, data[i].s, i));
            }
            if (entry.value != data[i].value) {
                errln(
                        String.format(
                                "trie iterator next().getValue()=%d=0x%x but expected %d=0x%x for item %d: %s",
                                entry.value,
                                entry.value,
                                data[i].value,
                                data[i].value,
