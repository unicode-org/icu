...
/**
 * Test MessagePatternUtil (MessagePattern-as-tree-of-nodes API) by building parallel trees of nodes
 * and verifying that they match.
 */
...
public final class MessagePatternUtilTest extends CoreTestFmwk {
    ...
    private class ExpectMessageNode {
        ...
        private boolean matches(MessageNode msg) {
            ...
            boolean ok =
                    assertEquals(
                            "different numbers of MessageContentsNode",
                            contents.size(),
                            msgContents.size());
            if (ok) {
    ...
    private class ExpectMessageContentsNode {
        protected boolean matches(MessageContentsNode c) {
            return assertEquals(
                    ...
                    MessageContentsNode.Type.REPLACE_NUMBER,
                    c.getType());
        }
    }
    ...
    private class ExpectTextNode extends ExpectMessageContentsNode {
        ...
        @Override
        protected boolean matches(MessageContentsNode c) {
            return assertEquals("not a TextNode", MessageContentsNode.Type.TEXT, c.getType())
                    && assertTrue(
                            "TextNode does not contain \"" + subString + "\"",
    ...
    private class ExpectArgNode extends ExpectMessageContentsNode {
        ...
        @Override
        protected boolean matches(MessageContentsNode c) {
            boolean ok = assertEquals("not an ArgNode", MessageContentsNode.Type.ARG, c.getType());
            if (!ok) {
                return ok;
            }
            ArgNode arg = (ArgNode) c;
            ok &= assertEquals("unexpected ArgNode argType", argType, arg.getArgType());
            ok &= assertEquals("unexpected ArgNode arg name", name, arg.getName());
            ok &= assertEquals("unexpected ArgNode arg number", number, arg.getNumber());
            ok &= assertEquals("unexpected ArgNode arg type name", type, arg.getTypeName());
            ok &= assertEquals("unexpected ArgNode arg style", style, arg.getSimpleStyle());
    ...
    private class ExpectComplexArgNode extends ExpectArgNode {
        private ExpectComplexArgNode(
                ExpectMessageNode parent, Object name, MessagePattern.ArgType argType) {
        ...
        protected boolean matches(MessageContentsNode c) {
            ...
            ok &=
                    assertEquals(
                            "unexpected complex-style argType", argType, complexStyle.getArgType());
            ok &=
                    assertEquals(
                            "unexpected complex-style hasExplicitOffset()",
                            explicitOffset,
                            complexStyle.hasExplicitOffset());
            ok &= assertEquals("unexpected complex-style offset", offset, complexStyle.getOffset());
            List<VariantNode> complexVariants = complexStyle.getVariants();
            ok &=
                    assertEquals(
                            "different number of variants",
                            variants.size(),
                            complexVariants.size());
            if (!ok) {
                return ok;
            }
    ...
    private class ExpectVariantNode {
        ...
        private boolean matches(VariantNode v) {
            boolean ok = assertEquals("different selector strings", selector, v.getSelector());
            ok &=
                    assertEquals(
                            "different selector strings",
                            isSelectorNumeric(),
                            v.isSelectorNumeric());
            ok &= assertEquals("different selector strings", numericValue, v.getSelectorValue());
    ...
    public void TestNoneArg() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        .expectTextThatContains("abc")
                        .expectNoneArg(0)
                        .expectTextThatContains("def");
        expect.checkMatches(msg);
        // Named argument.
    ...
    public void TestSimpleArg() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        .expectTextThatContains("a{bc'def")
                        .expectSimpleArg(0, "number", "g'hi''jk'l#");
        expect.checkMatches(msg);
    }
    ...
    public void TestSelectArg() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        .expectTextThatContains("abc")
                        .expectSimpleArg(2, "number")
                        .expectTextThatContains("ghi")
                        .expectSelectArg(3)
                        .expectVariant("xx")
                        .expectTextThatContains("xxx")
                        .finishVariant()
                        .expectVariant("other")
                        .expectTextThatContains("ooo")
                        .finishVariant()
                        .finishComplexArg()
    ...
    public void TestPluralArg() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        ...
                        .expectOffset(17)
                        .expectVariant("few")
                        .expectTextThatContains("fff")
                        .finishVariant()
                        .expectVariant("other")
                        .expectTextThatContains("oooo")
                        .finishVariant()
                        .finishComplexArg()
                        .expectTextThatContains("xyz");
        expect.checkMatches(msg);
        ...
        expect =
                new ExpectMessageNode()
                        ...
                        .expectOffset(2)
                        .expectVariant("=1", 1)
                        .expectTextThatContains("1")
                        .finishVariant()
                        .expectVariant("=-1", -1)
                        .expectTextThatContains("-1")
                        .finishVariant()
                        .expectVariant("=3.14", 3.14)
                        .expectTextThatContains("3.14")
                        .finishVariant()
                        .expectVariant("other")
                        .expectTextThatContains("oo")
                        .finishVariant()
                        .finishComplexArg()
                        .expectTextThatContains("xyz");
        expect.checkMatches(msg);
        ...
        expect =
                new ExpectMessageNode()
                        ...
                        .expectPluralArg(0)
                        .expectVariant("other")
                        .expectTextThatContains("num=")
                        .expectReplaceNumber()
                        .expectTextThatContains("#=")
                        .expectReplaceNumber()
                        .expectTextThatContains("#=")
                        .expectSimpleArg(1, "number", "##")
                        .expectTextThatContains("!")
                        .finishVariant()
                        .finishComplexArg()
    ...
    public void TestChoiceArg() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        ...
                        .expectTextThatContains("-inf")
                        .finishVariant()
                        .expectVariant("≤", 5)
                        .expectTextThatContains(" five ")
                        .finishVariant()
                        .expectVariant("#", 99)
                        .expectTextThatContains(" ninety|nine  ")
                        .finishVariant()
                        .finishComplexArg()
                        .expectTextThatContains("_z");
        expect.checkMatches(msg);
    ...
    public void TestComplexArgs() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        ...
                        .expectSelectArg("b")
                        .expectVariant("other")
                        .expectTextThatContains("shan't}")
                        .finishVariant()
                        .finishComplexArg()
                        .expectTextThatContains(" {'know'} and ")
                        .expectChoiceArg("c")
                        .expectVariant("#", 0)
                        .expectTextThatContains("can't|")
                        .finishVariant()
                        .finishComplexArg()
    ...
    /**
     * @return the text string of the VariantNode's message; assumes that its message consists of
     *     only text
     */
    ...
    public void TestPluralVariantsByType() {
        ...
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        .expectPluralArg("p")
                        .expectVariant("a")
                        .expectTextThatContains("A")
                        .finishVariant()
                        .expectVariant("other")
                        .expectTextThatContains("O")
                        .finishVariant()
                        .expectVariant("=4", 4)
                        .expectTextThatContains("iv")
                        .finishVariant()
                        .expectVariant("b")
                        .expectTextThatContains("B")
                        .finishVariant()
                        .expectVariant("other")
                        .expectTextThatContains("U")
                        .finishVariant()
                        .expectVariant("=2", 2)
                        .expectTextThatContains("ii")
                        .finishVariant()
                        .finishComplexArg();
    ...
    public void TestSelectVariantsByType() {
        MessageNode msg =
                MessagePatternUtil.buildMessageNode("{s,select,a{A}other{O}b{B}other{U}}");
        ExpectMessageNode expect =
                new ExpectMessageNode()
                        .expectSelectArg("s")
                        .expectVariant("a")
