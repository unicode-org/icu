...
public class UPropertyAliasesTest extends CoreTestFmwk {
    ...
    public void TestPropertyNames() {
        ...
        int i = UCharacter.getIntPropertyMinValue(UProperty.CANONICAL_COMBINING_CLASS);
        try {
            for (;
                    i <= UCharacter.getIntPropertyMaxValue(UProperty.CANONICAL_COMBINING_CLASS);
                    i++) {
                UCharacter.getPropertyValueName(
                        UProperty.CANONICAL_COMBINING_CLASS, i, UProperty.NameChoice.LONG);
            }
        } catch (IllegalArgumentException e) {
            errln("0x" + Integer.toHexString(i) + " should have a null property value name");
        }
    }
    ...
    public void TestUnknownPropertyNames() {
        try {
            ...
            errln(
                    "UCharacter.getPropertyEnum(??) returned "
                            + p
        ...
        try {
            ...
            errln(
                    "UCharacter.getPropertyValueEnum(UProperty.LINE_BREAK, ?!) returned "
                            + p
                            + " rather than throwing an exception");
        } catch (IllegalArgumentException e) {
            // ok
