...
public class FormattedValueTest {
    ...
    public void testBasic() {
        ...
        assertAllPartsEqual("basic", cfpos, 15, null, null, 0, 0, 0L);
    }
    ...
    public void testSetters() {
        ...
        assertAllPartsEqual("setters 1", cfpos, 10, NumberFormat.Field.COMPACT, null, 0, 0, 0L);

        cfpos.constrainFieldAndValue(NumberFormat.Field.COMPACT, 42);
        assertAllPartsEqual("setters 1.2", cfpos, 8, NumberFormat.Field.COMPACT, 42, 0, 0, 0L);

        cfpos.constrainClass(NumberFormat.Field.class);
        assertAllPartsEqual("setters 1.5", cfpos, 11, null, null, 0, 0, 0L);

        cfpos.setInt64IterationContext(42424242424242L);
        assertAllPartsEqual("setters 2", cfpos, 11, null, null, 0, 0, 42424242424242L);
        ...
        cfpos.reset();
        assertAllPartsEqual("setters 4", cfpos, 15, null, null, 0, 0, 0L);
    }
    ...
    private void assertAllPartsEqual(
        ...
        assertEquals(
                messagePrefix + ": integer field",
                ((matching & 1) != 0),
                cfpos.matchesField(NumberFormat.Field.INTEGER, null));
        assertEquals(
                messagePrefix + ": compact field",
                ((matching & 2) != 0),
                cfpos.matchesField(NumberFormat.Field.COMPACT, null));
        assertEquals(
                messagePrefix + ": date field",
                ((matching & 4) != 0),
                cfpos.matchesField(DateFormat.Field.AM_PM, null));
        assertEquals(
                messagePrefix + ": compact field with value",
                ((matching & 8) != 0),
                cfpos.matchesField(NumberFormat.Field.COMPACT, 42));
    }

    public static void checkFormattedValue(
            String message,
            FormattedValue fv,
    ...
    public static void checkFormattedValue(
            ...
            String expectedString,
            Object[][] expectedFieldPositions,
            boolean skipAttributedCharacterIterator) {
        ...
        assertEquals(
                baseMessage + "All known fields should be in the iterator",
                uniqueFields.size(),
                allAttributes.size());
        assertEquals(
                baseMessage + "Iterator should have length of string output",
                stringLength,
                fpi.getEndIndex());
        int i = 0;
        for (char c = fpi.first(); c != AttributedCharacterIterator.DONE; c = fpi.next(), i++) {
            // Strings with adjacent fields cannot be disambiguated using
            ...
            Set<AttributedCharacterIterator.Attribute> currentAttributes =
                    fpi.getAttributes().keySet();
            ...
            for (Object[] cas : expectedFieldPositions) {
                ...
                assertTrue(
                        baseMessage + "A Character at " + i + " should have field " + expectedField,
                        currentAttributes.contains(expectedField));
                assertTrue(
                        baseMessage + "A Field " + expectedField + " should be a known attribute",
                ...
                assertEquals(
                        baseMessage + expectedField + " A begin @" + i,
                        expectedBeginIndex,
                        actualBeginIndex);
                assertEquals(
                        baseMessage + expectedField + " A end @" + i,
                        expectedEndIndex,
                        actualEndIndex);
                assertEquals(
                        baseMessage + expectedField + " A value @" + i, expectedValue, actualValue);
                attributesRemaining--;
            }
            assertEquals(
                    baseMessage
                            + "A Should have looked at every field: "
                            + i
                            + ": "
                            + currentAttributes,
                    0,
