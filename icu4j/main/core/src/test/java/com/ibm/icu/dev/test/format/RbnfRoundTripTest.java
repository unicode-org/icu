...
public class RbnfRoundTripTest extends CoreTestFmwk {
    ...
    public void TestSwissFrenchSpelloutRT() {
        RuleBasedNumberFormat formatter =
                new RuleBasedNumberFormat(
                        new Locale("fr", "CH", ""), RuleBasedNumberFormat.SPELLOUT);
    ...
    public void TestSwedishSpelloutRT() {
        RuleBasedNumberFormat formatter =
                new RuleBasedNumberFormat(
                        new Locale("sv", "SE", ""), RuleBasedNumberFormat.SPELLOUT);
    ...
    public void TestDutchSpelloutRT() {
        RuleBasedNumberFormat formatter =
                new RuleBasedNumberFormat(
                        new Locale("nl", "NL", ""), RuleBasedNumberFormat.SPELLOUT);
    ...
    public void TestGreekSpelloutRT() {
        RuleBasedNumberFormat formatter =
                new RuleBasedNumberFormat(
                        new Locale("el", "GR", ""), RuleBasedNumberFormat.SPELLOUT);
    ...
    public void TestHebrewNumberingRT() {
        RuleBasedNumberFormat formatter =
                new RuleBasedNumberFormat(
                        new Locale("he", "IL", ""), RuleBasedNumberFormat.NUMBERING_SYSTEM);

        formatter.setDefaultRuleSet("%hebrew");
        doTest(formatter, 0, 12345678);
    }

    void doTest(RuleBasedNumberFormat formatter, long lowLimit, long highLimit) {
        try {
            long count = 0;
            long increment = 1;
            for (long i = lowLimit; i <= highLimit; i += increment) {
                if (count % 1000 == 0) logln(Long.toString(i));

                if (Math.abs(i) < 5000) increment = 1;
                else if (Math.abs(i) < 500000) increment = 2737;
                else increment = 267437;
