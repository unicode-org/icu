...
public class JapaneseTest extends CalendarTestFmwk {
    ...
    public void TestCoverage() {
...

        {
            ...
            JapaneseCalendar cal =
                    new JapaneseCalendar(TimeZone.getDefault(), ULocale.getDefault());
            if (cal == null) {
...

        {
            ...
            JapaneseCalendar cal =
                    new JapaneseCalendar(JapaneseCalendar.MEIJI, 43, Calendar.JANUARY, 1);
            if (cal == null) {
...

        {
            ...
            DateFormat fmt =
                    cal.getDateTimeFormat(DateFormat.FULL, DateFormat.FULL, Locale.ENGLISH);
    ...
    public void Test3860() {
        ...
        final String jCalGannenDate =
                "1/5/9"; // A date in the above format after the accession date for Heisei era
        // (Heisei year 1 Jan 8)
        ...
        DateFormat enjformat =
                cal.getDateTimeFormat(
                        DateFormat.FULL, DateFormat.FULL, new ULocale("en_JP@calendar=japanese"));
        DateFormat format =
                cal.getDateTimeFormat(
                        DateFormat.SHORT,
                        DateFormat.SHORT,
                        loc); // SHORT => no jpanyear since we will apply a short pattern
        ...
        SimpleDateFormat fmt2 =
                new SimpleDateFormat(
                        "HH:mm:ss.S MMMM d, yyyy G", new ULocale("en_US@calendar=gregorian"));
        cal2.clear();
        String samplestr = "1";
        ...
        if ((gotYear != 1) || (gotEra != expectEra)) {
            errln(
                    "parse "
                            + samplestr
                            + " of 'y' as Japanese Calendar, expected year "
                            + expectYear
                            + " and era "
                            + expectEra
                            + ", but got year "
        ...
        if (aString.charAt(2) != '\u5143') { // 元
            errln(
                    "Formatting year 1 as Gannen, got "
                            + aString
                            + " but expected 3rd char to be \u5143");
        } else {
            ...
            try {
                ...
                if (!bString.equals(aString)) {
                    errln(
                            "Parsing 1 when expecting \u5143, formatting the result produced "
                                    + bString
                                    + " but expected "
                                    + aString);
                }
    ...
    public void TestForceGannenNumbering() {
        //        final String jCalShortPattern = "y/M/d"; // Note: just 'y' doesn't work here.
        //        final String jCalGannenDate = "1/5/9"; // A date in the above format after the
        // accession date for Heisei [1989-] era (Heisei year 1 Jan 8)
        //                                               // or Reiwa [2019-] era (Reiwa year 1 May
        // 1). If before the accession date,
        //                                               // the year will be in the previous era.
        ULocale loc = new ULocale("ja_JP@calendar=japanese");
        ...
        if (testString1.length() < 3 || testString1.charAt(2) != '\u5143') { // 元
            errln(
                    "Formatting year 1 in created text style, got "
                            + testString1
                            + " but expected 3rd char to be \u5143");
        }
        ...
        if (testString2.length() < 2 || testString2.charAt(1) != '1') {
            errln(
                    "Formatting year 1 in created numeric style, got "
                            + testString2
        ...
        if (testString1.length() < 2 || testString1.charAt(1) != '1') { //
            errln(
                    "Formatting year 1 in applied numeric style, got "
                            + testString1
                            + " but expected 2nd char to be 1");
        }
        ...
        if (testString2.length() < 3 || testString2.charAt(2) != '\u5143') { // 元
            errln(
                    "Formatting year 1 in applied text style, got "
                            + testString2
                            + " but expected 3rd char to be \u5143");
        }
        ...
        SimpleDateFormat testFmt3 =
                new SimpleDateFormat(
                        pattern, "", loc); // empty override string to disable Gannen year numbering
        String testString3 = testFmt3.format(refDate);
        if (testString3.length() < 3 || testString3.charAt(2) != '1') {
            errln(
                    "Formatting year 1 with Gannen disabled, got "
                            + testString3
    ...
    public void Test5345parse() {
        ...
        SimpleDateFormat fmt =
                (SimpleDateFormat)
                        c.getDateTimeFormat(1, 1, new ULocale("en_US@calendar=japanese"));
        ...
        // ULocale("en_US@calendar=japanese"));
        ...
        SimpleDateFormat jdf =
                (SimpleDateFormat)
                        SimpleDateFormat.getDateInstance(
                                jcal, SimpleDateFormat.DEFAULT, Locale.getDefault());
    ...
    public void TestJpnCalAddSetNextEra() {
        ...
        final int[] startYears = {
            1926, 1989, 2019, 0
        }; // start years for Show, Heisei, Reiwa; 0 marks invalid era beyond
        ...
        for (int iEra = 1; iEra < 3; iEra++) {
            ...
            if (eYear != startYears[iEra]) {
                ...
                int nextEraStart =
                        (startYears[iEra + 1] == 0) ? startYears[iEra] : startYears[iEra + 1];
                if (eYear != nextEraStart) {
                    errln(
                            "ERROR: set "
                                    + iEra
                                    + " then add ERA 1, expected start year "
                                    + nextEraStart
                                    + " but get "
                                    + eYear);
                }
