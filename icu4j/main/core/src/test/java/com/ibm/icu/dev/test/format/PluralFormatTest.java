...
public class PluralFormatTest extends CoreTestFmwk {
    private void helperTestRules(
            String localeIDs, String testPattern, Map<Integer, String> changes) {
        ...
        for (int i = 0; i < locales.length; ++i) {
            try {
                ...
                for (int n = 0; n < 200; ++n) {
                    ...
                    assertEquals(
                            "Locale: " + locales[i] + ", number: " + n, expected, plf.format(n));
                }
    ...
    public void TestSingular1Locales() {
        String localeIDs =
                "bem,da,de,el,en,eo,es,et,fi,fo,he,it,mr,nb,nl,nn,no,pt_PT,sv,af,bg,ca,eu,fur,fy,ha,ku,lb,ml,"
    ...
    public void TestGetPluralRules() {
        ...
        try {
            ...
            errln(
                    "CurrencyPluralInfo.getPluralRules() was not suppose to "
                            + "return an exception.");
        }
    }
    ...
    public void TestGetLocale() {
        ...
        if (!cpi.getLocale().equals(new ULocale("en_US"))) {
            errln(
                    "CurrencyPluralInfo.getLocale() was suppose to return true "
                            + "when passing the same ULocale");
        }
        if (cpi.getLocale().equals(new ULocale("jp_JP"))) {
            errln(
                    "CurrencyPluralInfo.getLocale() was not suppose to return true "
                            + "when passing a different ULocale");
        }
    }
    ...
    public void TestSetLocale() {
        ...
        if (!cpi.getLocale().equals(new ULocale("en_US"))) {
            errln(
                    "CurrencyPluralInfo.setLocale() was suppose to return true when passing the same ULocale");
        }
        if (cpi.getLocale().equals(new ULocale("jp_JP"))) {
            errln(
                    "CurrencyPluralInfo.setLocale() was not suppose to return true when passing a different ULocale");
        }
    }
    ...
    public void TestEquals() {
        ...
        if (cpi.equals(0)) {
            errln(
                    "CurrencyPluralInfo.equals(Object) was not suppose to return true when comparing to an invalid object for integer 0.");
        }
        if (cpi.equals(0.0)) {
            errln(
                    "CurrencyPluralInfo.equals(Object) was not suppose to return true when comparing to an invalid object for float 0.");
        }
        if (cpi.equals("0")) {
            errln(
                    "CurrencyPluralInfo.equals(Object) was not suppose to return true when comparing to an invalid object for string 0.");
        }
    }
