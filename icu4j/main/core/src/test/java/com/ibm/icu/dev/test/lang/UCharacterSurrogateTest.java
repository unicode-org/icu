...
/**
 * ****************************************************************************** Copyright (C)
 * 2004-2008, International Business Machines Corporation and * others. All Rights Reserved. *
 * ******************************************************************************
 */
...
public final class UCharacterSurrogateTest extends CoreTestFmwk {
    ...
    public void TestAllCodePoints() {
        ...
        assertEquals(
                ...
                0x110000,
                UCharacter.allCodePointsStream()
                        .filter(UCharacter::isValidCodePoint)
                        .distinct()
                        .count());
    }
    ...
    public void TestAllScalarValues() {
        ...
        assertEquals(
                "filtered stream count",
                0x110000 - 0x800,
                UCharacter.allScalarValuesStream()
                        .filter(UCharacter::isScalarValue)
                        .distinct()
                        .count());
    }
    ...
    public void TestIsSupplementaryCodePoint() {
        ...
        if (UCharacter.isSupplementaryCodePoint(UCharacter.MIN_SUPPLEMENTARY_CODE_POINT - 1))
            errln("0xffff");
        if (!UCharacter.isSupplementaryCodePoint(UCharacter.MIN_SUPPLEMENTARY_CODE_POINT))
            errln("0x10000");
    ...
    public void TestIsSurrogatePair() {
        ...
        if (UCharacter.isSurrogatePair(
            ...
            errln("0xd800,0xe000");
        if (!UCharacter.isSurrogatePair(
                UCharacter.MIN_HIGH_SURROGATE, UCharacter.MIN_LOW_SURROGATE))
            errln("0xd800,0xdc00");
    }
    ...
    public void TestToCodePoint() {
        final char[] pairs = {
            (char) (UCharacter.MIN_HIGH_SURROGATE + 0),
    ...
    public void TestCodePointAtBefore() {
        String s =
                ""
                        ...
                        + UCharacter.MIN_LOW_SURROGATE; // isolated
        // low
    ...
    public void TestToChars() {
        ...
        if (chars[1] != UCharacter.MIN_HIGH_SURROGATE || chars[2] != UCharacter.MIN_LOW_SURROGATE) {

            errln("fail");
        }

        chars = UCharacter.toChars(cp);
        if (chars[0] != UCharacter.MIN_HIGH_SURROGATE || chars[1] != UCharacter.MIN_LOW_SURROGATE) {
        ...
        class Test {
            ...
            void fail(String s, int start, int limit, Class<? extends RuntimeException> exc) {
                try {
                    ...
                    if (!exc.isInstance(e)) {
                        warnln("bad exception " + str(s, start, limit) + e.getClass().getName());
                    }
                }
            }
        }
        ...
        class Test {
            ...
            void test(
                ...
                if (val1 != expected) {
                    errln(
                            "char[] "
                                    + str(s, start, count, index, offset)
                                    + "("
                                    + val1
                                    + ") != "
                                    + expected);
