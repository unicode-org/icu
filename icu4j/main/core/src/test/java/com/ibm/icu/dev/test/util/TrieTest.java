...
public final class TrieTest extends CoreTestFmwk {
    ...
    /**
     * Values for testing: value is set from the previous boundary's limit to before this boundary's
     * limit
     */
    ...
    private static final class _testFoldedValue implements TrieBuilder.DataManipulate {
    ...
    private static final class _testFoldingOffset implements Trie.DataManipulate {
        @Override
    ...
    private void _testTrieRanges(
        ...
        IntTrieBuilder newTrie =
                new IntTrieBuilder(
                        null, 2000, checkRanges[0].value, checkRanges[0].value, latin1Linear);
    ...
    public void TestDummyCharTrie() {
        ...
        for (c = 0; c <= 0x10ffff; ++c) {
            ...
            if (value != initialValue) {
                errln(
                        "CharTrie/dummy.getCodePointValue(c)(U+"
                                + hex(c)
                                + ")=0x"
                                + hex(value)
                                + " instead of 0x"
        ...
        for (c = 0xd800; c <= 0xdbff; ++c) {
            ...
            if (value != leadUnitValue) {
                errln(
                        "CharTrie/dummy.getLeadValue(c)(U+"
                                + hex(c)
                                + ")=0x"
                                + hex(value)
                                + " instead of 0x"
