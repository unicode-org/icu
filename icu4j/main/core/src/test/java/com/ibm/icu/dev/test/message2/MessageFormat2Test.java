...
public class MessageFormat2Test extends CoreTestFmwk {
    ...
    public void testDateFormat() {
        ...
        mf2 =
                MessageFormatter.builder()
                        .setPattern(
                                "Your card expires on {$exp :datetime year=numeric month=short day=numeric weekday=short}!")
                        .build();
        assertEquals(
                "date format",
                "Your card expires on Thu, Oct 27, 2022!",
    ...
    static class TemperatureFormatterFactory implements FormatterFactory {
        ...
        private static class TemperatureFormatterImpl implements Formatter {
            ...
            private final Map<String, LocalizedNumberFormatter> cachedFormatters = new HashMap<>();

            TemperatureFormatterImpl(
                    LocalizedNumberFormatter nf, TemperatureFormatterFactory formatterFactory) {
                this.nf = nf;
    ...
    public void testFormatterIsCreatedOnce() {
        ...
        MFFunctionRegistry registry =
                MFFunctionRegistry.builder().setFormatter("temp", counter).build();
