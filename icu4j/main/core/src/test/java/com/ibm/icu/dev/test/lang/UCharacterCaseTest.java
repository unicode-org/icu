...
/**
 * ****************************************************************************** Copyright (C)
 * 1996-2014, International Business Machines Corporation and others. All Rights Reserved.
 * ******************************************************************************
 */
 ...
 *
 * <p>Mostly following the test cases in strcase.cpp for ICU
 *
 * @author Syn Wee Quek
 * @since march 14 2002
 */
...
public final class UCharacterCaseTest extends CoreTestFmwk {
    ...
    public void TestCharacter() {
        for (int i = 0; i < CHARACTER_LOWER_.length; i++) {
            ...
            if (CHARACTER_LOWER_[i] != UCharacter.toLowerCase(CHARACTER_UPPER_[i])
                    || (CHARACTER_UPPER_[i] != UCharacter.toUpperCase(CHARACTER_LOWER_[i])
                            && CHARACTER_UPPER_[i]
                                    != UCharacter.toTitleCase(CHARACTER_LOWER_[i]))) {
                errln(
                        "FAIL case conversion test for \\u"
            ...
            if (CHARACTER_UPPER_[i] != UCharacter.toUpperCase(CHARACTER_UPPER_[i])
                    && CHARACTER_UPPER_[i] != UCharacter.toTitleCase(CHARACTER_UPPER_[i])) {
    ...
    public void TestFolding() {
        ...
        for (int i = 0; i < FOLDING_SIMPLE_.length; i += 3) {
            if (UCharacter.foldCase(FOLDING_SIMPLE_[i], true) != FOLDING_SIMPLE_[i + 1]) {
                errln(
                        "FAIL: foldCase(\\u"
                                + hex(FOLDING_SIMPLE_[i])
                                + ", true) should be \\u"
                                + hex(FOLDING_SIMPLE_[i + 1]));
            }
            if (UCharacter.foldCase(FOLDING_SIMPLE_[i], UCharacter.FOLD_CASE_DEFAULT)
                    != FOLDING_SIMPLE_[i + 1]) {
                errln(
                        "FAIL: foldCase(\\u"
                                + hex(FOLDING_SIMPLE_[i])
            ...
            if (UCharacter.foldCase(FOLDING_SIMPLE_[i], false) != FOLDING_SIMPLE_[i + 2]) {
                errln(
                        "FAIL: foldCase(\\u"
                                + hex(FOLDING_SIMPLE_[i])
                                + ", false) should be \\u"
            ...
            if (UCharacter.foldCase(FOLDING_SIMPLE_[i], UCharacter.FOLD_CASE_EXCLUDE_SPECIAL_I)
                    != FOLDING_SIMPLE_[i + 2]) {
                errln(
                        "FAIL: foldCase(\\u"
                                + hex(FOLDING_SIMPLE_[i])
        ...
        if (!FOLDING_DEFAULT_[0].equals(UCharacter.foldCase(FOLDING_MIXED_[0], true))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[0])
                            + ", true)="
                            + prettify(UCharacter.foldCase(FOLDING_MIXED_[0], true))
                            + " should be "
                            + prettify(FOLDING_DEFAULT_[0]));
        }

        if (!FOLDING_DEFAULT_[0].equals(
                UCharacter.foldCase(FOLDING_MIXED_[0], UCharacter.FOLD_CASE_DEFAULT))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[0])
                            ...
                            + prettify(
                                    UCharacter.foldCase(
                                            FOLDING_MIXED_[0], UCharacter.FOLD_CASE_DEFAULT))
                            + " should be "
                            + prettify(FOLDING_DEFAULT_[0]));
        }

        if (!FOLDING_EXCLUDE_SPECIAL_I_[0].equals(UCharacter.foldCase(FOLDING_MIXED_[0], false))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[0])
                            + ", false)="
                            + prettify(UCharacter.foldCase(FOLDING_MIXED_[0], false))
                            + " should be "
                            + prettify(FOLDING_EXCLUDE_SPECIAL_I_[0]));
        }
        ...
        if (!FOLDING_DEFAULT_[1].equals(UCharacter.foldCase(FOLDING_MIXED_[1], true))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[1])
                            + ", true)="
                            + prettify(UCharacter.foldCase(FOLDING_MIXED_[1], true))
                            + " should be "
                            + prettify(FOLDING_DEFAULT_[1]));
        }

        if (!FOLDING_DEFAULT_[1].equals(
                UCharacter.foldCase(FOLDING_MIXED_[1], UCharacter.FOLD_CASE_DEFAULT))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[1])
                            + ", UCharacter.FOLD_CASE_DEFAULT)="
                            + prettify(
                                    UCharacter.foldCase(
                                            FOLDING_MIXED_[1], UCharacter.FOLD_CASE_DEFAULT))
                            + " should be "
                            + prettify(FOLDING_DEFAULT_[1]));
        }

        // alternate handling for dotted I/dotless i (U+0130, U+0131)
        if (!FOLDING_EXCLUDE_SPECIAL_I_[1].equals(UCharacter.foldCase(FOLDING_MIXED_[1], false))) {
            errln(
                    "FAIL: foldCase("
                            + prettify(FOLDING_MIXED_[1])
                            + ", false)="
                            + prettify(UCharacter.foldCase(FOLDING_MIXED_[1], false))
                            + " should be "
    ...
    public void TestInvalidCodePointFolding() {
        ...
        for (int cp : invalidCodePoints) {
            assertEquals(
                    "Invalid code points should be echoed back", cp, UCharacter.foldCase(cp, true));
            assertEquals(
                    "Invalid code points should be echoed back",
                    cp,
                    UCharacter.foldCase(cp, false));
            assertEquals(
                    "Invalid code points should be echoed back",
                    cp,
                    UCharacter.foldCase(cp, UCharacter.FOLD_CASE_DEFAULT));
            assertEquals(
                    "Invalid code points should be echoed back",
                    cp,
                    UCharacter.foldCase(cp, UCharacter.FOLD_CASE_EXCLUDE_SPECIAL_I));
        }
    }
    ...
    public void TestUpper() {
        ...
        if (!UPPER_TURKISH_.equals(UCharacter.toUpperCase(TURKISH_LOCALE_, UPPER_BEFORE_))) {
            errln(
                    "Fail "
                            + UPPER_BEFORE_
        ...
        if (!SHARED_UPPERCASE_TOPKAP_.equals(UCharacter.toUpperCase(SHARED_LOWERCASE_TOPKAP_))) {
            errln(
                    "toUpper failed: expected \""
        ...
        if (!SHARED_UPPERCASE_TURKISH_.equals(
                UCharacter.toUpperCase(TURKISH_LOCALE_, SHARED_LOWERCASE_TOPKAP_))) {
            errln(
                    "toUpper failed: expected \""
                            + SHARED_UPPERCASE_TURKISH_
                            + "\", got \""
                            + UCharacter.toUpperCase(TURKISH_LOCALE_, SHARED_LOWERCASE_TOPKAP_)
                            + "\".");
        }
        ...
        if (!SHARED_UPPERCASE_GERMAN_.equals(
                UCharacter.toUpperCase(GERMAN_LOCALE_, SHARED_LOWERCASE_GERMAN_))) {
            errln(
                    "toUpper failed: expected \""
                            + SHARED_UPPERCASE_GERMAN_
                            + "\", got \""
                            + UCharacter.toUpperCase(GERMAN_LOCALE_, SHARED_LOWERCASE_GERMAN_)
                            + "\".");
        }

        if (!SHARED_UPPERCASE_GREEK_.equals(UCharacter.toUpperCase(SHARED_LOWERCASE_GREEK_))) {
            errln(
                    "toLower failed: expected \""
    ...
    public void TestLower() {
        ...
        if (!LOWER_TURKISH_.equals(UCharacter.toLowerCase(TURKISH_LOCALE_, LOWER_BEFORE_))) {
            errln(
                    "Fail "
                            + LOWER_BEFORE_
        ...
        if (!SHARED_LOWERCASE_ISTANBUL_.equals(
            ...
            errln(
                    "1. toLower failed: expected \""
                            + SHARED_LOWERCASE_ISTANBUL_
                            + "\", got \""
                            + UCharacter.toLowerCase(SHARED_UPPERCASE_ISTANBUL_)
                            + "\".");
        }
        ...
        if (!SHARED_LOWERCASE_TURKISH_.equals(
                UCharacter.toLowerCase(TURKISH_LOCALE_, SHARED_UPPERCASE_ISTANBUL_))) {
            errln(
                    "2. toLower failed: expected \""
                            + SHARED_LOWERCASE_TURKISH_
                            + "\", got \""
                            + UCharacter.toLowerCase(TURKISH_LOCALE_, SHARED_UPPERCASE_ISTANBUL_)
                            + "\".");
        }
        if (!SHARED_LOWERCASE_GREEK_.equals(
                UCharacter.toLowerCase(GREEK_LOCALE_, SHARED_UPPERCASE_GREEK_))) {
            errln(
                    "toLower failed: expected \""
            ...
            for (int i = 0; i < TITLE_DATA_.length; ) {
                ...
                BreakIterator iter =
                        breakType >= 0
                                ...
                                : breakType == -2
                                        ?
                                        // Open a trivial break iterator that only delivers { 0,
                                        // length }
    ...
    public void TestTitleOptions() {
        ...
        TestCasingImpl(
                "«丰(aBc)»", "«丰(Abc)»", CaseMap.toTitle().wholeString().adjustToCased(), root);
        TestCasingImpl(
                " john. Smith",
                " John. Smith",
        ...
        TestCasingImpl(
                " john. Smith",
                " john. smith",
                CaseMap.toTitle().wholeString().noBreakAdjustment(),
                root);
        TestCasingImpl("«ijs»", "«IJs»", CaseMap.toTitle().wholeString(), new Locale("nl", "BE"));
        TestCasingImpl("«ijs»", "«İjs»", CaseMap.toTitle().wholeString(), new Locale("tr", "DE"));
        ...
        try {
            ...
            fail(
                    "CaseMap.toTitle(multiple adjustment options) "
        ...
        try {
            CaseMap.toTitle()
                    ...
                    .apply(root, null, "", new StringBuilder(), null);
            fail(
                    "CaseMap.toTitle(multiple iterator options) "
                            + "did not throw an IllegalArgumentException");
        } catch (IllegalArgumentException expected) {
        }
        ...
        try {
            ...
            fail(
                    "CaseMap.toTitle(iterator option + iterator) "
                            + "did not throw an IllegalArgumentException");
        } catch (IllegalArgumentException expected) {
        }
    }
    ...
    public void TestLithuanianTitle() {
        ...
        assertEquals(
                "Lithuanian titlecase check in Lithuanian",
                "\u0058\u0069\u0307\u0308",
    ...
    public void TestDutchTitle() {
        ...
        assertEquals(
                "Dutch titlecase check in English (Java Locale)",
                "Ijssel Igloo Ijmuiden",
                UCharacter.toTitleCase(Locale.ENGLISH, "ijssel igloo IJMUIDEN", null));

        assertEquals(
                "Dutch titlecase check in Dutch (Java Locale)",
                "IJssel Igloo IJmuiden",
        ...
        assertEquals(
                ...
                UCharacter.toTitleCase(
                        LOC_DUTCH, "ijssel igloo IjMUIdEN iPoD ijenough", iter, options));
        ...
        for (String[] caseDatum : dutchIJCasesData) {
            ...
            for (boolean isOnlyChanged : Arrays.asList(true, false)) {
                String testMsg = "Dutch accented ij" + (isOnlyChanged ? ", only changes" : "");
                ...
                CaseMap.Title titleCaseMap =
                        isOnlyChanged ? titleCaseMapBase.omitUnchangedText() : titleCaseMapBase;
    ...
    public void TestSpecial() {
        for (int i = 0; i < SPECIAL_LOCALES_.length; i++) {
            ...
            if (locale != null) {
                if (!SPECIAL_DATA_[j + 1].equals(UCharacter.toLowerCase(locale, str))) {
                    errln(
                            "error lowercasing special characters "
                                    + hex(str)
                                    + " expected "
                                    + hex(SPECIAL_DATA_[j + 1])
                                    + " for locale "
                                    + locale.toString()
                                    + " but got "
                                    + hex(UCharacter.toLowerCase(locale, str)));
                }
                if (!SPECIAL_DATA_[j + 2].equals(UCharacter.toUpperCase(locale, str))) {
                    errln(
                            "error uppercasing special characters "
                                    + hex(str)
                                    + " expected "
                                    + SPECIAL_DATA_[j + 2]
                                    + " for locale "
        ...
        // more accents above
        if (!SPECIAL_DOTTED_LOWER_TURKISH_.equals(
                UCharacter.toLowerCase(TURKISH_LOCALE_, SPECIAL_DOTTED_))) {
            errln(
                    "error in dots.toLower(tr)=\""
                            + SPECIAL_DOTTED_
                            + "\" expected \""
                            + SPECIAL_DOTTED_LOWER_TURKISH_
                            + "\" but got "
                            + UCharacter.toLowerCase(TURKISH_LOCALE_, SPECIAL_DOTTED_));
        }
        if (!SPECIAL_DOTTED_LOWER_GERMAN_.equals(
                UCharacter.toLowerCase(GERMAN_LOCALE_, SPECIAL_DOTTED_))) {
            errln(
                    "error in dots.toLower(de)=\""
                            + SPECIAL_DOTTED_
                            + "\" expected \""
                            + SPECIAL_DOTTED_LOWER_GERMAN_
                            + "\" but got "
                            + UCharacter.toLowerCase(GERMAN_LOCALE_, SPECIAL_DOTTED_));
        }
        ...
        if (!SPECIAL_DOT_ABOVE_UPPER_GERMAN_.equals(
                UCharacter.toUpperCase(GERMAN_LOCALE_, SPECIAL_DOT_ABOVE_))) {
            errln(
                    "error in dots.toUpper(de)=\""
                            + SPECIAL_DOT_ABOVE_
                            + "\" expected \""
        ...
        if (!SPECIAL_DOT_ABOVE_LOWER_LITHUANIAN_.equals(
                UCharacter.toLowerCase(LITHUANIAN_LOCALE_, SPECIAL_DOT_ABOVE_UPPER_))) {
            errln(
                    "error in dots.toLower(lt)=\""
                            + SPECIAL_DOT_ABOVE_UPPER_
                            + "\" expected \""
                            + SPECIAL_DOT_ABOVE_LOWER_LITHUANIAN_
                            + "\" but got "
                            + UCharacter.toLowerCase(LITHUANIAN_LOCALE_, SPECIAL_DOT_ABOVE_UPPER_));
        }
        if (!SPECIAL_DOT_ABOVE_LOWER_GERMAN_.equals(
                UCharacter.toLowerCase(GERMAN_LOCALE_, SPECIAL_DOT_ABOVE_UPPER_))) {
            errln(
                    "error in dots.toLower(de)=\""
                            + SPECIAL_DOT_ABOVE_UPPER_
                            + "\" expected \""
                            + SPECIAL_DOT_ABOVE_LOWER_GERMAN_
                            + "\" but got "
                            + UCharacter.toLowerCase(GERMAN_LOCALE_, SPECIAL_DOT_ABOVE_UPPER_));
        }
    }
    ...
    /**
     * Tests for case mapping in the file SpecialCasing.txt This method reads in SpecialCasing.txt
     * file for testing purposes. A default path is provided relative to the src path, however the
     * user could set a system property to change the directory path.<br>
    ...
    public void TestSpecialCasingTxt() {
        try {
            ...
            BufferedReader input = TestUtil.getUtf8DataReader("unicode/SpecialCasing.txt");
            while (true) {
                ...
                for (int i = 4; i < chstr.length; i++) {
                    ...
                    } else if (condition.compareToIgnoreCase("Not_Before_Dot") == 0) {
                        // turns I into dotless i
                    } else if (condition.compareToIgnoreCase("More_Above") == 0) {
                        strbuffer.append((char) 0x300);
                        lowerbuffer.append((char) 0x300);
                        upperbuffer.append((char) 0x300);
                    } else if (condition.compareToIgnoreCase("After_Soft_Dotted") == 0) {
                    ...
                    } else if (condition.compareToIgnoreCase("Final_Sigma") == 0) {
    ...
    public void TestUpperLower() {
        int upper[] = {0x0041, 0x0042, 0x00b2, 0x01c4, 0x01c6, 0x01c9, 0x01c8, 0x01c9, 0x000c};
        int lower[] = {0x0061, 0x0062, 0x00b2, 0x01c6, 0x01c6, 0x01c9, 0x01c9, 0x01c9, 0x000c};
        ...
        for (int i = 0; i < upper.length; i++) {
            if (UCharacter.toLowerCase(upper[i]) != lower[i]) {
                errln(
                        "FAILED UCharacter.tolower() for \\u"
                                + Utility.hex(upper[i], 4)
                                + " Expected \\u"
                                + Utility.hex(lower[i], 4)
                                + " Got \\u"
        ...
        for (int i = 0; i < upperTest.length(); i++) {
            ...
            if (UCharacter.isLetter(upperTest.charAt(i))
                ...
                errln("Failed isLowerCase test at \\u" + Utility.hex(upperTest.charAt(i), 4));
            } else if (UCharacter.isLetter(lowerTest.charAt(i))
                    && !UCharacter.isUpperCase(lowerTest.charAt(i))) {
                errln("Failed isUpperCase test at \\u" + Utility.hex(lowerTest.charAt(i), 4));
            } else if (upperTest.charAt(i) != UCharacter.toLowerCase(lowerTest.charAt(i))) {
                errln(
                        "Failed case conversion from \\u"
                                + Utility.hex(lowerTest.charAt(i), 4)
                                + " To \\u"
                                + Utility.hex(upperTest.charAt(i), 4));
            } else if (lowerTest.charAt(i) != UCharacter.toUpperCase(upperTest.charAt(i))) {
                errln(
                        "Failed case conversion : \\u"
                                + Utility.hex(upperTest.charAt(i), 4)
                                + " To \\u"
                                + Utility.hex(lowerTest.charAt(i), 4));
            } else if (upperTest.charAt(i) != UCharacter.toLowerCase(upperTest.charAt(i))) {
                errln("Failed case conversion with itself: \\u" + Utility.hex(upperTest.charAt(i)));
            } else if (lowerTest.charAt(i) != UCharacter.toUpperCase(lowerTest.charAt(i))) {
                errln("Failed case conversion with itself: \\u" + Utility.hex(lowerTest.charAt(i)));
            }
        }
    ...
    public void TestGreekUpper() {
        ...
        assertGreekUpper(
                "Επειδή η αναγνώριση της αξιοπρέπειας", "ΕΠΕΙΔΗ Η ΑΝΑΓΝΩΡΙΣΗ ΤΗΣ ΑΞΙΟΠΡΕΠΕΙΑΣ");
        ...
        assertGreekUpper(
                "Ελάτε να φάτε τα καλύτερα παϊδάκια!", "ΕΛΑΤΕ ΝΑ ΦΑΤΕ ΤΑ ΚΑΛΥΤΕΡΑ ΠΑΪΔΑΚΙΑ!");
    ...
    /**
     * Maps indexes according to the expected edits. A destination index can occur multiple times
     * when there are source deletions. Map according to the last occurrence, normally in a
     * non-empty destination span. Simplest is to search from the back.
     */
    ...
    private static void checkEditsIter(
            String name,
            Edits.Iterator ei1,
            Edits.Iterator ei2, // two equal iterators
            EditChange[] expected,
            boolean withUnchanged) {
    ...
    public void TestEdits() {
        ...
        EditChange[] coarseExpectedChanges =
                new EditChange[] {
                    new EditChange(false, 10003, 10003), new EditChange(true, 103106, 104013)
                };
        checkEditsIter(
                "coarse",
                edits.getCoarseIterator(),
                edits.getCoarseIterator(),
                coarseExpectedChanges,
                true);
        checkEditsIter(
                "coarse changes",
    ...
    public void TestCaseMapWithEdits() {
        ...
        EditChange[] lowerExpectedChanges =
                new EditChange[] {
        ...
        EditChange[] upperExpectedChanges =
                new EditChange[] {
        ...
        sb =
                CaseMap.toTitle()
                        ...
                        .noBreakAdjustment()
                        .noLowercase()
                        .apply(DUTCH_LOCALE_, null, "IjssEL IglOo", sb, edits);
        assertEquals("toTitle(IjssEL IglOo)", "J", sb.toString());
        EditChange[] titleExpectedChanges =
                new EditChange[] {
                    new EditChange(false, 1, 1),
                    new EditChange(true, 1, 1),
        ...
        EditChange[] foldExpectedChanges =
                new EditChange[] {
                    new EditChange(true, 1, 1),
    ...
    public void TestCaseMapToString() {
        ...
        assertEquals(
                ...
                CaseMap.toTitle()
                        ...
                        .noBreakAdjustment()
                        .noLowercase()
                        .apply(DUTCH_LOCALE_, null, "IjssEL IglOo"));
        assertEquals(
                "fold(IßtanBul)",
                "ıssb",
        ...
        assertEquals("toUpper(Πατάτα)", "ΠΑΤΑΤΑ", CaseMap.toUpper().apply(GREEK_LOCALE_, "Πατάτα"));
        assertEquals(
                "toTitle(IjssEL IglOo)",
                "IJssEL IglOo",
                CaseMap.toTitle()
                        ...
                        .noLowercase()
                        .apply(DUTCH_LOCALE_, null, "IjssEL IglOo"));
        assertEquals("fold(IßtanBul)", "ısstanbul", CaseMap.fold().turkic().apply("IßtanBul"));
    }
    ...
    public void TestCaseMapEditsIteratorDocs() {
        ...
        String[] coarseChangesIteratorExpected = {
            "{ src[3..5] ⇝ dest[3..6], repl[0..3] }", "{ src[6..7] ⇝ dest[7..8], repl[3..4] }",
        };
    ...
    private static final String SHARED_UPPERCASE_ISTANBUL_ = "\u0130STANBUL, NOT CONSTANTINOPLE!";
    private static final String SHARED_LOWERCASE_ISTANBUL_ = "i\u0307stanbul, not constantinople!";
    private static final String SHARED_LOWERCASE_TOPKAP_ = "topkap\u0131 palace, istanbul";
    private static final String SHARED_UPPERCASE_TOPKAP_ = "TOPKAPI PALACE, ISTANBUL";
    private static final String SHARED_LOWERCASE_GERMAN_ = "S\u00FC\u00DFmayrstra\u00DFe";
    private static final String SHARED_UPPERCASE_GERMAN_ = "S\u00DCSSMAYRSTRASSE";
     ...
     * the break iterator is specified as an int, same as in BreakIterator.KIND_*: 0=KIND_CHARACTER
     * 1=KIND_WORD 2=KIND_LINE 3=KIND_SENTENCE 4=KIND_TITLE -1=default (NULL=words) -2=no breaks
     * (.*) options: T=U_FOLD_CASE_EXCLUDE_SPECIAL_I L=U_TITLECASE_NO_LOWERCASE
     * A=U_TITLECASE_NO_BREAK_ADJUSTMENT see ICU4C source/test/testdata/casing.txt
     */
    private static final String TITLE_DATA_[] = {
        ...
        "1",
        "",
        "\u02bbaMeLikA huI P\u016b \u02bb\u02bb\u02bbiA",
        "\u02bbAmelika Hui P\u016b \u02bb\u02bb\u02bbIa", // titlecase first _cased_ letter, j4933
        "",
        "-1",
        "",
        " tHe QUIcK bRoWn",
    ...
    private static final String SPECIAL_DATA_[] = {
        ...
        "ab'cD \uFB00i\u0131I\u0130 \u01C7\u01C8\u01C9 "
                + UTF16.valueOf(0x1043C)
                + UTF16.valueOf(0x10414),
        "ab'cd \uFB00i\u0131ii\u0307 \u01C9\u01C9\u01C9 "
                + UTF16.valueOf(0x1043C)
                + UTF16.valueOf(0x1043C),
        "AB'CD FFIII\u0130 \u01C7\u01C7\u01C7 " + UTF16.valueOf(0x10414) + UTF16.valueOf(0x10414),
