...
public class NumberRegressionTests extends CoreTestFmwk {
    ...
    public void Test4075713() {
        ...
        try {
            ...
            if (!tmp.equals(null)) logln("NumberFormat.equals passed");
    ...
    public void Test4088161() {
        ...
        if (!sBuf2.toString().equals("100")) errln(" format(d) = '" + sBuf2 + "'");
    }

    /**
     * DecimalFormatSymbols should be cloned in the ctor DecimalFormat. DecimalFormat(String,
    ...
    public void Test4087245() {
        ...
        if (!buf1.toString().equals(buf2.toString())) errln("Test for bug 4087245 failed");
    }
    ...
    public void Test4087535() {
        ...
        buffer = df.format(n);
        if (buffer.length() == 0) errln(n + ": '" + buffer + "'");
        n = 0.1;
        buffer = df.format(n);
        if (buffer.length() == 0) errln(n + ": '" + buffer + "'");
    }
    ...
    public void Test4083018() {
        ...
        try {
            if (!dfs.equals(null)) logln("Test Passed!");
        } catch (Exception foo) {
    ...
    /**
     * A space as a group separator for localized pattern causes wrong format. WorkAround : use
     * non-breaking space.
     */
    @Test
    public void Test4086575() {
        ...
        if (!buffer.equals("1\u202f234,00")) errln("nf : " + buffer); // Expect 1 234,00
    ...
    public void Test4092561() {
        ...
        assertEquals(
                "Bug 4092561 test failed when multiplier is set to not 1.",
    ...
    public void Test4092480() {
        ...
        try {
            ...
            if (!dfFoo.toPattern().equals("0000")) errln("dfFoo.toPattern : " + dfFoo.toPattern());
            logln(dfFoo.format(42));
            logln(dfFoo.format(-42));
            dfFoo.applyPattern("000;-000");
            if (!dfFoo.toPattern().equals("000")) errln("dfFoo.toPattern : " + dfFoo.toPattern());
            logln(dfFoo.format(42));
            logln(dfFoo.format(-42));
    ...
    /**
     * NumberFormat.getCurrencyInstance() produces format that uses decimal separator instead of
     * monetary decimal separator.
     *
     * <p>Rewrote this test not to depend on the actual pattern. Pattern should never contain the
     * monetary separator! Decimal separator in pattern is interpreted as monetary separator if
     * currency symbol is seen!
     */
    @Test
    ...
    /**
     * Data rounding errors for Italian locale number formats Note- with the Euro, there is no need
     * for currency rounding anymore
     */
    @Test
    ...
    /**
     * Tests illegal pattern exception. Fix comment : HShih A31 Part1 will not be fixed and javadoc
     * needs to be updated. Part2 has been fixed.
     */
    @Test
    public void Test4074454() {
        try {
            ...
            logln(
                    "format 3456.78: "
                            + fmt.format(3456.78)); // fix "The variable 'fmt' is never used"
    ...
    /**
     * Tests all different comments. Response to some comments : [1] DecimalFormat.parse API
     * documentation is more than just one line. This is not a reproducible doc error in 116 source
     * code. [2] See updated javadoc. [3] Fixed. [4] NumberFormat.parse(String, ParsePosition) : If
     * parsing fails, a null object will be returned. The unchanged parse position also reflects an
     * error. NumberFormat.parse(String) : If parsing fails, an ParseException will be thrown. See
     * updated javadoc for more details. [5] See updated javadoc. [6] See updated javadoc. [7] This
     * is a correct behavior if the DateFormat object is linient. Otherwise, an
     * IllegalArgumentException will be thrown when formatting "January 35". See GregorianCalendar
    ...
    public void Test4061302() {
        ...
        if (currency.equals("") || intlCurrency.equals("") || monDecSeparator == 0) {
        ...
        if (!currency.equals("XYZ") || !intlCurrency.equals("ABC") || monDecSeparator != '*') {
    ...
    public void Test4062486() {
        ...
        if (field.getBeginIndex() != 0 && field.getEndIndex() != 5)
            errln(
                    "Format 1234.5 failed. Begin index: "
                            + field.getBeginIndex()
                            + " End index: "
                            + field.getEndIndex());
        ...
        if (field.getBeginIndex() != 7 && field.getEndIndex() != 4)
            errln(
                    "Set begin/end field indexes failed. Begin index: "
                            + field.getBeginIndex()
                            + " End index: "
                            + field.getEndIndex());
    }
    ...
    public void Test4108738() {
        ...
        if (!num.toString().equals("1.222")) errln("\"" + text + "\"  is parsed as " + num);
        text = "1.222x111";
    ...
    public void Test4114639() {
        ...
        if (result != null)
            errln("Should return null but got : " + result); // Should be null; it isn't
    }
    ...
    public void Test4106664() {
        ...
        df.setMultiplier(m);
        df.setGroupingUsed(false);
        logln("formatted: " + df.format(n, new StringBuffer(), new FieldPosition(0)));
     ...
     *
     * <p>1) Make sure that all currency formats use the generic currency symbol. 2) Make sure we
     * get the same results using the generic symbol or a hard-coded one.
     *
     * <p>ICU 62: DecimalFormatSymbols currency symbol has long been deprecated. In the absence of a
     * user-specified currency, XXX is used instead.
     */
    @Test
    @Ignore
    public void Test4122840() {
        ...
        for (int i = 0; i < locales.length; i++) {
            ICUResourceBundle rb =
                    (ICUResourceBundle)
                            ICUResourceBundle.getBundleInstance(ICUData.ICU_BASE_NAME, locales[i]);
            ...
            //
            String pattern =
                    rb.getStringWithFallback("NumberElements/latn/patterns/currencyFormat");
            ...
            // Currency spacing may have been added by the real DecimalFormat.  Account for that
            // here.
    ...
    public void Test4125885() {
        ...
        if (!rateString.equals("012.34")) errln("result : " + rateString + " expected : 012.34");
        rate = 0.1234;
        formatDec = null;
    ...
    public void Test4134034() {
        ...
        if (f.equals("9.02")) logln(f + " ok");
        else errln("9.02 -> " + f + "; want 9.02");

        f = nf.format(0);
        if (f.equals(".00")) logln(f + " ok");
        else errln("0 -> " + f + "; want .00");
    }

    /**
     * CANNOT REPRODUCE - This bug could not be reproduced. It may be a duplicate of 4134034.
     *
     * <p>JDK 1.1.6 Bug, did NOT occur in 1.1.5 Possibly related to bug 4125885.
     *
     * <p>This class demonstrates a regression in version 1.1.6 of DecimalFormat class.
     *
     * <p>1.1.6 Results Value 1.2 Format #.00 Result '01.20' !!!wrong Value 1.2 Format 0.00 Result
     * '001.20' !!!wrong Value 1.2 Format 00.00 Result '0001.20' !!!wrong Value 1.2 Format #0.0#
     * Result '1.2' Value 1.2 Format #0.00 Result '001.20' !!!wrong
     *
     * <p>1.1.5 Results Value 1.2 Format #.00 Result '1.20' Value 1.2 Format 0.00 Result '1.20'
     * Value 1.2 Format 00.00 Result '01.20' Value 1.2 Format #0.0# Result '1.2' Value 1.2 Format
     * #0.00 Result '1.20'
     */
    @Test
    public void Test4134300() {
        ...
        for (int i = 0; i < DATA.length; i += 2) {
            ...
            if (!result.equals(DATA[i + 1])) {
                errln("Fail: 1.2 x " + DATA[i] + " = " + result + "; want " + DATA[i + 1]);
            } else {
    ...
    public void Test4140009() {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                assertEquals(
                        "<empty pat " + i + ">.format(" + IN[j] + ")", OUT[j], f.format(IN[j]));
            }
        }
    }
                ...
                if (!pat.equals(pat2))
                    errln(
                            "Fail with \""
                                    ...
                                    + pat2
                                    + "\"");
                else logln("Ok \"" + PATS[i] + "\" toPattern() -> \"" + pat + '"');
    ...
    /**
     * DecimalFormat.applyPattern() sets minimum integer digits incorrectly. CANNOT REPRODUCE This
     * bug is a duplicate of 4139344, which is a duplicate of 4134300
     */
    @Test
    ...
    public void Test4162852() throws ParseException {
        for (int i = 0; i < 2; ++i) {
            NumberFormat f =
                    (i == 0) ? NumberFormat.getInstance() : NumberFormat.getPercentInstance();
            double d = -0.0;
            String s = f.format(d);
    ...
    /**
     * Some DecimalFormatSymbols changes are not picked up by DecimalFormat. This includes the minus
     * sign, currency symbol, international currency symbol, percent, and permille. This is filed as
     * bugs 4212072 and 4212073.
     */
    @Test
    public void Test4212072() throws IOException, ClassNotFoundException {
        ...
        if (!fmt.format(12.5).equals("usd 12.50")) {
            errln("FAIL: 12.5 x (currency=usd) -> " + fmt.format(12.5) + ", exp usd 12.50");
        }
        if (!fmt.getPositivePrefix().equals("usd")) {
            errln(
                    "FAIL: (currency=usd).getPositivePrefix -> "
        ...
        if (!fmt.format(12.5).equals("DOL 12.50")) {
            errln("FAIL: 12.5 x (intlcurrency=DOL) -> " + fmt.format(12.5) + ", exp DOL 12.50");
        }
        if (!fmt.getPositivePrefix().equals("DOL")) {
            errln(
                    "FAIL: (intlcurrency=DOL).getPositivePrefix -> "
                            + fmt.getPositivePrefix()
                            + ", exp DOL");
        }
                    ...
                    if (!s1.equals(s2)) {
                        errln(
                                "FAIL: "
                                        ...
                                        + '"'
                                        + " in locale "
                                        + df.getLocale(ULocale.ACTUAL_LOCALE));
                    }

                    // Equality of formatter objects is NOT guaranteed across
                    // toLocalizedPattern/applyLocalizedPattern.
                    // However, equality of relevant properties is guaranteed.
                    assertEquals(
                            "Localized FAIL on posPrefix",
                            df.getPositivePrefix(),
                            f2.getPositivePrefix());
                    assertEquals(
                            "Localized FAIL on posSuffix",
                            df.getPositiveSuffix(),
                            f2.getPositiveSuffix());
                    assertEquals(
                            "Localized FAIL on negPrefix",
                            df.getNegativePrefix(),
                            f2.getNegativePrefix());
                    assertEquals(
                            "Localized FAIL on negSuffix",
                            df.getNegativeSuffix(),
                            f2.getNegativeSuffix());
                    assertEquals(
                            "Localized FAIL on groupingSize",
                            df.getGroupingSize(),
                            f2.getGroupingSize());
                    assertEquals(
                            "Localized FAIL on secondaryGroupingSize",
                            df.getSecondaryGroupingSize(),
                            f2.getSecondaryGroupingSize());
                    assertEquals(
                            "Localized FAIL on minFrac",
                            df.getMinimumFractionDigits(),
                            f2.getMinimumFractionDigits());
                    assertEquals(
                            "Localized FAIL on maxFrac",
                            df.getMaximumFractionDigits(),
                            f2.getMaximumFractionDigits());
                    assertEquals(
                            "Localized FAIL on minInt",
                            df.getMinimumIntegerDigits(),
                            f2.getMinimumIntegerDigits());
                    assertEquals(
                            "Localized FAIL on maxInt",
                            df.getMaximumIntegerDigits(),
                            f2.getMaximumIntegerDigits());
    ...
    public void Test4216742() throws ParseException {
        ...
        for (int i = 0; i < DATA.length; ++i) {
            ...
            for (int m = 1; m <= 100; m++) {
                ...
                if (n > 0 != DATA[i] > 0) {
                    errln("\"" + str + "\" parse(x " + fmt.getMultiplier() + ") => " + n);
                }
            }
        }
    }
