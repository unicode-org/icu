...
public class DataDrivenCalendarTest extends CoreTestFmwk {
    ...
    void testConvert(
        ...
        if (!fromSet.matches(fromCalendar, diffSet)) {
            ...
            errln(
                    (String) "FAIL: "
                            + thisString
                            + ", SOURCE calendar was not set: Differences: "
                            + diffs);
        } else {
    ...
    private void testOps(TestDataModule.TestData testData, DataMap settings) {
        // Get 'from' time
        CalendarFieldsSet fromSet = new CalendarFieldsSet(),
                toSet = new CalendarFieldsSet(),
                paramsSet = new CalendarFieldsSet(),
                diffSet = new CalendarFieldsSet();
        //        DateFormat fmt = new SimpleDateFormat("EEE MMM dd yyyy / YYYY'-W'ww-ee");
        // Start the processing
        int n = 0;
        long fromDate = 0;
        ...
        for (Iterator<DataMap> iter = testData.getDataIterator(); iter.hasNext(); ) {
            ...
            if (!fromSet.matches(fromCalendar, diffSet)) {
                ...
                logln(
                        " "
                                + caseString
                                + " SET SOURCE calendar match."); // verifies that the requested
                // fields were set.
            }
