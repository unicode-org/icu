...
public class DataDrivenFormatTest extends CoreTestFmwk {
    ...
    private void testConvertDate(TestDataModule.TestData testData, DataMap settings, boolean fmt) {
        ...
        for (Iterator<DataMap> iter = testData.getDataIterator(); iter.hasNext(); ) {
            ...
            String caseString =
                    "[" + testData.getName() + "#" + n + (fmt ? "format" : "parse") + "]";
            ...
            if (spec.startsWith(kPATTERN)) {
                ...
                format =
                        DateFormat.getDateTimeInstance(
                                styleSet.getDateStyle(), styleSet.getTimeStyle(), loc);
            }
            ...
            if (date.startsWith(kMILLIS)) {
                ...
                useDate = true;
                fromDate =
                        new Date(now + Long.parseLong(date.substring(kRELATIVE_MILLIS.length())));
            } else if (date.startsWith(kRELATIVE_ADD)) {
                String add =
                        date.substring(
                                kRELATIVE_ADD
                                        .length()); // "add" is a string indicating which fields to
                // add
            ...
            if (fmt) {
                @SuppressWarnings(
                        "JdkObsolete") // Because of DateFormat.format(...,StringBuffer,...)
