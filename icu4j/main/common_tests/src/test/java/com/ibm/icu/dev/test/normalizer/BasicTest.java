...
public class BasicTest extends CoreTestFmwk {
    String[][] canonTests = {
        ...
        {"A\u0300\u0316", "A\u0316\u0300", "\u00C0\u0316"},
        {
            "\\U0001d15e\\U0001d157\\U0001d165\\U0001d15e",
            "\\U0001D157\\U0001D165\\U0001D157\\U0001D165\\U0001D157\\U0001D165",
            "\\U0001D157\\U0001D165\\U0001D157\\U0001D165\\U0001D157\\U0001D165"
        },
    };
    ...
    /**
     * The Tibetan vowel sign AA, 0f71, was messed up prior to Unicode version 2.1.9. Once 2.1.9 or
     * 3.0 is released, uncomment this test.
     */
    @Test
    public void TestTibetan() throws Exception {
        String[][] decomp = {{"\u0f77", "\u0f77", "\u0fb2\u0f71\u0f80"}};
        String[][] compose = {{"\u0fb2\u0f71\u0f80", "\u0fb2\u0f71\u0f80", "\u0fb2\u0f71\u0f80"}};
    ...
    /**
     * Test for a problem that showed up just before ICU 1.6 release having to do with combining
     * characters with an index of zero. Such characters do not participate in any canonical
     * decompositions. However, having an index of zero means that they all share one typeMask[]
     * entry, that is, they all have to map to the same canonical class, which is not the case, in
     * reality.
     */
    @Test
    ...
    /**
     * Test for a problem found by Verisign. Problem is that characters at the start of a string are
     * not put in canonical order correctly by compose() if there is no starter.
     */
    @Test
    ...
    public void TestQuickCheckResultNO() throws Exception {
        final char CPNFD[] = {
            0x00C5, 0x0407, 0x1E00, 0x1F57, 0x220C, 0x30AE, 0xAC00, 0xD7A3, 0xFB36, 0xFB4E
        };
        final char CPNFC[] = {
            0x0340, 0x0F93, 0x1F77, 0x1FBB, 0x1FEB, 0x2000, 0x232A, 0xF900, 0xFA1E, 0xFB4E
        };
        final char CPNFKD[] = {
            0x00A0, 0x02E4, 0x1FDB, 0x24EA, 0x32FE, 0xAC00, 0xFB4E, 0xFA10, 0xFF3F, 0xFA2D
        };
        final char CPNFKC[] = {
            0x00A0, 0x017F, 0x2000, 0x24EA, 0x32FE, 0x33FE, 0xFB4E, 0xFA10, 0xFF3F, 0xFA2D
        };
        ...
        for (; count < SIZE; count++) {
            if (Normalizer.quickCheck(String.valueOf(CPNFD[count]), Normalizer.NFD, 0)
                    != Normalizer.NO) {
                errln("ERROR in NFD quick check at U+" + Integer.toHexString(CPNFD[count]));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(CPNFC[count]), Normalizer.NFC, 0)
                    != Normalizer.NO) {
                errln("ERROR in NFC quick check at U+" + Integer.toHexString(CPNFC[count]));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(CPNFKD[count]), Normalizer.NFKD, 0)
                    != Normalizer.NO) {
                errln("ERROR in NFKD quick check at U+" + Integer.toHexString(CPNFKD[count]));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(CPNFKC[count]), Normalizer.NFKC, 0)
                    != Normalizer.NO) {
                errln("ERROR in NFKC quick check at U+" + Integer.toHexString(CPNFKC[count]));
                return;
            }
            // for improving coverage
            if (Normalizer.quickCheck(String.valueOf(CPNFKC[count]), Normalizer.NFKC)
                    != Normalizer.NO) {
                errln("ERROR in NFKC quick check at U+" + Integer.toHexString(CPNFKC[count]));
                return;
            }
    ...
    public void TestQuickCheckResultYES() throws Exception {
        final char CPNFD[] = {
            0x00C6, 0x017F, 0x0F74, 0x1000, 0x1E9A, 0x2261, 0x3075, 0x4000, 0x5000, 0xF000
        };
        final char CPNFC[] = {
            0x0400, 0x0540, 0x0901, 0x1000, 0x1500, 0x1E9A, 0x3000, 0x4000, 0x5000, 0xF000
        };
        final char CPNFKD[] = {
            0x00AB, 0x02A0, 0x1000, 0x1027, 0x2FFB, 0x3FFF, 0x4FFF, 0xA000, 0xF000, 0xFA27
        };
        final char CPNFKC[] = {
            0x00B0, 0x0100, 0x0200, 0x0A02, 0x1000, 0x2010, 0x3030, 0x4000, 0xA000, 0xFA0E
        };
        ...
        char cp = 0;
        while (cp < 0xA0) {
            if (Normalizer.quickCheck(String.valueOf(cp), Normalizer.NFD, 0) != Normalizer.YES) {
                errln("ERROR in NFD quick check at U+" + Integer.toHexString(cp));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(cp), Normalizer.NFC, 0) != Normalizer.YES) {
                errln("ERROR in NFC quick check at U+" + Integer.toHexString(cp));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(cp), Normalizer.NFKD, 0) != Normalizer.YES) {
                errln("ERROR in NFKD quick check at U+" + Integer.toHexString(cp));
                return;
            }
            if (Normalizer.quickCheck(String.valueOf(cp), Normalizer.NFKC, 0) != Normalizer.YES) {
                errln("ERROR in NFKC quick check at U+" + Integer.toHexString(cp));
                return;
            }
            // improve the coverage
            if (Normalizer.quickCheck(String.valueOf(cp), Normalizer.NFKC) != Normalizer.YES) {
                errln("ERROR in NFKC quick check at U+" + Integer.toHexString(cp));
                return;
            }
            cp++;
    ...
    public void TestQuickCheckResultMAYBE() throws Exception {
        ...
        final char[] CPNFC = {
            0x0306, 0x0654, 0x0BBE, 0x102E, 0x1161, 0x116A, 0x1173, 0x1175, 0x3099, 0x309A
        };
        final char[] CPNFKC = {
            0x0300, 0x0654, 0x0655, 0x09D7, 0x0B3E, 0x0DCF, 0xDDF, 0x102E, 0x11A8, 0x3099
        };
        ...
        for (; count < SIZE; count++) {
            ...
            if (Normalizer.quickCheck(new char[] {CPNFC[count]}, Normalizer.NFC, 0)
                    != Normalizer.MAYBE) {
                errln("ERROR in NFC quick check at U+" + Integer.toHexString(CPNFC[count]));
                return;
            }
            if (Normalizer.quickCheck(new char[] {CPNFKC[count]}, Normalizer.NFKC, 0)
                    != Normalizer.MAYBE) {
                errln("ERROR in NFKC quick check at U+" + Integer.toHexString(CPNFKC[count]));
                return;
            }
            if (Normalizer.quickCheck(new char[] {CPNFKC[count]}, Normalizer.NONE, 0)
                    != Normalizer.YES) {
                errln("ERROR in NONE quick check at U+" + Integer.toHexString(CPNFKC[count]));
                return;
            }
        }
    }
    ...
    public void TestQuickCheckStringResult() throws Exception {
        ...
        for (count = 0; count < canonTests.length; count++) {
            ...
            if (Normalizer.quickCheck(d, Normalizer.NFD, 0) != Normalizer.YES) {
                errln("ERROR in NFD quick check for string at count " + count);
                return;
            }

            if (Normalizer.quickCheck(c, Normalizer.NFC, 0) == Normalizer.NO) {
        ...
        for (count = 0; count < compatTests.length; count++) {
            ...
            if (Normalizer.quickCheck(d, Normalizer.NFKD, 0) != Normalizer.YES) {
                errln("ERROR in NFKD quick check for string at count " + count);
                return;
            }

            if (Normalizer.quickCheck(c, Normalizer.NFKC, 0) != Normalizer.YES) {
    ...
    public void TestQuickCheckPerCP() {
        ...
        if (UCharacter.getIntPropertyMaxValue(UProperty.NFD_QUICK_CHECK) != 1
                || // YES
                UCharacter.getIntPropertyMaxValue(UProperty.NFKD_QUICK_CHECK) != 1
                || UCharacter.getIntPropertyMaxValue(UProperty.NFC_QUICK_CHECK) != 2
                || // MAYBE
                UCharacter.getIntPropertyMaxValue(UProperty.NFKC_QUICK_CHECK) != 2
                || UCharacter.getIntPropertyMaxValue(UProperty.LEAD_CANONICAL_COMBINING_CLASS)
                        != UCharacter.getIntPropertyMaxValue(UProperty.CANONICAL_COMBINING_CLASS)
                || UCharacter.getIntPropertyMaxValue(UProperty.TRAIL_CANONICAL_COMBINING_CLASS)
                        != UCharacter.getIntPropertyMaxValue(UProperty.CANONICAL_COMBINING_CLASS)) {
            errln(
                    "wrong result from one of the u_getIntPropertyMaxValue(UCHAR_NF*_QUICK_CHECK) or UCHAR_*_CANONICAL_COMBINING_CLASS");
        }
        ...
        while (c < 0x110000) {
            ...
            if (qc1 != qc2) {
                errln(
                        "getIntPropertyValue(NFC)="
                                + qc1
                                + " != "
                                + qc2
                                + "=quickCheck(NFC) for U+"
            ...
            if (qc1 != qc2) {
                errln(
                        "getIntPropertyValue(NFD)="
                                + qc1
                                + " != "
                                + qc2
                                + "=quickCheck(NFD) for U+"
                                + Integer.toHexString(c));
            }
            ...
            if (qc1 != qc2) {
                errln(
                        "getIntPropertyValue(NFKC)="
                                + qc1
                                + " != "
                                + qc2
                                + "=quickCheck(NFKC) for U+"
                                + Integer.toHexString(c));
            }
            ...
            if (qc1 != qc2) {
                errln(
                        "getIntPropertyValue(NFKD)="
                                + qc1
                                + " != "
                                + qc2
                                + "=quickCheck(NFKD) for U+"
                                + Integer.toHexString(c));
            }
            ...
            if (lccc1 != lccc2) {
                errln(
                        "getIntPropertyValue(lccc)="
                                + lccc1
                                + " != "
                                + lccc2
                                + "=getCombiningClass(lead) for U+"
                                + Integer.toHexString(c));
            }
            if (tccc1 != tccc2) {
                errln(
                        "getIntPropertyValue(tccc)="
                                + tccc1
                                + " != "
                                + tccc2
                                + "=getCombiningClass(trail) for U+"
                                + Integer.toHexString(c));
            }
    ...
    private void backAndForth(Normalizer iter, String[][] tests) {
        for (int i = 0; i < tests.length; i++) {
            ...
            if (!forward.toString().equals(reverse.toString())) {
                errln(
                        "FAIL: Forward/reverse mismatch for input "
                                + hex(tests[i][0])
                                + ", forward: "
                                + hex(forward)
                                + ", backward: "
    ...
    private void staticTest(Normalizer.Mode mode, String[][] tests, int outCol) throws Exception {
        for (int i = 0; i < tests.length; i++) {
    ...
    private void decomposeTest(Normalizer.Mode mode, String[][] tests, int outCol)
            throws Exception {
        for (int i = 0; i < tests.length; i++) {
            String input = Utility.unescape(tests[i][0]);
            String expect = Utility.unescape(tests[i][outCol]);
        ...
        for (int i = 0; i < tests.length; i++) {
            ...
            if (retLength != reqLength) {
                logln(
                        "FAIL: Normalizer.compose did not return the expected length. Expected: "
                                + reqLength
                                + " Got: "
                                + retLength);
            }
    ...
    private void composeTest(Normalizer.Mode mode, String[][] tests, int outCol) throws Exception {
        for (int i = 0; i < tests.length; i++) {
            String input = Utility.unescape(tests[i][0]);
            String expect = Utility.unescape(tests[i][outCol]);
    ...
    public void TestDebugStatic() {
        ...
        String expect =
                "\u1100\u116F\u11AA\u1100\u116F\u11AA\u1100\u116F"
                        ...
                        + "\u1100\u116F\u11AA\u1100\u116F\u11AA\u1100\u116F"
                        + "\u11AA\u1100\u116F\u11AA\u0064\u031B\u0323\u0307";
        String output = Normalizer.normalize(Utility.unescape(input), Normalizer.NFD);
    ...
    public void TestDebugIter() {
        ...
        Normalizer iter =
                new Normalizer(
                        new StringCharacterIterator(Utility.unescape(src)), Normalizer.NONE, 0);
        int index = 0;
        int ch;
        UCharacterIterator cIter = UCharacterIterator.getInstance(expected);
    ...
    public void TestDebugIterOld() {
        ...
        Normalizer iter =
                new Normalizer(
                        new StringCharacterIterator(Utility.unescape(input)), Normalizer.NFKC, 0);
    ...
    public void TestPreviousNext() {
        ...
        char src[] = {
            UTF16.getLeadSurrogate(0x2f999),
            UTF16.getTrailSurrogate(0x2f999),
            UTF16.getLeadSurrogate(0x1d15f),
            UTF16.getTrailSurrogate(0x1d15f),
            0xc4,
            0x1ed0
        };
        ...
        int expectIndex[] = {
            0, 2, 2, 4, 4, 5, 5, 5, 6 // behind last character
        };
        ...
        while (movesIndex < moves.length()) {
            ...
            if (c1 != c2) {
                ...
                errln(
                        "error: mismatch in Normalizer iteration at "
                                + history
                                + ": "
                                + "got c1= "
                                + hex(c1)
            ...
            if (iter.getIndex() != expectIndex[iter32.getIndex()]) {
                ...
                String history = moves.substring(0, movesIndex);
                errln(
                        "error: index mismatch in Normalizer iteration at "
                                + history
                                + " : "
                                + "Normalizer index "
                                + iter.getIndex()
                                + " expected "
                                + expectIndex[iter32.getIndex()]);
                break;
            }
        }
    }
    ...
    public void TestNormalizerAPI() throws Exception {
        try {
            ...
            //    errln("error in
            // Normalizer(Normalizer(CharacterIterator)).clone().hashCode()!=copy.hashCode()");
            // }
            ...
            if (clone.hashCode() == norm.hashCode()) {
                errln(
                        "error in Normalizer(Normalizer(CharacterIterator)).clone().next().hashCode()==copy.hashCode()");
            }
            ...
            if (!nfkc.equals(Utility.unescape("TE\u0139TELTELTELTELTELTELTELTELTEL"))
            ...
            if (!Utility.arrayRegionMatches(fillIn1, 0, fillIn2, 0, len)) {
                errln(
                        "error in Normalizer.getText(). Normalizer: "
                                + Utility.hex(new String(fillIn1))
                                + " Iter: "
                                + Utility.hex(new String(fillIn2)));
            }
            ...
            if (!Utility.arrayRegionMatches(fillIn1, 0, fillIn2, 0, len)) {
                errln(
                        "error in Normalizer.setText() or Normalizer.getText()"
                                + Utility.hex(new String(fillIn1))
                                + " Iter: "
                                + Utility.hex(new String(fillIn2)));
            }
    ...
    public void TestConcatenate() {
        ...
        Object[][] cases =
                new Object[][] {
                    ...
                    {Normalizer.NFC, "re", "\u0301sum\u00e9", "r\u00e9sum\u00e9"},
                    {Normalizer.NFC, "a\u1100", "\u1161bcdefghijk", "a\uac00bcdefghijk"},
        ...
        for (i = 0; i < cases.length; ++i) {
            ...
            expect = (String) cases[i][3];
            {
                ...
                if (!result.equals(expect)) {
                    errln(
                            "error in Normalizer.concatenate(), cases[] failed"
        ...
        Normalizer.concatenate(
                left.toCharArray(), 0, 2, right.toCharArray(), 2, 15, destination, 3, 17, mode, 0);
        if (!String.valueOf(destination).equals(expect)) {
            errln(
                    "error in Normalizer.concatenate(), cases2[] failed"
                            + ", result==expect: expected: "
                            + hex(expect)
                            + " =========> got: "
                            + hex(destination));
    ...
    public void TestCheckFCD() {
        ...
        char[] FALSE = {
            0x0001, 0x0002, 0x02EA, 0x03EB, 0x0300, 0x0301, 0x02B9, 0x0314, 0x0315, 0x0316
        };

        char[] TRUE = {
            0x0030, 0x0040, 0x0440, 0x056D, 0x064F, 0x06E7, 0x0050, 0x0730, 0x09EE, 0x1E10
        };
        ...
        Normalizer.QuickCheckResult result[] = {
            Normalizer.YES, Normalizer.NO, Normalizer.NO, Normalizer.YES
        };

        char[] datachar = {
            0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0xe0, 0xe1, 0xe2,
            0xe3, 0xe4, 0xe5, 0xe6, 0xe7, 0xe8, 0xe9, 0xea, 0x0300, 0x0301, 0x0302, 0x0303, 0x0304,
            0x0305, 0x0306, 0x0307, 0x0308, 0x0309, 0x030a, 0x0320, 0x0321, 0x0322, 0x0323, 0x0324,
            0x0325, 0x0326, 0x0327, 0x0328, 0x0329, 0x032a, 0x1e00, 0x1e01, 0x1e02, 0x1e03, 0x1e04,
            0x1e05, 0x1e06, 0x1e07, 0x1e08, 0x1e09, 0x1e0a
        };
        ...
        if (Normalizer.quickCheck(FAST, 0, FAST.length, Normalizer.FCD, 0) != Normalizer.YES)
            errln(
                    "Normalizer.quickCheck(FCD) failed: expected value for fast Normalizer.quickCheck is Normalizer.YES\n");
        if (Normalizer.quickCheck(FALSE, 0, FALSE.length, Normalizer.FCD, 0) != Normalizer.NO)
            errln(
                    "Normalizer.quickCheck(FCD) failed: expected value for error Normalizer.quickCheck is Normalizer.NO\n");
        if (Normalizer.quickCheck(TRUE, 0, TRUE.length, Normalizer.FCD, 0) != Normalizer.YES)
            errln(
                    "Normalizer.quickCheck(FCD) failed: expected value for correct Normalizer.quickCheck is Normalizer.YES\n");

        while (count < 4) {
            Normalizer.QuickCheckResult fcdresult =
                    Normalizer.quickCheck(
                            datastr[count], 0, datastr[count].length, Normalizer.FCD, 0);
            if (result[count] != fcdresult) {
                errln(
                        "Normalizer.quickCheck(FCD) failed: Data set "
                                + count
                                + " expected value "
                                + result[count]);
            }
        ...
        for (count = 0; count < 50; count++) {
            ...
            while (size != 19) {
                ...
                normStart +=
                        Normalizer.normalize(
                                data, size, size + 1, norm, normStart, 100, Normalizer.NFD, 0);
                size++;
            }
            logln("\n");
            ...
            //                      nfd, 100, &status);
            if (nfdsize != normStart
                    || Utility.arrayRegionMatches(nfd, 0, norm, 0, nfdsize) == false) {
            ...
            if (Normalizer.quickCheck(data, 0, data.length, Normalizer.FCD, 0) != testresult) {
                errln(
                        "Normalizer.quickCheck(FCD) failed: expected "
                                + testresult
                                + " for random data: "
                                + hex(new String(data)));
            }
    ...
    private Temp[] opt =
            new Temp[] {
                ...
                new Temp(
                        Normalizer.COMPARE_CODE_POINT_ORDER | Normalizer.COMPARE_IGNORE_CASE,
                        "code point order & ignore case"),
                new Temp(
                        Normalizer.COMPARE_IGNORE_CASE | Normalizer.FOLD_CASE_EXCLUDE_SPECIAL_I,
                        "ignore case & special i"),
                new Temp(
                        Normalizer.COMPARE_CODE_POINT_ORDER
                                | Normalizer.COMPARE_IGNORE_CASE
                                | Normalizer.FOLD_CASE_EXCLUDE_SPECIAL_I,
                        "code point order & ignore case & special i"),
                new Temp(
                        Normalizer.UNICODE_3_2 << Normalizer.COMPARE_NORM_OPTIONS_SHIFT,
                        "Unicode 3.2")
            };
    ...
    public void TestCompare() {
                ...
                for (k = 0; k < opt.length; ++k) {
                    ...
                    if (0 != (opt[k].options & Normalizer.COMPARE_IGNORE_CASE)) {
                        ...
                        if ((opt[k].options & Normalizer.FOLD_CASE_EXCLUDE_SPECIAL_I) == 0) {
                            ...
                            comp.setIgnoreCase(
                                    true, UTF16.StringComparator.FOLD_CASE_EXCLUDE_SPECIAL_I);
                        }

                        comp.setCodePointCompare(
                                (opt[k].options & Normalizer.COMPARE_CODE_POINT_ORDER) != 0);
        ...
        if (nfcNorm2.getDecomposition(0x20) != null
            ...
            errln(
                    "NFC.getDecomposition() returns true for characters which do not have decompositions");
        }
        ...
        if (nfcNorm2.getRawDecomposition(0x20) != null
            ...
            errln(
                    "getRawDecomposition() returns true for characters which do not have decompositions");
        }
        ...
        if (fn2.getRawDecomposition(0xe4) != null
                || !"A\u0304".equals(fn2.getRawDecomposition(0x100))) {
    ...
    int countFoldFCDExceptions(int foldingOptions) {
        ...
        logln(
                "Test if case folding may un-FCD a string (folding options 0x)"
                        + hex(foldingOptions));
        ...
        for (c = 0; c <= 0x10ffff; ++c) {
            ...
            if (qcResult != Normalizer.YES
                    || s.length() == 0
                    || (cc != foldCC && foldCC != 0)
                    || (trailCC != foldTrailCC && foldTrailCC != 0)) {
                ++count;
                errln(
                        "U+"
                                + hex(c)
                                + ": case-folding may un-FCD a string (folding options 0x"
                                + hex(foldingOptions)
                                + ")");
                // errln("  cc %02x trailCC %02x    foldCC(U+%04lx) %02x foldTrailCC(U+%04lx) %02x
                // quickCheck(folded)=%d", cc, trailCC, UTF16.charAt(d,0), foldCC,
                // UTF16.charAt(d,d.length()-1), foldTrailCC, qcResult);
                continue;
            }
        ...
        logln(
                "There are "
                        + hex(count)
                        + " code points for which case-folding may un-FCD a string (folding options"
                        + foldingOptions
                        + "x)");
        return count;
    }
    ...
    public void TestFindFoldFCDExceptions() {
        ...
        if (count > 0) {
            ...
            errln(
                    "error: There are "
                            + count
                            + " code points for which case-folding"
    ...
    public void TestCombiningMarks() {
        ...
        if (!expected.equals(result)) {
            errln(
                    "Reordering of combining marks failed. Expected: "
                            + Utility.hex(expected)
                            + " Got: "
                            + Utility.hex(result));
        }
    }
    ...
    */
    ...
    private static UnicodeSet[] initSkippables(UnicodeSet[] skipSets) {
        ...
        while (iter.next()) {
            ...
            for (int i = 0; i < numCombineBack; ++i) {
                ...
                // c can only change if it combines with a character with a non-zero combining
                // class.
    ...
    public void TestSkippable() {
        ...
        UnicodeSet[] expectSets =
                new UnicodeSet[] {
                    new UnicodeSet(), new UnicodeSet(), new UnicodeSet(), new UnicodeSet()
                };
        ...
        for (int i = 0; i < expectSets.length; ++i) {
            if (!skipSets[i].equals(expectSets[i])) {
                ...
                pattern =
                        new StringBuilder(
                                ((UnicodeSet) skipSets[i].clone())
                                        .removeAll(expectSets[i])
                                        .toPattern(true));
                s.append(pattern);
                ...
                pattern =
                        new StringBuilder(
                                ((UnicodeSet) expectSets[i].clone())
                                        .removeAll(skipSets[i])
                                        .toPattern(true));
                s.append(pattern);
                s.append("\n\n");
                ...
                UnicodeSet intersection =
                        ((UnicodeSet) expectSets[i].clone()).retainAll(skipSets[i]);
    ...
    public void TestSerializedSet() {
        ...
        char[] serialized = {
            0x8007, // length
            3, // bmpLength
            0xc0, 0xfe, 0xfffc, 1, 9, 0x10, 0xfffc
        };
    ...
    public void TestReturnFailure() {
        ...
        int rc =
                Normalizer.decompose(
                        term, 0, term.length, decomposed_term, 0, decomposed_term.length, true, 0);
        int rc1 =
                Normalizer.decompose(
                        term, 0, term.length, decomposed_term, 10, decomposed_term.length, true, 0);
        if (rc != rc1) {
    ...
    public void TestComposition() {
        final TestCompositionCase cases[] =
                new TestCompositionCase[] {
                    /*
    ...
    public void TestGetDecomposition() {
        ...
        assertEquals(
                "fcc.getDecomposition(Hangul syllable U+AC01) failed",
                "\u1100\u1161\u11a8",
                decomp);
    }
    ...
    public void TestGetRawDecomposition() {
        ...
        assertEquals(
                "nfkc.getRawDecomposition(Hangul syllable U+AC00) failed", "\u1100\u1161", decomp);
        /* A Hangul LVT syllable has a raw decomposition of an LV syllable + T. */
        decomp = n2.getRawDecomposition(0xac01);
        assertEquals(
                "nfkc.getRawDecomposition(Hangul syllable U+AC01) failed", "\uac00\u11a8", decomp);
    }
    ...
    public void TestFilteredNormalizer2() {
        ...
        for (c = 0; c <= 0x3ff; ++c) {
            ...
            assertEquals(
                    "FilteredNormalizer2(NFC, ^A0-FF,310-31F).getCombiningClass(U+"
                            + hex(c)
                            + ")==filtered NFC.getCC()",
                    expectedCC,
                    cc);
    ...
    public void TestFilteredAppend() {
        ...
        assertEquals(
                ...
                "a\u0313á\u0313",
                fn2.normalizeSecondAndAppend(sb, second).toString());
    ...
    public void TestGetRawDecompositionBase() {
        int c = 'à';
        assertEquals(
                "Unexpected value returned from Normalizer2.getRawDecomposition()",
                null,
    ...
    public void TestComposePairBase() {
        ...
        int b = '\u0300';
        assertEquals(
                "Unexpected value returned from Normalizer2.composePair()",
                -1,
                tnorm2.composePair(a, b));
    }
    ...
    public void TestGetCombiningClassBase() {
        int c = '\u00e0';
        assertEquals(
                "Unexpected value returned from Normalizer2.getCombiningClass()",
                0,
                tnorm2.getCombiningClass(c));
    }
}
