...
/**
 * Low-level CollationData builder. Takes (character, CE) pairs and builds them into runtime data
 * structures. Supports characters with context prefixes and contraction suffixes.
 */
final class CollationDataBuilder { // not final in C++
    /**
     * Collation element modifier. Interface class for a modifier that changes a tailoring builder's
     * temporary CEs to final CEs. Called for every non-special CE32 and every expansion CE.
     */
    ...
    /**
     * Encodes the ces as either the returned ce32 by itself, or by storing an expansion, with the
     * returned ce32 referring to that.
     *
    ...
    /**
     * Copies all mappings from the src builder, with modifications. This builder here must not be
     * built yet, and should be empty.
     */
    ...
    /**
     * Looks up CEs for s and appends them to the ces array. Does not handle normalization: s should
     * be in FCD form.
     *
     * <p>Does not write completely ignorable CEs. Does not write beyond
    ...
    /**
     * Build-time context and CE32 for a code point. If a code point has contextual mappings, then
     * the default (no-context) mapping and all conditional mappings are stored in a singly-linked
     * list of ConditionalCE32, sorted by context strings.
     *
     * <p>Context strings sort by prefix length, then by prefix, then by contraction suffix. Context
     * strings must be unique and in ascending order.
     */
    private static final class ConditionalCE32 {
         ...
         *
         * <p>Otherwise one unit with the length of the prefix string, then the prefix string, then
         * the contraction suffix.
         */
        String context;

        /**
         * CE32 for the code point and its context. Can be special (e.g., for an expansion) but not
         * contextual (prefix or contraction tag).
         */
        int ce32;

        /**
         * Default CE32 for all contexts with this same prefix. Initially NO_CE32. Set only while
         * building runtime data structures, and only on one of the nodes of a sub-list with the
         * same prefix.
         */
        int defaultCE32;

        /**
         * CE32 for the built contexts. When fetching CEs from the builder, the contexts are built
         * into their runtime form so that the normal collation implementation can process them. The
         * result is cached in the list head. It is reset when the contexts are modified. All of
         * these builtCE32 are invalidated by clearContexts(), via incrementing the contextsEra.
         */
        int builtCE32;

        /**
         * The "era" of building intermediate contexts when the above builtCE32 was set. When the
         * array of cached, temporary contexts overflows, then clearContexts() removes them all and
         * invalidates the builtCE32 that used to point to built tries.
         */
        int era = -1;
        ...
        for (int i = 0; i <= ce64sMax; ++i) {
            if (first == ce64s.elementAti(i)) {
                ...
                for (int j = 1; ; ++j) {
                    ...
                    if (ce64s.elementAti(i + j) != ces[start + j]) {
                        break;
                    }
                }
            }
        ...
        for (int i = 0; i <= ce32sMax; ++i) {
            if (first == ce32s.elementAti(i)) {
                ...
                for (int j = 1; ; ++j) {
                    ...
                    if (ce32s.elementAti(i + j) != newCE32s[start + j]) {
                        break;
                    }
                }
            }
        ...
        switch (Collation.tagFromCE32(ce32)) {
            ...
            case Collation.PREFIX_TAG:
                {
                    ...
                    ConditionalCE32 cond =
                            getConditionalCE32(index); // the last ConditionalCE32 so far
                    CharsTrie.Iterator prefixes =
                            CharsTrie.iterator(base.contexts, trieIndex + 2, 0);
                    while (prefixes.hasNext()) {
                        CharsTrie.Entry entry = prefixes.next();
                        context.setLength(0);
                        context.append(entry.chars)
                                .reverse()
                                .insert(0, (char) entry.chars.length());
                        ce32 = entry.value;
                        if (Collation.isContractionCE32(ce32)) {
            ...
            default:
                throw new AssertionError(
                        "copyFromBaseCE32(c, ce32, withContext) "
    ...
    /**
     * Copies base contractions to a list of ConditionalCE32. Sets cond.next to the index of the
     * first new item and returns the index of the last new item.
     */
    protected int copyContractionsFromBaseCE32(
            StringBuilder context, int c, int ce32, ConditionalCE32 cond) {
        int trieIndex = Collation.indexFromCE32(ce32);
        int index;
    ...
    private static final class CopyHelper {
        CopyHelper(
                CollationDataBuilder s, CollationDataBuilder d, CollationDataBuilder.CEModifier m) {
            src = s;
            dest = d;
            modifier = m;
        }
        ...
        int copyCE32(int ce32) {
            if (!Collation.isSpecialCE32(ce32)) {
                ...
                if (tag == Collation.EXPANSION32_TAG) {
                    ...
                    int destIndex = dest.addConditionalCE32(cond.context, copyCE32(cond.ce32));
                    ...
                    while (cond.next >= 0) {
                        ...
                        destIndex = dest.addConditionalCE32(cond.context, copyCE32(cond.ce32));
                    ...
                    // Just copy long CEs and Latin mini expansions (and other expected values) as
                    // is,
            ...
            if (Collation.isSpecialCE32(ce32)) {
                switch (Collation.tagFromCE32(ce32)) {
                    ...
                    case Collation.LEAD_SURROGATE_TAG:
                        throw new AssertionError(
                                String.format("unexpected special tag in ce32=0x%08x", ce32));
                }
            }
            jamoCE32s[j] = ce32;
        }
        if (anyJamoAssigned && needToCopyFromBase) {
            for (int j = 0; j < CollationData.JAMO_CE32S_LENGTH; ++j) {
                if (jamoCE32s[j] == Collation.FALLBACK_CE32) {
                    int jamo = jamoCpFromIndex(j);
                    jamoCE32s[j] =
                            copyFromBaseCE32(jamo, base.getCE32(jamo), /* withContext= */ true);
                }
            }
        }
            ...
            if (ce32 != Collation.FALLBACK_CE32 && ce32 != Collation.UNASSIGNED_CE32) {
                ...
                ce32 =
                        Collation.makeCE32FromTagIndexAndLength(
                                Collation.DIGIT_TAG,
                                index,
                                UCharacter.digit(c)); // u_charDigitValue(c)
                trie.set(c, ce32);
            }
        }
    }
    ...
    protected void setLeadSurrogates() {
        for (char lead = 0xd800; lead < 0xdc00; ++lead) {
            ...
            trie.setForLeadSurrogateCodeUnit(
                    lead,
    ...
    /**
     * Build-time collation element and character iterator. Uses the runtime CollationIterator for
     * fetching CEs for a string but reads from the builder's unfinished data structures. In
     * particular, this class reads from the unfinished trie and has to avoid
     * CollationIterator.nextCE() and redirect other calls to data.getCE32() and
     * data.getCE32FromSupplementary().
     *
     * <p>We do this so that we need not implement the collation algorithm again for the builder and
     * make it behave exactly like the runtime code. That would be more difficult to test and
     * maintain than this indirection.
     *
     * <p>Some CE32 tags (for example, the DIGIT_TAG) do not occur in the builder data, so the data
     * accesses from those code paths need not be modified.
     *
     * <p>This class iterates directly over whole code points so that the CollationIterator does not
     * need the finished trie for handling the LEAD_SURROGATE_TAG.
     */
    private static final class DataBuilderCollationIterator extends CollationIterator {
        DataBuilderCollationIterator(CollationDataBuilder b, CollationData newData) {
            ...
            for (int j = 0; j < CollationData.JAMO_CE32S_LENGTH; ++j) { // Count across Jamo types.
                ...
                jamoCE32s[j] =
                        Collation.makeCE32FromTagAndIndex(Collation.BUILDER_DATA_TAG, jamo)
        ...
        protected int getCE32FromBuilderData(int ce32) {
            ...
            if ((ce32 & CollationDataBuilder.IS_BUILDER_JAMO_CE32) != 0) {
                ...
                if (cond.builtCE32 == Collation.NO_CE32 || cond.era != builder.contextsEra) {
                    // Build the context-sensitive mappings into their runtime form and cache the
                    // result.
                    try {
    ...
    /**
     * The "era" of building intermediate contexts. When the array of cached, temporary contexts
     * overflows, then clearContexts() removes them all and invalidates the builtCE32 that used to
     * point to built tries. See {@link ConditionalCE32#era}.
     */
