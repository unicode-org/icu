...
/**
 * Allocates n collation element weights between two exclusive limits. Used only internally by the
 * collation tailoring builder.
 */
public final class CollationWeights {
    ...
    /**
     * Determine heuristically what ranges to use for a given number of weights between (excluding)
     * two limits.
     *
     * @param lowerLimit A collation element weight; the ranges will be filled to cover weights
     *     greater than this one.
     * @param upperLimit A collation element weight; the ranges will be filled to cover weights less
     *     than this one.
    ...
    /**
     * Given a set of ranges calculated by allocWeights(), iterate through the weights. The ranges
     * are modified to keep the current iteration state.
     *
    ...
    private long incWeightByOffset(long weight, int length, int offset) {
        for (; ; ) {
            ...
            if (offset <= maxBytes[length]) {
                ...
                weight =
                        setWeightByte(
                                weight, length, minBytes[length] + offset % countBytes(length));
    ...
    /**
     * Takes two CE weights and calculates the possible ranges of weights between the two limits,
     * excluding them. For weights with up to 4 bytes there are up to 2*4-1=7 ranges.
     */
    private boolean getWeightRanges(long lowerLimit, long upperLimit) {
        ...
        WeightRange[] lower =
                new WeightRange[5]; /* [0] and [1] are not used - this simplifies indexing */
                    ...
                    if (lowerEnd > upperStart) {
                        ...
                        lower[length].count =
                                getWeightTrail(lower[length].end, length)
                                        - getWeightTrail(lower[length].start, length)
                                        + 1;
    ...
    private boolean allocWeightsInMinLengthRanges(int n, int minLength) {
        ...
        for (minLengthRangeCount = 0;
                minLengthRangeCount < rangeCount && ranges[minLengthRangeCount].length == minLength;
                ++minLengthRangeCount) {
