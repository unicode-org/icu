...
final class CollationFastLatinBuilder {
    ...
    private void getCEs(CollationData data) {
        ...
        for (char c = 0; ; ++i, ++c) {
            ...
            if (c == 0 && !isContractionCharCE(ce0)) {
                ...
                // Write a contraction list with only a default value if there is no real
                // contraction.
    ...
    private boolean getCEsFromCE32(CollationData data, int c, int ce32) {
        ...
        if (((ce0 | ce1) & Collation.QUATERNARY_MASK) != 0) {
            return false;
        }
        return true;
    }
    ...
    private void addUniqueCE(long ce) {
        if (ce == 0 || (ce >>> 32) == Collation.NO_CE_PRIMARY) {
            return;
        }
                ...
                if (p < firstShortPrimary) {
                    ...
                    if (pri < CollationFastLatin.MIN_SHORT) {
                        pri = CollationFastLatin.MIN_SHORT;
                            ...
                            < (CollationFastLatin.MAX_SHORT - CollationFastLatin.SHORT_INC)) {
    ...
    private void encodeContractions() {
        ...
        for (int i = 0; i < CollationFastLatin.NUM_FAST_CHARS; ++i) {
            ...
            for (int index = (int) ce & 0x7fffffff; ; index += 3) {
                ...
                if (x == CollationFastLatin.CONTR_CHAR_MASK && !firstTriple) {
                    break;
                }
            ...
            // However, that seems unlikely.
            result.setCharAt(
                    headerLength + i, (char) (CollationFastLatin.CONTRACTION | contractionIndex));
        }
    ...
    private int encodeTwoCEs(long first, long second) {
        ...
        if (miniCE1 == CollationFastLatin.BAIL_OUT) {
            return miniCE1;
        }
        ...
        if (miniCE >= CollationFastLatin.MIN_SHORT
            ...
            if (sec1 >= CollationFastLatin.MIN_SEC_HIGH
                    && case1 == 0
        ...
        if (miniCE1 <= CollationFastLatin.SECONDARY_MASK
                || CollationFastLatin.MIN_SHORT <= miniCE1) {
