...
final class CollationDataReader /* all static */ {
     ...
     *
     * <p>Can be 2 if there are only options. Can be 7 or 8 if there are only options and a script
     * reordering. The loader treats any index>=indexes[IX_INDEXES_LENGTH] as 0.
     */
    static final int IX_INDEXES_LENGTH = 0;

    /**
     * Bits 31..24: numericPrimary, for numeric collation 23..16: fast Latin format version (0 = no
     * fast Latin table) 15.. 0: options bit set
     */
    ...
    /**
     * Byte offset to uint8_t reorderTable[]. Empty table if <256 bytes (padding only). Otherwise
     * 256 bytes or more (with padding).
     */
    ...
    /**
     * Byte offset to boolean compressibleBytes[]. Empty table if <256 bytes (padding only).
    ...
    static void read(CollationTailoring base, ByteBuffer inBytes, CollationTailoring tailoring)
            throws IOException {
        ...
        if (length >= 4) {
            ...
            while (reorderRangesLength < reorderCodesLength
                    && (reorderCodes[reorderCodesLength - reorderRangesLength - 1] & 0xffff0000)
                            != 0) {
                ++reorderRangesLength;
            }
        ...
        if (jamoCE32sStart >= 0) {
            ...
            System.arraycopy(
                    data.ce32s, jamoCE32sStart, data.jamoCE32s, 0, CollationData.JAMO_CE32S_LENGTH);
        } else if (data == null) {
            // Nothing to do.
        } else if (baseData != null) {
            ...
            if (((inIndexes[IX_OPTIONS] >> 16) & 0xff) == CollationFastLatin.VERSION) {
                if (length >= 2) {
                    ...
                    if ((header0 >> 8) != CollationFastLatin.VERSION) {
                        throw new ICUException(
                                "Fast-Latin table version differs from version in data header");
                    }
                } else if (baseData != null) {
        ...
        int fastLatinOptions =
                CollationFastLatin.getOptions(tailoring.data, ts, fastLatinPrimaries);
        ...
        settings.variableTop =
                tailoring.data.getLastPrimaryForGroup(
        ...
        settings.fastLatinOptions =
                CollationFastLatin.getOptions(
                        tailoring.data, settings, settings.fastLatinPrimaries);
    }
