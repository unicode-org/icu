...
/**
 * AlphabeticIndex supports the creation of a UI index appropriate for a given language. It can
 * support either direct use, or use with a client that doesn't support localized collation. The
 * following is an example of what an index might look like in a UI:
 *
 * <pre>
 ...
 * </pre>
 *
 * The class can generate a list of labels for use as a UI "index", that is, a list of clickable
 * characters (or character sequences) that allow the user to see a segment (bucket) of a larger
 * "target" list. That is, each label corresponds to a bucket in the target list, where everything
 * in the bucket is greater than or equal to the character (according to the locale's collation).
 * Strings can be added to the index; they will be in sorted order in the right bucket.
 *
 * <p>The class also supports having buckets for strings before the first (underflow), after the
 * last (overflow), and between scripts (inflow). For example, if the index is constructed with
 * labels for Russian and English, Greek characters would fall into an inflow bucket between the
 * other two scripts.
 *
 * <p><em>Note:</em> If you expect to have a lot of ASCII or Latin characters as well as characters
 * from the user's language, then it is a good idea to call addLabels(ULocale.English).
 *
 * <h2>Direct Use</h2>
 *
 * <p>The following shows an example of building an index directly. The "show..." methods below are
 ...
 * </pre>
 *
 * The caller can build different UIs using this class. For example, an index character could be
 * omitted or grayed-out if its bucket is empty. Small buckets could also be combined based on size,
 * such as:
 *
 * <pre>
 * <b>... A-F G-N O-Z ...</b>
 ...
 *
 * <p>Callers can also use the {@link AlphabeticIndex.ImmutableIndex}, or the AlphabeticIndex
 * itself, to support sorting on a client that doesn't support AlphabeticIndex functionality.
 *
 * <p>The ImmutableIndex is both immutable and thread-safe. The corresponding AlphabeticIndex
 * methods are not thread-safe because they "lazily" build the index buckets.
 *
 * <ul>
 *   <li>ImmutableIndex.getBucket(index) provides random access to all buckets and their labels and
 *       label types.
 *   <li>AlphabeticIndex.getBucketLabels() or the bucket iterator on either class can be used to get
 *       a list of the labels, such as "...", "A", "B",..., and send that list to the client.
 *   <li>When the client has a new name, it sends that name to the server. The server needs to call
 *       the following methods, and communicate the bucketIndex and collationKey back to the client.
 *       <pre>
 ...
 * </pre>
 *   <li>The client would put the name (and associated information) into its bucket for bucketIndex.
 *       The collationKey is a sequence of bytes that can be compared with a binary compare, and
 *       produce the right localized result.
 * </ul>
 *
 * @author Mark Davis
 * @stable ICU 4.8
...
public final class AlphabeticIndex<V> implements Iterable<Bucket<V>> {
    /**
     * Prefix string for Chinese index buckets. See
     * http://unicode.org/repos/cldr/trunk/specs/ldml/tr35-collation.html#Collation_Indexes
     */
    ...
    /**
     * Immutable, thread-safe version of {@link AlphabeticIndex}. This class provides thread-safe
     * methods for bucketing, and random access to buckets and their properties, but does not offer
     * adding records to the index.
     *
     * @param <V> The Record value type is unused. It can be omitted for this class if it was
     *     omitted for the AlphabeticIndex that built it.
     * @stable ICU 51
     */
    public static final class ImmutableIndex<V> implements Iterable<Bucket<V>> {
        ...
        /**
         * Finds the index bucket for the given name and returns the number of that bucket. Use
         * {@link #getBucket(int)} to get the bucket's properties.
         *
     ...
     *
     * <p>The index will be created with no labels; the addLabels() function must be called after
     * creation to add the desired labels to the index.
     *
     * <p>The index will work directly with the supplied collator. If the caller will need to
     * continue working with the collator it should be cloned first, so that the collator provided
     * to the AlphabeticIndex remains unchanged after creation of the index.
     *
    ...
    private AlphabeticIndex(ULocale locale, RuleBasedCollator collator) {
        collatorOriginal =
                collator != null ? collator : (RuleBasedCollator) Collator.getInstance(locale);
        try {
    ...
    /**
     * Get the default label used for abbreviated buckets <i>between</i> other labels. For example,
     * consider the labels for Latin and Greek are used: X Y Z ... &#x0391; &#x0392; &#x0393;.
     *
     * @return inflow label
     * @stable ICU 4.8
     */
    ...
    /**
     * Get the limit on the number of labels in the index. The number of buckets can be slightly
     * larger: see getBucketCount().
     *
    ...
    /**
     * Set a limit on the number of labels in the index. The number of buckets can be slightly
     * larger: see getBucketCount().
     *
     * @param maxLabelCount Set the maximum number of labels. Currently, if the number is exceeded,
     *     then every nth item is removed to bring the count down. A more sophisticated mechanism
    ...
    /**
     * Determine the best labels to use. This is based on the exemplars, but we also process to make
     * sure that they are unique, and sort differently, and that the overall list is small enough.
     */
    private List<String> initLabels() {
        ...
        for (String item : initialLabels) {
            ...
            if (collatorPrimaryOnly.compare(item, firstScriptBoundary) < 0) {
                ...
                int insertionPoint =
                        Collections.binarySearch(indexCharacters, item, collatorPrimaryOnly);
                if (insertionPoint < 0) {
        ...
        // if the result is still too large, cut down to maxLabelCount elements, by removing every
        // nth element
    ...
    /**
     * This method is called to get the index exemplars. Normally these come from the locale
     * directly, but if they aren't available, we have to synthesize them.
     */
    ...
    /**
     * Get a clone of the collator used internally. Note that for performance reasons, the clone is
     * only done once, and then stored. The next time it is accessed, the same instance is returned.
     *
     * <p><b><i>Don't use this method across threads if you are changing the settings on the
     * collator, at least not without synchronizing.</i></b>
     *
    ...
    /**
     * Add a record (name and data) to the index. The name will be used to sort the items into
     * buckets, and to sort within the bucket. Two records may have the same name. When they do, the
     * sort order is according to the order added: the first added comes first.
     *
     * @param name Name, such as a name
     * @param data Data, such as an address or link
     * @return this, for chaining
     * @stable ICU 4.8
     */
    ...
    /**
     * Get the bucket number for the given name. This routine permits callers to implement their own
     * bucket handling mechanisms, including client-server handling. For example, when a new name is
     * created on the client, it can ask the server for the bucket for that name, and the sortkey
     * (using getCollator). Once the client has that information, it can put the name into the right
     * bucket, and sort it within that bucket, without having access to the index or collator.
     *
     * <p>Note that the bucket number (and sort key) are only valid for the settings of the current
     * AlphabeticIndex; if those are changed, then the bucket number and sort key must be
     * regenerated.
     *
     * @param name Name, such as a name
    ...
    /**
     * Return the number of buckets in the index. This will be the same as the number of labels,
     * plus buckets for the underflow, overflow, and inflow(s).
     *
     ...
     * &lt;name,data&gt; pairs added with addRecord(...), over all the buckets.
     *
    ...
    private void initBuckets() {
        ...
        // This makes the process order n*log(n), since we just sort the list and then do a linear
        // process.
        // However, if the user adds an item at a time and then gets the buckets, this isn't
        // efficient, so
    ...
    /**
     * Returns true if one index character string is "better" than the other. Shorter NFKD is
     * better, and otherwise NFKD-binary-less-than is better, and otherwise binary-less-than is
     * better.
     */
    private static boolean isOneLabelBetterThanOther(
            Normalizer2 nfkdNormalizer, String one, String other) {
    ...
    /**
     * An index "bucket" with a label string and type. It is referenced by {@link
     * AlphabeticIndex#getBucketIndex(CharSequence)} and {@link
     * AlphabeticIndex.ImmutableIndex#getBucketIndex(CharSequence)}, returned by {@link
     * AlphabeticIndex.ImmutableIndex#getBucket(int)}, and {@link
     * AlphabeticIndex#addRecord(CharSequence, Object)} adds a record into a bucket according to the
     * record's name.
     *
     * @param <V> Data type
     * @stable ICU 4.8
    ...
    public static class Bucket<V> implements Iterable<Record<V>> {
         ...
         * @param labelType is an underflow, overflow, or inflow bucket
         * @stable ICU 4.8
         */
    ...
    private BucketList<V> createBucketList() {
        ...
        for (String current : indexCharacters) {
            if (collatorPrimaryOnly.compare(current, scriptUpperBoundary) >= 0) {
                ...
                if (skippedScript && bucketList.size() > 1) {
                    ...
                    bucketList.add(
                            new Bucket<V>(getInflowLabel(), inflowBoundary, LabelType.INFLOW));
                }
            }
            ...
            } else if (current.length() == BASE.length() + 1
                    && current.startsWith(BASE)
                    && 'A' <= (c = current.charAt(BASE.length()))
                    && c <= 'Z') {
                ...
                for (int i = bucketList.size() - 2; ; --i) {
                    ...
                    if (singleBucket.displayBucket == null
                            && !hasMultiplePrimaryWeights(
                                    collatorPrimaryOnly, variableTop, singleBucket.lowerBoundary)) {
        ...
        bucketList.add(
                new Bucket<V>(
                        getOverflowLabel(), scriptUpperBoundary, LabelType.OVERFLOW)); // final
