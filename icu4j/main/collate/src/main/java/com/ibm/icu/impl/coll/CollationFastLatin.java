...
public final class CollationFastLatin /* all static */ {
    /**
     * Fast Latin format version (one byte 1..FF). Must be incremented for any runtime-incompatible
     * changes, in particular, for changes to any of the following constants.
     *
     * <p>When the major version number of the main data format changes, we can reset this fast
     * Latin version to 1.
     */
    ...
    static final int TWO_SECONDARIES_MASK = (SECONDARY_MASK << 16) | SECONDARY_MASK; // 0x3e003e0
    static final int TWO_CASES_MASK = (CASE_MASK << 16) | CASE_MASK; // 0x180018
    static final int TWO_TERTIARIES_MASK = (TERTIARY_MASK << 16) | TERTIARY_MASK; // 0x70007

    /**
     * Contraction with one fast Latin character. Use INDEX_MASK to find the start of the
     * contraction list after the fixed table. The first entry contains the default mapping.
     * Otherwise use CONTR_CHAR_MASK for the contraction character index (in ascending order). Use
     * CONTR_LENGTH_SHIFT for the length of the entry (1=BAIL_OUT, 2=one CE, 3=two CEs).
     *
     * <p>Also, U+0000 maps to a contraction entry, so that the fast path need not check for NUL
     * termination. It usually maps to a contraction list with only the completely ignorable default
     * value.
     */
    static final int CONTRACTION = 0x400;

    /**
     * An expansion encodes two CEs. Use INDEX_MASK to find the pair of CEs after the fixed table.
     *
     * <p>The higher a mini CE value, the easier it is to process. For expansions and higher, no
     * context needs to be considered.
     */
    static final int EXPANSION = 0x800;

    /**
     * Encodes one CE with a long/low mini primary (there are 128). All potentially-variable
     * primaries must be in this range, to make the short-primary path as fast as possible.
     */
    ...
    /**
     * Encodes one CE with a short/high primary (there are 60), plus a secondary CE if the secondary
     * weight is high. Fast handling: At least all letter primaries should be in this range.
     */
    ...
    /**
     * Lookup: Add this offset to secondary weights, except for completely ignorable CEs. Must be
     * greater than any special value, e.g., MERGE_WEIGHT. The exact value is not relevant for the
     * format version.
     */
    ...
    static final int TWO_SEC_OFFSETS = (SEC_OFFSET << 16) | SEC_OFFSET; // 0x200020
    ...
    /**
     * Lookup: Add this offset to tertiary weights, except for completely ignorable CEs. Must be
     * greater than any special value, e.g., MERGE_WEIGHT. Must be greater than case bits as well,
     * so that with combined case+tertiary weights plus the offset the tertiary bits does not spill
     * over into the case bits. The exact value is not relevant for the format version.
     */
    ...
    /**
     * Contraction result first word bits 8..0 contain the second contraction character, as a char
     * index 0..NUM_FAST_CHARS-1. Each contraction list is terminated with a word containing
     * CONTR_CHAR_MASK.
     */
    static final int CONTR_CHAR_MASK = 0x1ff;

    /**
     * Contraction result first word bits 10..9 contain the result length: 1=bail out, 2=one mini
     * CE, 3=two mini CEs
     */
    static final int CONTR_LENGTH_SHIFT = 9;

    /**
     * Comparison return value when the regular comparison must be used. The exact value is not
     * relevant for the format version.
     */
    ...
    /**
     * Computes the options value for the compare functions and writes the precomputed primary
     * weights. Returns -1 if the Latin fastpath is not supported for the data and settings. The
     * capacity must be LATIN_LIMIT.
     */
            ...
            for (; ; ) {
                ...
                if (leftSecondary != rightSecondary) {
                    if ((options & CollationSettings.BACKWARD_SECONDARY) != 0) {
                        // Full support for backwards secondary requires backwards contraction
                        // matching
    ...
    /**
     * Java returns a negative result (use the '~' operator) if sIndex is to be incremented. C++
     * modifies sIndex.
     */
