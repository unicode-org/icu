...
/**
 * Collation data container. Immutable data created by a CollationDataBuilder, or loaded from a
 * file, or deserialized from API-provided binary data.
 *
 * <p>Includes data for the collation base (root/default), aliased if this is not the base.
 */
public final class CollationData {
    ...
    /**
     * Returns the CE32 from two contexts words. Access to the defaultCE32 for contraction and
     * prefix matching.
     */
    ...
    /**
     * Returns the CE32 for an indirect special CE32 (e.g., with DIGIT_TAG). Requires that ce32 is
     * special.
     */
    ...
    /**
     * Returns the single CE that c maps to. Throws UnsupportedOperationException if c does not map
     * to a single CE.
     */
            ...
            switch (Collation.tagFromCE32(ce32)) {
                ...
                case Collation.RESERVED_TAG_3:
                    throw new AssertionError(
                            String.format("unexpected CE32 tag for U+%04X (CE32 0x%08x)", c, ce32));
     ...
     *
     * @return the primary with only the first primary lead byte of the group (not necessarily an
     *     actual root collator primary weight), or 0 if the script is unknown
     */
     ...
     *
     * @return the last primary of the group (not an actual root collator primary weight), or 0 if
    ...
    /**
     * Writes the permutation of primary-weight ranges for the given reordering of scripts and
     * groups. The caller checks for illegal arguments and takes care of [DEFAULT] and memory
     * allocation.
     *
     * <p>Each list element will be a (limit, offset) pair as described for the
     * CollationSettings.reorderRanges. The list will be empty if no ranges are reordered.
     */
    ...
    private void makeReorderRanges(int[] reorder, boolean latinMustMove, UVector32 ranges) {
        ...
        for (int i = 0; i < length; ) {
            ...
            if (script == Collator.ReorderCodes.DEFAULT) {
                // The default code must be the only one in the list, and that is handled by the
                // caller.
    ...
    /**
     * Array of CE32 values. At index 0 there must be CE32(U+0000) to support U+0000's special-tag
     * for NUL-termination handling.
     */
    int[] ce32s;
    ...
    /**
     * Simple array of JAMO_CE32S_LENGTH=19+21+27 CE32s, one per canonical Jamo L/V/T. They are
     * normally simple CE32s, rarely expansions. For fast handling of HANGUL_TAG.
     */
    ...
    /**
     * Fast Latin table for common-Latin-text string comparisons. Data structure see class
     * CollationFastLatin.
     */
    public char[] fastLatinTable;

    /**
     * Header portion of the fastLatinTable. In C++, these are one array, and the header is skipped
     * for mapping characters. In Java, two arrays work better.
     */
    char[] fastLatinTableHeader;

    /**
     * Data for scripts and reordering groups. Uses include building a reordering permutation table
     * and providing script boundaries to AlphabeticIndex.
     */
    int numScripts;

    /**
     * The length of scriptsIndex is numScripts+16. It maps from a UScriptCode or a special reorder
     * code to an entry in scriptStarts. 16 special reorder codes (not all used) are mapped starting
     * at numScripts. Up to MAX_NUM_SPECIAL_REORDER_CODES are codes for special groups like
     * space/punct/digit. There are special codes at the end for reorder-reserved primary ranges.
     *
    ...
    /**
     * Start primary weight (top 16 bits only) for a group/script/reserved range indexed by
     * scriptsIndex. The first range (separators & terminators) and the last range (trailing
     * weights) are not reorderable, and no scriptsIndex entry points to them.
     */
    char[] scriptStarts;

    /**
     * Collation elements in the root collator. Used by the CollationRootElements class. The data
     * structure is described there. null in a tailoring.
     */
