 ...
 *
 * <p>Data structures except for expansion tables store 32-bit CEs which are either specials (see
 * tags below) or are compact forms of 64-bit CEs.
 */
public final class Collation {
    ...
    /**
     * Merge-sort-key separator. Same as the unique primary and identical-level weights of U+FFFE.
     * Must not be used as primary compression low terminator. Otherwise usable.
     */
    ...
    /**
     * Primary compression low terminator, must be greater than MERGE_SEPARATOR_BYTE. Reserved value
     * in primary second byte if the lead byte is compressible. Otherwise usable in all CE weight
     * bytes.
     */
    public static final int PRIMARY_COMPRESSION_LOW_BYTE = 3;

    /**
     * Primary compression high terminator. Reserved value in primary second byte if the lead byte
     * is compressible. Otherwise usable in all CE weight bytes.
     */
    ...
    /**
     * First unassigned: AlphabeticIndex overflow boundary. We want a 3-byte primary so that it fits
     * into the root elements table.
     *
     * <p>This 3-byte primary will not collide with any unassigned-implicit 4-byte primaries because
    ...
    /**
     * A CE32 is special if its low byte is this or greater. Impossible case bits 11 mark special
     * CE32s. This value itself is used to indicate a fallback to the base collator.
     */
    ...
    /**
     * Sort key level flags: xx_FLAG = 1 << xx_LEVEL. In Java, use enum Level with flag() getters,
     * or use EnumSet rather than hand-made bit sets.
     */
    ...
    /**
     * Fall back to the base collator. This is the tag value in SPECIAL_CE32_LOW_BYTE and
     * FALLBACK_CE32. Bits 31..8: Unused, 0.
     */
    ...
    /**
     * Long-secondary CE with zero primary. Bits 31..16: Secondary weight. Bits 15.. 8: Tertiary
     * weight.
     */
    static final int LONG_SECONDARY_TAG = 2;

    /**
     * Unused. May be used in the future for single-byte secondary CEs (SHORT_SECONDARY_TAG),
     * storing the secondary in bits 31..24, the ccc in bits 23..16, and the tertiary in bits 15..8.
     */
    static final int RESERVED_TAG_3 = 3;

    /**
     * Latin mini expansions of two simple CEs [pp, 05, tt] [00, ss, 05]. Bits 31..24: Single-byte
     * primary weight pp of the first CE. Bits 23..16: Tertiary weight tt of the first CE. Bits 15..
     * 8: Secondary weight ss of the second CE.
     */
    static final int LATIN_EXPANSION_TAG = 4;

    /**
     * Points to one or more simple/long-primary/long-secondary 32-bit CE32s. Bits 31..13: Index
     * into int table. Bits 12.. 8: Length=1..31.
     */
    static final int EXPANSION32_TAG = 5;

    /**
     * Points to one or more 64-bit CEs. Bits 31..13: Index into CE table. Bits 12.. 8:
     * Length=1..31.
     */
     ...
     *
     * <p>If bit 8 is 0: Builder context, points to a list of context-sensitive mappings. Bits
     * 31..13: Index to the builder's list of ConditionalCE32 for this character. Bits 12.. 9:
     * Unused, 0.
     *
     * <p>If bit 8 is 1 (IS_BUILDER_JAMO_CE32): Builder-only jamoCE32 value. The builder fetches the
     * Jamo CE32 from the trie. Bits 31..13: Jamo code point. Bits 12.. 9: Unused, 0.
     */
    static final int BUILDER_DATA_TAG = 7;

    /**
     * Points to prefix trie. Bits 31..13: Index into prefix/contraction data. Bits 12.. 8: Unused,
     * 0.
     */
    static final int PREFIX_TAG = 8;

    /**
     * Points to contraction data. Bits 31..13: Index into prefix/contraction data. Bits 12..11:
     * Unused, 0. Bit 10: CONTRACT_TRAILING_CCC flag. Bit 9: CONTRACT_NEXT_CCC flag. Bit 8:
     * CONTRACT_SINGLE_CP_NO_MATCH flag.
     */
    static final int CONTRACTION_TAG = 9;

    /**
     * Decimal digit. Bits 31..13: Index into int table for non-numeric-collation CE32. Bit 12:
     * Unused, 0. Bits 11.. 8: Digit value 0..9.
     */
    static final int DIGIT_TAG = 10;

    /**
     * Tag for U+0000, for moving the NUL-termination handling from the regular fastpath into
     * specials-handling code. Bits 31..8: Unused, 0.
     */
    ...
    /**
     * Tag for a lead surrogate code unit. Optional optimization for UTF-16 string processing. Bits
     * 31..10: Unused, 0. 9.. 8: =0: All associated supplementary code points are
     * unassigned-implicit. =1: All associated supplementary code points fall back to the base data.
     * else: (Normally 2) Look up the data for the supplementary code point.
     */
    static final int LEAD_SURROGATE_TAG = 13;

    /**
     * Tag for CEs with primary weights in code point order. Bits 31..13: Index into CE table, for
     * one data "CE". Bits 12.. 8: Unused, 0.
     *
     * <p>This data "CE" has the following bit fields: Bits 63..32: Three-byte primary pppppp00.
     * 31.. 8: Start/base code point of the in-order range. 7: Flag isCompressible primary. 6.. 0:
     * Per-code point primary-weight increment.
     */
    static final int OFFSET_TAG = 14;

    /**
     * Implicit CE tag. Compute an unassigned-implicit CE. All bits are set
     * (UNASSIGNED_CE32=0xffffffff).
     */
    ...
    /**
     * We limit the number of CEs in an expansion so that we can use a small number of length bits
     * in the data structure, and so that an implementation can copy CEs at runtime without growing
    ...
    /**
     * Set if there is no match for the single (no-suffix) character itself. This is only possible
     * if there is a prefix. In this case, discontiguous contraction matching cannot add combining
     * marks starting from an empty suffix. The default CE32 is used anyway if there is no suffix
     * match.
     */
    ...
    static int makeLongPrimaryCE32(long p) {
        return (int) (p | LONG_PRIMARY_CE32_LOW_BYTE);
    }
    ...
    static boolean ce32HasContext(int ce32) {
        return isSpecialCE32(ce32)
                && (tagFromCE32(ce32) == PREFIX_TAG || tagFromCE32(ce32) == CONTRACTION_TAG);
    }
    ...
    static long ceFromSimpleCE32(int ce32) {
        ...
        return ((long) (ce32 & 0xffff0000) << 32)
                | ((long) (ce32 & 0xff00) << 16)
                | ((ce32 & 0xff) << 8);
    }
    ...
    /**
     * Creates a CE from a primary weight, 16-bit secondary/tertiary weights, and a 2-bit
     * quaternary.
     */
    ...
    public static long incTwoBytePrimaryByOffset(
            long basePrimary, boolean isCompressible, int offset) {
