...
/**
 * Port From: ICU4C v2.1 : collate/CollationMonkeyTest Source File:
 * $ICU4CRoot/source/test/intltest/mnkytst.cpp
 */
...
/**
 * CollationFrozenMonkeyTest is a third level test class. This tests the random substrings of the
 * default test strings to verify if the compare and sort key algorithm works correctly on frozen
 * collators. For example, any string is always less than the string itself appended with any
 * character.
 */
...
public class CollationFrozenMonkeyTest extends TestFmwk {
    ...
    public void TestCollationKey() {
        if (source.length() == 0) {
            errln(
                    "CollationMonkeyTest.TestCollationKey(): source is empty - ICU_DATA not set or data missing?");
            return;
        }
        ...
        int slen =
                Math.abs(rand.nextInt(0x7fff) % source.length() - source.length())
                        % source.length();
        int tlen =
                Math.abs(rand.nextInt(0x7fff) % source.length() - source.length())
                        % source.length();
    ...
    public void TestCompare() {
        if (source.length() == 0) {
            errln(
                    "CollationMonkeyTest.TestCompare(): source is empty - ICU_DATA not set or data missing?");
            return;
        }
