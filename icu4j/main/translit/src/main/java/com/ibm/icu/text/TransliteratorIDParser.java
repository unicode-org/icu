 ...
 * instantiated. Methods in this class parse various ID formats, including the following:
 *
 * <p>A basic ID, which contains source, target, and variant, but no filter and no explicit inverse.
 * Examples include "Latin-Greek/UNGEGN" and "Null".
 *
 * <p>A single ID, which is a basic ID plus optional filter and optional explicit inverse. Examples
 * include "[a-zA-Z] Latin-Greek" and "Lower (Upper)".
 *
 * <p>A compound ID, which is a sequence of one or more single IDs, separated by semicolons, with
 * optional forward and reverse global filters. The global filters are UnicodeSet patterns prepended
 * or appended to the IDs, separated by semicolons. An appended filter must be enclosed in
 * parentheses and applies in the reverse direction.
 *
 * @author Alan Liu
 */
class TransliteratorIDParser {
     ...
     * a filter.
     *
     * <p>'source' and 'target' will always be non-null. The 'variant' will be non-null only if a
     * non-empty variant was parsed.
     *
     * <p>'sawSource' is true if there was an explicit source in the parsed id. If there was no
     * explicit source, then an implied source of ANY is returned and 'sawSource' is set to false.
     *
     * <p>'filter' is the parsed filter pattern, or null if there was no filter.
     */
    ...
    /**
     * A structure containing the canonicalized data of a filtered ID, that is, a basic ID
     * optionally with a filter.
     *
     * <p>'canonID' is always non-null. It may be the empty string "". It is the id that should be
     * assigned to the created transliterator. It _cannot_ be instantiated directly.
     *
     * <p>'basicID' is always non-null and non-empty. It is always of the form S-T or S-T/V. It is
     * designed to be fed to low-level instantiation code that only understands these two formats.
     *
     * <p>'filter' may be null, if there is none, or non-null and non-empty.
     */
    ...
    /**
     * Parse a filter ID, that is, an ID of the general form "[f1] s1-t1/v1", with the filters
     * optional, and the variants optional.
     *
     * @param id the id to be parsed
     * @param pos INPUT-OUTPUT parameter. On input, the position of the first character to parse. On
     *     output, the position after the last character parsed.
    ...
    /**
     * Parse a single ID, that is, an ID of the general form "[f1] s1-t1/v1 ([f2] s2-t3/v2)", with
     * the parenthesized element optional, the filters optional, and the variants optional.
     *
     * @param id the id to be parsed
     * @param pos INPUT-OUTPUT parameter. On input, the position of the first character to parse. On
     *     output, the position after the last character parsed.
     * @param dir the direction. If the direction is REVERSE then the SingleID is constructed for
    ...
    public static SingleID parseSingleID(String id, int[] pos, int dir) {
        ...
        if (sawParen) {
            if (dir == FORWARD) {
                ...
                single.canonID =
                        single.canonID + OPEN_REV + specsToID(specsB, FORWARD).canonID + CLOSE_REV;
                if (specsA != null) {
     ...
     * @param withParens INPUT-OUTPUT parameter. On entry, if withParens[0] is 0, then parens are
     *     disallowed. If it is 1, then parens are requires. If it is -1, then parens are optional,
     *     and the return result will be set to 0 or 1.
     * @param canonID OUTPUT parameter. The pattern for the filter added to the canonID, either at
     *     the end, if dir is FORWARD, or at the start, if dir is REVERSE. The pattern will be
     *     enclosed in parentheses if appropriate, and will be suffixed with an ID_DELIM character.
     *     May be null.
     * @return a UnicodeSet object or null. A non-null results indicates a successful parse,
     *     regardless of whether the filter applies to the given direction. The caller should
     *     discard it if withParens != (dir == REVERSE).
     */
    public static UnicodeSet parseGlobalFilter(
            String id, int[] pos, int dir, int[] withParens, StringBuilder canonID) {
    ...
    /**
     * Parse a compound ID, consisting of an optional forward global filter, a separator, one or
     * more single IDs delimited by separators, an an optional reverse global filter. The separator
     * is a semicolon. The global filters are UnicodeSet patterns. The reverse global filter must be
     * enclosed in parentheses.
     *
     * @param id the pattern the parse
     * @param dir the direction.
     * @param canonID OUTPUT parameter that receives the canonical ID, consisting of canonical IDs
     *     for all elements, as returned by parseSingleID(), separated by semicolons. Previous
     *     contents are discarded.
     * @param list OUTPUT parameter that receives a list of SingleID objects representing the parsed
     *     IDs. Previous contents are discarded.
     * @param globalFilter OUTPUT parameter that receives a pointer to a newly created global filter
     *     for this ID in this direction, or null if there is none.
     * @return true if the parse succeeds, that is, if the entire id is consumed without syntax
     *     error.
     */
    public static boolean parseCompoundID(
            String id,
            int dir,
    ...
    /**
     * Parse an ID into pieces. Take IDs of the form T, T/V, S-T, S-T/V, or S/V-T. If the source is
     * missing, return a source of ANY.
     *
     * @param id the id string, in any of several forms
     * @return an array of 4 strings: source, target, variant, and isSourcePresent. If the source is
     *     not present, ANY will be given as the source, and isSourcePresent will be null. Otherwise
     *     isSourcePresent will be non-null. The target may be empty if the id is not well-formed.
    ...
    /**
     * Given source, target, and variant strings, concatenate them into a full ID. If the source is
     * empty, then "Any" will be used for the source, so the ID will always be of the form s-t/v or
     * s-t.
     */
     ...
     * Any-Target2. The target should have canonical casing (the casing desired to be produced when
     * an inverse is formed) and should contain no whitespace or other extraneous characters.
     *
     ...
     */
    public static void registerSpecialInverse(
            String target, String inverseTarget, boolean bidirectional) {
    ...
    /**
     * Parse an ID into component pieces. Take IDs of the form T, T/V, S-T, S-T/V, or S/V-T. If the
     * source is missing, return a source of ANY.
     *
     * @param id the id string, in any of several forms
     * @param pos INPUT-OUTPUT parameter. On input, pos[0] is the offset of the first character to
     *     parse in id. On output, pos[0] is the offset after the last parsed character. If the
     *     parse failed, pos[0] will be unchanged.
     * @param allowFilter if true, a UnicodeSet pattern is allowed at any location between specs or
     *     delimiters, and is returned as the fifth string in the array.
     * @return a Specs object, or null if the parse failed. If neither source nor target was seen in
     *     the parsed id, then the parse fails. If allowFilter is true, then the parsed filter
     *     pattern is returned in the Specs object, otherwise the returned filter reference is null.
     *     If the parse fails for any reason null is returned.
     */
    ...
    /**
     * Givens a Spec object, convert it to a SingleID object. The Spec object is a more unprocessed
     * parse result. The SingleID object contains information about canonical and basic IDs.
     *
    ...
    /**
     * Given a Specs object, return a SingleID representing the special inverse of that ID. If there
     * is no special inverse then return null.
     *
