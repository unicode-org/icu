    ...
    public void testConverter() {
        ...
        for (TestData test : tests) {
            UnitConverter converter = new UnitConverter(
                MeasureUnitImpl.UnitsParser.parseForIdentifier(test.source),
                MeasureUnitImpl.UnitsParser.parseForIdentifier(test.target), conversionRates);

            double maxDelta = 1e-6 * Math.abs(test.expected);
            if (test.expected == 0) {
                maxDelta = 1e-12;
            }
            assertEquals("testConverter: " + test.source + " to " + test.target, test.expected,
                         converter.convert(BigDecimal.valueOf(test.input)).doubleValue(), maxDelta);
        }
    }
