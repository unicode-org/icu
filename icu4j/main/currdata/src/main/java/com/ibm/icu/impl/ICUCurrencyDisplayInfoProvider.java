...
public class ICUCurrencyDisplayInfoProvider implements CurrencyDisplayInfoProvider {
    ...
    public CurrencyDisplayInfo getInstance(ULocale locale, boolean withFallback) {
        ...
        if (instance == null
                || !instance.locale.equals(locale)
                || instance.fallback != withFallback) {
    ...
    static class ICUCurrencyDisplayInfo extends CurrencyDisplayInfo {
        ...
        /**
         * Single-item cache for getName(), getSymbol(), and getFormatInfo(). Holds data for only
         * one currency. If another currency is requested, the old cache item is overwritten.
         */
        private volatile FormattingData formattingDataCache = null;

        /**
         * Single-item cache for variant symbols. Holds data for only one currency. If another
         * currency is requested, the old cache item is overwritten.
         */
         ...
         *
         * <p>array[0] is the ISO code.<br>
         * array[1+p] is the plural name where p=standardPlural.ordinal().
         *
         * <p>Holds data for only one currency. If another currency is requested, the old cache item
         * is overwritten.
         */
        ...
        FormattingData fetchFormattingData(String isoCode) {
            ...
            if (result == null || !result.isoCode.equals(isoCode)) {
                ...
                CurrencySink sink =
                        new CurrencySink(!fallback, CurrencySink.EntrypointTable.CURRENCIES);
        ...
        VariantSymbol fetchVariantSymbol(String isoCode, String variant) {
            ...
            if (result == null
                    || !result.isoCode.equals(isoCode)
                    || !result.variant.equals(variant)) {
                result = new VariantSymbol(isoCode, variant);
                CurrencySink sink =
                        new CurrencySink(!fallback, CurrencySink.EntrypointTable.CURRENCY_VARIANT);
        ...
        String[] fetchPluralsData(String isoCode) {
            ...
            if (result == null || !result[0].equals(isoCode)) {
                ...
                result[0] = isoCode;
                CurrencySink sink =
                        new CurrencySink(!fallback, CurrencySink.EntrypointTable.CURRENCY_PLURALS);
        ...
        Map<String, String> fetchUnitPatterns() {
            ...
            if (result == null) {
                ...
                CurrencySink sink =
                        new CurrencySink(
                                false, CurrencySink.EntrypointTable.CURRENCY_UNIT_PATTERNS);
        ...
        CurrencySpacingInfo fetchSpacingInfo() {
            ...
            if (result == null) {
                ...
                CurrencySink sink =
                        new CurrencySink(!fallback, CurrencySink.EntrypointTable.CURRENCY_SPACING);
        ...
        private static final class CurrencySink extends UResource.Sink {
                ...
                for (int i = 0; table.getKeyAndValue(i, key, value); i++) {
                    ...
                    if (value.getType() != UResourceBundle.ARRAY) {
                        throw new ICUException(
                                "Unexpected data type in Currencies table for " + isoCode);
                    }
                    UResource.Array array = value.getArray();

                    parsingData.symbolToIsoCode.put(
                            isoCode, isoCode); // Add the ISO code itself as a symbol
            ...
            void consumeCurrenciesEntry(UResource.Key key, UResource.Value value) {
                ...
                // TODO: Write unit test to ensure that this data is being used by number
                // formatting.
                if (array.getSize() > 2 && formattingData.formatInfo == null) {
                    ...
                    formattingData.formatInfo =
                            new CurrencyFormatInfo(
            ...
            void consumeCurrencyPluralsTable(UResource.Key key, UResource.Value value) {
                ...
                for (int i = 0; table.getKeyAndValue(i, key, value); i++) {
                    ...
                    for (int j = 0; pluralsTable.getKeyAndValue(j, key, value); j++) {
                        ...
                        if (plural == null) {
                            throw new ICUException(
                                    "Could not make StandardPlural from keyword " + key);
                        }
