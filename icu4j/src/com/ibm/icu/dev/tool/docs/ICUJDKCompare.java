...
//  	pw.println("============\n" + n1);
//  	dumpMethodMap(map1, pw);
//  	pw.println("============\n" + n2);
//  	dumpMethodMap(map2, pw);
//    	pw.println("\n============\nAPI in " + n1 + " missing from " + n2);
//  	pw.println("\nCONSTRUCTORS");
//  	dumpMethodMap(diffMethodMaps(cmap2, cmap1), pw);
//  	pw.println("\nMETHODS");
//  	dumpMethodMap(diffMethodMaps(map1, map2), pw);
//  	pw.println("\nFIELDS");
//  	dumpFieldSet(diffFieldSets(set1, set2), pw);

	pw.println("\n============\nAPI in " + n2 + " missing from " + n1);
	pw.println("\nCONSTRUCTORS");
	dumpMethodMap(diffMethodMaps(cmap2, cmap1), pw);
	pw.println("\nMETHODS");
	dumpMethodMap(diffMethodMaps(map2, map1), pw);
	pw.println("\nFIELDS");
	dumpFieldSet(diffFieldSets(set2, set1), pw);
    }
	...
	/*
	// get all the methods mentioned in both m1 and m2
	Map result2 = (Map)((TreeMap)m1).clone();
	result2.keySet().retainAll(m2.keySet());
		
	// for each record, remove the overrides that are covered by m2,
	// and if all overrides are covered, remove it.  since we don't
	// want to modify the original records, clone them first.
	Iterator iter = result2.entrySet().iterator();
	while (iter.hasNext()) {
	    Map.Entry e = (Map.Entry)iter.next();
	    String key = (String)e.getKey();
	    MethodRecord mr = ((MethodRecord)e.getValue()).copy();
		    
	    MethodRecord tr = (MethodRecord)m2.get(key);
	    if (mr.removeOverridden(tr)) {
		if (mr.count() == 0) {
		    iter.remove();
		} else {
		    e.setValue(mr);  // update value with modified record
		}
	    }
	}
