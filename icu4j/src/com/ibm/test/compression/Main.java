...
public class Main 
{
    ...
    public static boolean printDiffs(char [] s1, 
				     int s1len, 
				     char [] s2, 
				     int s2len)
    {
        ...
        if(s1len != s2len) {
            println("====================");
            println("Length doesn't match: expected " + s1len
			       + ", got " + s2len);
            println("Expected:");
            printChars(s1, s1len);
            println("Got:");
        ...
        for(int i = 0; i < len; ++i) {
            if(s1[i] != s2[i]) {
                ...
                println("Exp. char: " + Integer.toHexString(s1[i]));
                println("Got char : " + Integer.toHexString(s2[i]));
                println("Expected:");
                printChars(s1, s1len);
                println("Got:");
                printChars(s2, s2len);
    ...
    public static int myTest(char [] chars, 
			     int len)
    {
    ...
    /** Test iterative compress/decompress API, returning # of errors */
    public static int myMultipassTest(char [] chars, 
				       int len)
    {
        UnicodeCompressor   myCompressor        = new UnicodeCompressor();
        UnicodeDecompressor myDecompressor      = new UnicodeDecompressor();
	...
	boolean             err                     = false;
        ...
        do {
            ...
            /*System.out.println("Compression pass complete.  Compressed "
			       + unicharsRead[0] + " chars into "
			       + bytesWritten + " bytes.");*/
        } while(totalCharsCompressed < len);

        if(totalCharsCompressed != len) {
            println("ERROR: Number of characters compressed("
        ...
        do {
            ...
            /*System.out.println("Decompression pass complete.  Decompressed "
        ...
        if(totalBytesDecompressed != totalBytesWritten) {
            println("ERROR: Number of bytes decompressed(" 
		    + totalBytesDecompressed 
		    + ") != totalBytesWritten(" 
		    + totalBytesWritten + ")");
	    err = true;
            ...
            println("MP: " + totalBytesWritten
			       + " bytes ===> "
			       + totalCharsWritten + " chars.");
        }
        
        if(printDiffs(chars, len, decompressed, totalCharsWritten))
	    err = true;
    ...
    public static void randomRun(char [] target, 
				 int pos, 
				 int len, 
				 Random random)
    {
	...
	if(args.length > 0)
		...
		errCount += simpleTest(fTestCases[i]);
		errCount += myTest(fTestCases[i].toCharArray(), 
				   fTestCases[i].length());
                errCount += myMultipassTest(fTestCases[i].toCharArray(), 
					    fTestCases[i].length());
            }
	    ...
	    // To run an infinite testing loop, change true to false
            if(true) {
		return;
            }
            ...
            int     len     = 0;
            while(true) {
                len = (int)(1000 * random.nextDouble());
                chars = randomChars(len, random);
		simpleTest(new String(chars, 0 ,len));
                myTest(chars, len);
    ...
    static void printBytes(byte [] byteBuffer, 
			   int len)
    {
	int curByteIndex = 0;
	...
	int aByte = 0x00;
        ...
        if(len > byteBuffer.length) {
            System.err.println("Warning: printBytes called with length too large. Truncating");
	...
	while(curByteIndex < byteBufferLimit) {
		...
		while(curByteIndex < byteBufferLimit 
		    ...
		    switch(aByte) {
		    default:
			System.out.print(Integer.toHexString(((int) aByte) 
							     & 0xFF) + " ");
			break;
			...
			// quote unicode
		    case SQUOTEU:
			System.out.print("SQUOTEU ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			break;
			...
			System.out.print("SQUOTE" + (aByte - SQUOTE0) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			break;
			...
			System.out.print("SSWITCH" + (aByte - SSWITCH0) + " ");
			break;
			...
			System.out.print("SDEFINE" + (aByte - SDEFINE0) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			break;
		...
		while(curByteIndex < byteBufferLimit && mode == UNICODEMODE) {
			...
			System.out.print("UDEFINE" + (aByte - UDEFINE0) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			mode = SINGLEBYTEMODE;
			break;
			...
			System.out.print("USWITCH" + (aByte - USWITCH0) + " ");
			mode = SINGLEBYTEMODE;
			break;
			
			// quote unicode
		    case UQUOTEU:
			System.out.print("UQUOTEU ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
		    ...
		    default:
			System.out.print(Integer.toHexString(((int) aByte) & 0xFF) + " ");
			if(curByteIndex < byteBufferLimit)
			    System.out.print(Integer.toHexString(((int) byteBuffer[curByteIndex++]) & 0xFF) + " ");
			break;
