...
final class TestData {
    /**
     * Returns an LSR from a locale ID pattern (e.g. "und", "zh-Hant", "en-*-GB"). This is
     * definitely not a general locale parser!
     */
     ...
     */
    static CldrValue alias(
            AliasType type, AliasReason reason, String value, String... replacement) {
        return supplemental(
    ...
    static CldrValue likelySubtag(String from, String to) {
        return supplemental("likelySubtags/likelySubtag[@from=\"%s\"][@to=\"%s\"]", from, to);
    }
    ...
    /**
     * Returns a map from expanded Trie keys to mapped value. This is useful in allowing tests to
     * use human readable data when testing Tries.
     *
     * @param star a string representing the Trie wildcard in the output keys, which for readability
     *     differs between use cases (e.g. "*" for subtags and "*-*" for match rules).
     * @param fn a function to map the actual Trie value to a more readable value for testing.
     */
    static <T> ImmutableMap<String, T> getTrieTable(
            BytesTrie trie, String star, Function<Integer, T> fn) {
