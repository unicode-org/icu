 ...
 * directly from one of the config files, simply because it avoids having to invent valid paths for
 * testing (and we still need "real" CLDR paths since the path parsing verifies attributes against
 * the DTD metadata). Basing tests on real rules illustrates that all of these tests are asserting
 * about relied-upon behaviour, however there is nothing inherently special about these paths.
 */
...
public class RegexTransformerTest {
    ...
    public void testSingleResults_multipleCapture() {
        ...
        CldrValue lenient =
                CldrValue.parseValue(
                        ...
                        "foo");
        assertSingleResult(transformer.transform(lenient), "/parse/general/lenient", "foo");
        ...
        CldrValue stricter =
                CldrValue.parseValue(
                        "//ldml/characters"
                                ...
                                + "/parseLenient[@sample=\"ignored\"]",
                        "bar");
        assertSingleResult(transformer.transform(stricter), "/parse/number/stricter", "bar");
    }
    ...
    public void testValueSplitting() {
        ...
        CldrValue parentLocale =
                CldrValue.parseValue(
                        "//supplementalData/parentLocales"
                                + "/parentLocale[@parent=\"foo\"][@locales=\"value is split\"]",
                        "");
        assertSingleResult(
                transformer.transform(parentLocale), "/parentLocales/foo", "value", "is", "split");
        ...
        CldrValue mapZone =
                CldrValue.parseValue(
                        ...
                        "");
        assertSingleResult(
                transformer.transform(mapZone), "/mapTimezones/\"foo\"/XX", "value is not split");
    }
    ...
    public void testFallbacks_simple() {
        ...
        ImmutableList<Result> symbol =
                transformer.transform(
                        CldrValue.parseValue(
                                "//ldml/numbers/currencies/currency[@type=\"Foo\"]/symbol",
                                "symbol"));
        assertSingleResult(symbol, "Currencies/Foo", "symbol");
        ImmutableList<Result> name =
                transformer.transform(
                        CldrValue.parseValue(
                                "//ldml/numbers/currencies/currency[@type=\"Foo\"]/displayName",
                                "name"));
    ...
    public void testFallbacks_missingArgs() {
        ...
        assertThat(e)
                .hasMessageThat()
                .contains(
                        "fallback values may only contain arguments from the resource bundle path");
    ...
    public void testHiddenLabelsAndMetazones() {
        ...
        ImmutableList<Result> britishTz =
                transformPath(
                        transformer,
                        "//supplementalData/metaZones/metazoneInfo"
    ...
    public void testBaseXpath() {
        PathValueTransformer transformer =
                transformer(
                        ...
                        // In the real data, these rules define multiple results which reflect the
                        // actual
                        // differences in the child elements, but the one tested is is only based on
                        // the
                        // <territory> path prefix, which is the same for many child elements (which
                        // is all
                        // that's ever actually transformed).
                        //
                        // So for a single path prefix you'll generate multiple identical results
                        // which need
                        // to be de-duplicated, which can only happen if they are considered to have
                        // come
                        // from the same source (since duplicate results happen all the time in
                        // general).
                        //
                        // This is what the base xpath does, it fakes a different source CLDR path
                        // which makes
                        // the results "equal" (even though they came from different CLDR paths
                        // sources).
        ...
        ImmutableList<Result> firstResult =
                transformPath(
                        transformer,
                        commonPrefix
                                + "/languagePopulation[@type=\"kfo\"][@populationPercent=\"0.3\"]");

        ImmutableList<Result> secondResult =
                transformPath(
                        transformer,
                        commonPrefix
                                + "/languagePopulation[@type=\"sef\"][@writingPercent=\"5\"][@populationPercent=\"4\"]");

        assertSingleResult(
                firstResult,
                "/territoryInfo/CI/territoryF:intvector",
                "97160000000",
                "57",
                "26260600");
        assertSingleResult(
    ...
    public void testResultGrouping() {
        ...
        Result grouped =
                transformSingleResult(
                        transformer,
                        "//ldml/numbers/currencies/currency[@type=\"USD\"]/decimal",
                        ".");
    ...
    private static Result transformSingleResult(
        ...
        ImmutableList<Result> results = transformer.transform(CldrValue.parseValue(path, value));
