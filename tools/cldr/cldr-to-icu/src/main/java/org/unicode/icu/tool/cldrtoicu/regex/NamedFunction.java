...
/**
 * Function used by {@code RegexTransformer} to convert CLDR values in special ways. See also {@code
 * IcuFunctions}.
 */
public final class NamedFunction implements Function<List<String>, String> {
    ...
    private NamedFunction(String name, int argCount, Function<List<String>, String> fn) {
        checkArgument(
                !name.isEmpty() && NAME_CHARS.matchesAllOf(name),
                "invalid function name (must be lower_case_underscore): %s",
                name);
    ...
    public String call(String argList) {
        ...
        checkArgument(
                args.size() <= maxArgs,
                "too many arguments for function '%s' (max=%s)",
                name,
                maxArgs);
        return checkNotNull(
                apply(args), "named functions must never return null: function=%s", name);
    }
