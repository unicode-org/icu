 ...
 *
 * <p>Mappings for {@code (desired, supported)} pairs are added at one of three levels in the table;
 * language, script and region. Distances can be resolved at any level in the table (e.g. {@code
 * ("en","fr")}, {@code ("en_Latn","ru_Cyrl")} or {@code ("en_Latn_GB", "en_Latn_AU")}).
 *
 * <p>However in reality the "regions" in the table are actually "partition IDs" representing groups
 * of regions with the same language characteristics. For more information on partitions and how
 * they are generated, see {@link PartitionInfo}.
 *
 * <p>This is mentioned here because anyone debugging this code might be surprised to see values
 * like {@code "5"} for a "region" in the code. Using the term "region" matches the conceptual level
 * of the data and is more familiar to most people, whereas "partition ID" would probably be
 * jarring.
 *
 * <p>The builder class is not resuable, and once a table is built, the builder is invalid.
 * Furthermore, since the table data itself is mutable, care must be taken to avoid modifying either
 * the Trie or the returned distance array.
 *
 * <p>Note that internally the {@code '*'} character used as a wildcard for subtags is replaced by
 * the {@code '�'} character (a.k.a ANY), whenever a subtag is passed into the API. This is because
 * the underlying Trie structure generated by the distance table reserves {@code '*'} for a
 * different purpose. This difference is encapsulated within this class and the {@link Trie} class
 * only.
 */
final class DistanceTable {
    ...
    static final class Builder {
        ...
        /**
         * Adds a distance to the table between the specified and desired tuples. This method takes
         * 1, 2 or 3 sequential {@code (desired, supported)} pairs of values corresponding to
         * language subtags, script subtags and regions (partition IDs). All values can be the
         * wildcard '*'.
         */
        ...
        private void pruneDefaultDistances(int defScriptDistance, int defRegionDistance) {
                                        ...
                                        .forEach(
                                                scriptNode -> {
                                                    if (scriptNode.subtables.size() == 1) {
                                                        // If a script node *only* contains region
                                                        // data with the default
                                                        // region distance, that region data can be
                                                        // removed. Since region
                                                        // is the lowest level, there's no need to
                                                        // worry about "skipping"
                                                        // anything during lookup (unlike the case
                                                        // below).
                                                        Node defRegionNode =
                                                                scriptNode.getAnyNode();
                                                        checkState(
                                                                defRegionNode != null,
                                                                "missing default region node for script: %s",
                                                                scriptNode);
                                                        if (defRegionNode.distance
                                                                == defRegionDistance) {
                                                            scriptNode.subtables.clear();
                                                        }
                                                    }
                                                });
                                // Do the pruning in the "upwards" phase of visitation (after
                                // recursion) so
                                // if script subtables are pruned, it's visible here.
                                if (langNode.subtables.size() == 1) {
                                    // If a language node *only* contains script data with the
                                    // default
                                    // script distance, we can't just remove it (because it might
                                    // contain
                                    // region data).
                                    ...
                                    if (defScriptNode.distance == defScriptDistance) {
                                        checkState(
                                                defScriptNode != null,
                                                "missing default script node for language: %s",
                                                langNode);
                                        if (defScriptNode.subtables.isEmpty()) {
                                            // If the default script node has no region data, remove
                                            // it.
                                            langNode.subtables.clear();
                                        } else {
                                            // Otherwise mark script data as "skippable", which
                                            // indicates
                                            // it should be written in a compact form in the Trie
                                            // (while
            ...
            checkState(
                    rootNode.getAnyNode().subtables.isEmpty(),
                    "invalid table state: %s",
                    rootNode.getAnyNode());
    ...
    private static final class Node {
        ...
        /**
         * Adds a new mapping to this node with the specified distance if it didn't already exist.
         *
         * <p>Note: If a mapping already exists, then this method has no effect (even if the
         * existing distance differs from the given distance). This is necessary to for two reasons:
         *
         * <ol>
         *   <li>An earlier match rule may have set an explicit value for the mapping, and we
         *       subsequently try to set a default value (via a wildcard mapping). This should be
         *       ignored, since we want the non-default value to win. This means it's important to
         *       always have explicit {@code <languageMatch>} rules before any related wildcard
         *       rules in the CLDR data.
         *   <li>A preferential {@code <languageMatch>} rule appears earlier in CLDR data. This
         *       occurs because of the way partitions are defined and allows for two distinct {@code
         *       <languageMatch>} rules to generate the same mapping (with different distances).
         *       This is because region variables reference sets of partition IDs and these are not
         *       always disjoint (e.g. "en_*_$!enUS" and "en_*_GB" both contain the partition ID for
         *       "GB").
         * </ol>
         *
         * @return true if a new mapping was added, or if the distances were equal (i.e. the
        ...
        /**
         * Returns a sub-node corresponding to the given {@code (desired, supported)} mapping. If
         * the node already exists, it is simply returned, otherwise a new node is created and any
         * existing wildcard mappings are copied into it.
         */
        private Node getIntermediateNode(MappingKey key) {
            ...
            if (node == null) {
                ...
                int distance =
                        key.getDesired().equals(key.getSupported()) ? 0 : wildcardMatch.distance;
                    ...
                    .forEach(
                            c ->
                                    subtables.put(
                                            c.getRowKey(),
                                            c.getColumnKey(),
                                            new Node(c.getValue().distance)));
        }

        /**
         * Writes all the mappings in the distance table sequentially to given Trie in sorted table
         * order.
         *
         * <p>Mappings are written in a top-down recursive visitation with sub-tables inheriting the
         * current prefix from parent tables via the given Trie span. At each level any mapped
         * distances are written before recursing into the sub-tables.
         */
        private void writeTo(Trie.Span trieSpan) {
            if (distance >= 0 && (distance & DISTANCE_SKIP_SCRIPT) != 0) {
                ...
                subtables
                        .rowMap()
                        .forEach(
                                (desired, supportedNodes) ->
                                        writeSupported(trieSpan, desired, supportedNodes));
            }
        }

        private void writeSupported(
                Trie.Span trieSpan, String desired, Map<String, Node> supportedNodes) {
            ...
            if (desired.equals(ANY)) {
                ...
                checkState(
                        node != null && supportedNodes.size() == 1,
                        "invalid supported subtags for desired='ANY': %s",
                        supportedNodes);
                ...
                trieSpan.with(
                        desired,
                        withDesiredSpan ->
                                supportedNodes.forEach(
                                        (supported, node) -> {
                                            checkState(
                                                    !supported.equals(ANY),
                                                    "unexpected supported='ANY' subtag: %s",
                                                    supported);
                                            withDesiredSpan.with(
                                                    supported, node::writeDistancePlusSubtables);
                                        }));
            }
        }
    ...
    /**
     * Excapsulates a sequence of {@code <desired,supported>} pairwise mappings over language,
     * script and region, with an associated distance. This is an alternate way to represent a
     * mapping of desired and supported language match rules.
     *
     * <p>For example:
     *
     * <pre>{@code
     ...
     * }</pre>
     *
     * where the "region" part {@code <X,Y>} is constructed from all the possible combinations of
     * partition IDs associated with the original region variables.
     *
     * <p>Mapping keys have several useful properties:
     *
     * <ul>
     *   <li>They can be reversed (e.g. {@code <A,B> -> <C,D> = N} becomes {@code <B,A> -> <D,C> =
     *       N}).
     *   <li>They can be symmetrical (e.g. {@code <X,X> -> <Y,Y> = N}), in which case the reversed
     *       key is the same as the original.
     *   <li>They can have wildcard mappings (i.e. {@code <ANY,ANY>}).
     *   <li>They can produce "suffix" keys (e.g. the suffix of {@code <A,B> -> <C,D> = N} is {@code
     *       <C,D> = N}).
     * </ul>
     */
    private static final class MappingKey {
        /**
         * Returns a new key from the specified subtag pairs, converting {@code '*'} subtags to the
         * special {@code ANY} string and performing consistency checks.
         *
        ...
        static MappingKey fromSubtags(String[] subtagPairs, int distance) {
            ...
            checkArgument(
                    pairCount == 2 || pairCount == 4 || pairCount == 6,
                    "invalid number of arguments (expected 1, 2 or 3 pairs): %s",
                    asList(subtagPairs));
        ...
        private MappingKey(ImmutableList<String> pairs, int distance, boolean isReversed) {
            ...
            checkArgument(
                    distance >= 0 && distance <= MAX_REGION_DISTANCE,
                    "invalid mapping key distance: %s",
                    distance);
            ...
            for (int i = 0; i < pairs.size(); i += 2) {
                ...
                checkArgument(
                        desired.equals(ANY) == supported.equals(ANY),
                        "invalid mapping key pairs: %s",
                        pairs);
        ...
        /**
         * Returns the number of {@code <desired,supported>} mappings in this key; this is either 1
         * (language-only), 2 (language & script) or 3 (language, script & region).
         */
        int getDepth() {
            return pairs.size() / 2;
        }
        ...
        /**
         * Returns the suffix of this non-leaf key with the top-level mapping removed. For example,
         * the suffix of {@code "<A,B> -> <C,D> = N"} is {@code "<C,D> = N"}).
         */
