...
public final class ConvertIcuDataTask extends Task {
    ...
    public void setOutputTypes(String types) {
        ImmutableList<OutputType> typeList =
                LIST_SPLITTER.splitToList(types).stream()
    ...
    public static final class Directory extends Task {
        ...
        public void addConfiguredLocaleIds(LocaleIds localeIds) {
            checkBuild(
                    this.localeIds == null,
                    "Cannot add more that one <localeIds> element for <directory>: %s",
                    dir);
    ...
    public void addConfiguredDirectory(Directory filter) {
        checkState(
                !perDirectoryIds.containsKey(filter.dir),
                "directory %s specified twice",
                filter.dir);
        ...
        if (!ids.containsAll(filter.inheritLanguageSubtag)) {
            log(
                    String.format(
                            ...
                            filter.dir,
                            String.join(", ", Sets.difference(filter.inheritLanguageSubtag, ids))));
    ...
    public void execute() throws BuildException {
        ...
        CldrDataSupplier src =
                CldrDataSupplier.forCldrFilesIn(cldrPath)
        ...
        for (IcuLocaleDir dir : IcuLocaleDir.values()) {
            ...
            inheritLanguageSubtag
                    .get(dir)
                    .forEach(
                            id -> {
                                checkArgument(
                                        id.matches("[a-z]{2}_[A-Z][a-z]{3}"),
                                        "Invalid locale ID for inheritLanguageSubtag (expect '<lang>_<Script>'): ",
                                        id);
                                checkArgument(
                                        supplementalData.getParent(id).equals("root"),
                                        "Invalid locale ID for inheritLanguageSubtag (parent must be 'root'): ",
                                        id);
    ...
    private static <T extends Enum<T>> T resolve(Class<T> enumClass, String name) {
        ...
        checkArgument(
                ...
                "invalid enumeration name '%s'; expected only ASCII letters or '_'",
                name);
        CaseFormat format;
        if (UPPER_UNDERSCORE.matchesAllOf(name)) {
            ...
            checkArgument(
                    !name.contains("_"),
                    "invalid enumeration name '%s'; mixed case with underscore not allowed: %s",
                    name);
            format =
