...
/**
 * Territory containment graph. This is built from CLDR supplemental data and represents all
 * territories and their containment, including macro regions such as {@code "016"}. The root node
 * of the graph is {@code "001"}.
 */
final class TerritoryContainment {
    ...
    /**
     * Returns the territory containment information described by the given CLDR supplemental data.
     */
    ...
    private TerritoryContainment(ImmutableSetMultimap<String, String> graph) {
        ...
        checkArgument(
                roots.equals(ImmutableSet.of(WORLD)),
                "world region '%s' must be the only containment graph root (was %s)",
                WORLD,
                roots);
        ...
        this.macroToLeafRegions =
                resolved.entries().stream()
                        // Only keep macro regions (leaf regions don't have child regions by
                        // definition).
    ...
    private static Set<String> resolve(
            String region,
            SetMultimap<String, String> graph,
            SetMultimap<String, String> resolved) {
        graph.get(region).forEach(sub -> resolved.putAll(region, resolve(sub, graph, resolved)));
        return resolved.get(region);
    }

    /**
     * Returns the leaf regions contained in the given region (if the given region is a leaf region,
