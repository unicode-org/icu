...
final class IcuFunctions {
     ...
     * and bottom parts of a deconstructed millisecond epoch value (i.e. {@code "<hi32bits>
     * <low32bits>"}).
     *
     ...
     *   <li>args[2] = Timezone for ISO date string, as CLDR canonical “long” time zone ID; Etc/UTC
     *       for most
     * </ul>
     */
    static final NamedFunction DATE_FN =
            NamedFunction.create(
                    ...
                    args -> {
                        long millis =
                                DateFieldType.toEnum(args.get(1))
                                        .toEpochMillis(LocalDate.parse(args.get(0)), args.get(2));
     ...
     *
     * <p>Resultant integers are in the form "xxyyyyyy", where "xx" is the exponent offset by 50 and
     * "yyyyyy" is the coefficient to 5 decimal places. Results may also have a leading '-' to
     ...
     * to do things like converting percentages into their decimal representation (i.e. by passing a
     * value of "-2").
     *
     * <ul>
                    ...
                    args -> {
                        ...
                        if (exponent < 0 || exponent > 99) {
                            throw new IllegalArgumentException(
                                    "Exponent out of bounds: " + exponent);
                        }
    ...
    /**
     * Converts an ISO date string (i.e. "YYYY-MM-DD") into an ICU date string, which is the same
     * but with spaces instead of hyphens. Since functions are expanded before the resulting value
     * is split, this function will result in 3 separate values being created, unless the function
     * call is enclosed in quotes.
     *
     * <p>Note that for some cases (e.g. "eras") the year part can be negative (e.g. "-2165-1-1") so
     * this is not as simple as "split by hyphen".
     *
     * <ul>
    ...
    static final NamedFunction YMD_FN =
            NamedFunction.create(
                    ...
                    args -> {
                        ...
                        checkArgument(
                                m.matches(), "invalid year-month-day string: %s", args.get(0));
                        // NOTE: Re-parsing is not optional since it removes leading zeros (needed
                        // for ICU).
