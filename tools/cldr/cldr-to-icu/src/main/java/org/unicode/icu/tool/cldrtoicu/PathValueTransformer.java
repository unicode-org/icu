...
/**
 * API for transforming CLDR path/value pairs. Transformed results support grouping by their key and
 * the ability to generate default "fallback" values to account for missing values in a group.
 *
 ...
 *   <li>For each group, add any fallback values which don't yet exist for that key (see {@link
 *       #getFallbackResultsFor(RbPath, DynamicVars)} and {@link Result#isFallbackFor(Result)}).
...
public abstract class PathValueTransformer {
     ...
     * some known key (see {@link PathValueTransformer#transform(CldrValue)} and {@link
     * PathValueTransformer#getFallbackResultsFor(RbPath, DynamicVars)}).
     */
    public abstract static class Result implements Comparable<Result> {
         ...
         * from other results in the same group. Grouped values cannot be split and must appear as a
         * single value.
         *
        ...
        /**
         * Returns the transformed values of this result, which may or may not be grouped according
         * to {@link #isGrouped()}.
         */
     ...
     * binding of variables incurs significant additional complexity in the code, despite being used
     * in exactly one situation so far (the '%D' variable to represent the default numbering scheme.
     */
    // TODO: Figure out how to get rid of all of this mess.
    public interface DynamicVars extends Function<CldrPath, String> {}

    /**
     * Transforms a CLDR value into a sequence of results (empty if the value was not matched by any
     * rule).
     *
    ...
    /**
     * Transforms a CLDR value into a sequence of results (empty if the value was not matched by any
     * rule). The dynamic variable function provides any "late bound" CLDR path variables to be
