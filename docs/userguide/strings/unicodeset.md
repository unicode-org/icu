...
> Notice: The ICU-TC is considering a proposal to require that `$` be escaped when
it represents U+0024 $ DOLLAR SIGN; see the [Extensions](#extensions) section
below.
Please provide feedback on [ICU-23517](https://unicode-org.atlassian.net/browse/ICU-23517).
...
Note that there are some defects in numeric property queries, tracked by
ticket [ICU-23516](https://unicode-org.atlassian.net/browse/ICU-23516).
...
* A `$` at the end of [Content](https://www.unicode.org/reports/tr61/#Content) (that is, preceding a
  ...
  > Note: The ICU-TC is considering a proposal that would reduce this extension to
    the `$` in [Æther](#Æther), eliminating
    [DollarElements](#DollarElements) and
    the change to [Range](https://www.unicode.org/reports/tr61/#Range).
    This would disallow a set such as `[$!]`, requiring that `$` be escaped as in
    `[\$!]` instead.
    See [ICU-23517](https://unicode-org.atlassian.net/browse/ICU-23517).
