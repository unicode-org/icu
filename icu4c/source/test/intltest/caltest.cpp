...
void CalendarTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char* /*par*/ )
{
    ...
    switch (index) {
        ...
        case 37:
          name = "TestLocaleTZExtension";
          if(exec) {
            logln("TestLocaleTZExtension---"); logln("");
            TestLocaleTZExtension();
          }
          break;
        default: name = ""; break;
    }
...
void CalendarTest::TestLocaleTZExtension() {
    ...
    for (int32_t tzidx = 0; tzidx < UPRV_LENGTHOF(tz); tzidx++) {
        ...
        UErrorCode status = U_ZERO_ERROR;
        std::unique_ptr<TimeZoneFormat> tzfmt(TimeZoneFormat::createInstance(l, status));
        if (U_FAILURE(status)) {
            dataerrln("Error TimeZoneFormat::createInstance(%s) - %s", locale.c_str(), u_errorName(status));
            continue;
        }
        std::unique_ptr<Calendar> cal(Calendar::createInstance(l, status));
        if (U_FAILURE(status)) {
            dataerrln("Error Calendar::createInstance(%s) - %s", locale.c_str(), u_errorName(status));
            continue;
        }
        UnicodeString id;
        tzfmt->format(UTZFMT_STYLE_ZONE_ID_SHORT, cal->getTimeZone(), now, id);
        if (id !=  tz[tzidx]) {
          std::string id_str;
          id.toUTF8String(id_str);
          dataerrln("Error timezone is %s but not %s which set by %s locale", id_str.c_str(), tz[tzidx], locale.c_str());
        }
    }
    for (int32_t tzidx = 0; tzidx < UPRV_LENGTHOF(timezone); tzidx++) {
        ...
        UnicodeString id;
        tzfmt->format(UTZFMT_STYLE_ZONE_ID, cal->getTimeZone(), now, id);
        if (id !=  timezone[tzidx]) {
          std::string id_str;
          id.toUTF8String(id_str);
          dataerrln("Error timezone is %s but not %s which set by %s locale", id_str.c_str(), timezone[tzidx], locale.c_str());
        }
    }
}
