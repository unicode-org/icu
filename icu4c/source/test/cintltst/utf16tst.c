...
void TestNextPrevChar(){
   ...
   /*next_unsafe    next_safe_ns  next_safe_s       prev_unsafe   prev_safe_ns     prev_safe_s*/
	...
	for(offset=0; offset<u_strlen(input); offset++){
		 ...
		 setOffset=offset;
		 UTF16_NEXT_CHAR_SAFE(input, setOffset, u_strlen(input), c, FALSE);
		 ...
		 setOffset=offset;
		 UTF16_NEXT_CHAR_SAFE(input, setOffset, u_strlen(input), c, TRUE);
		 if(setOffset != movedOffset[i+1]){
	...
	i=0;
	for(offset=(uint16_t)u_strlen(input)+1; offset > 0; --offset){
		 setOffset=offset;
...
void TestFwdBack(){	
	static UChar input[]={0x0061, 0xd800, 0xdc00, 0xdbff, 0xdfff, 0x0062, 0xd841, 0xd7ff, 0xd841, 0xdc41, 0xdc00, 0x0000};
    static uint16_t fwd_unsafe[] ={1, 3, 5, 6,	8, 10, 11};
    static uint16_t fwd_safe[]   ={1, 3, 5, 6, 7, 8, 10, 11};
	...
	static uint16_t fwd_N_safe[]   ={0, 1, 5, 8, 10, 11, 11}; /*safe macro keeps it at the end of the string */
	...
	uint16_t i=0;
    while(offunsafe < u_strlen(input)){
		...
		UTF16_FWD_1_SAFE(input, offsafe, u_strlen(input));
		if(offsafe != fwd_safe[i]){
			log_err("ERROR: Forward_safe offset expected:%d, Got:%d\n", fwd_safe[i], offsafe);
		}
		i++;
	}
	offunsafe=u_strlen(input)+1;
	offsafe=u_strlen(input)+1;
	i=0;
	while(offunsafe > 0){
	...
	for(i=0; i<sizeof(Nvalue)/sizeof(Nvalue[0]); i++){
		UTF_FWD_N_SAFE(input, offsafe, u_strlen(input), Nvalue[i]);
...
void TestSetChar(){
	...
	for(offset=0; offset<u_strlen(input); offset++){
		 ...
		 setOffset=offset; 
		 UTF16_SET_CHAR_LIMIT_SAFE(input,0, setOffset, u_strlen(input));
