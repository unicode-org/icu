...
#
#  Script to check ICU source files to contain only UTF-8 encoded characters.
#
...
#    is_source:        Flag, set to True if file is a souece code file (.c, .cpp, .h, .java).
#
...
def main(argv):
    ...
    for f in file_list:
        ...
        if source_file_re.match(f):
            print "Checking source file: " + f
            source_file = True
            check_file(f, source_file);
        else:
            print "Checking non-source file: " + f
            source_file = False
            check_file(f, source_file)
