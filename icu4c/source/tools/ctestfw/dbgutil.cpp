...
T_CTEST_API const UnicodeString& T_CTEST_EXPORT2 udbg_enumString(UDebugEnumType type, int32_t field) {
	if(strs == NULL ) {
		...
		//fflush(stderr);
		strs = new UnicodeString*[UDBG_ENUM_COUNT+1];
		for(int t=0;t<UDBG_ENUM_COUNT;t++) {
			int32_t c = udbg_enumCount((UDebugEnumType)t);
			strs[t] = new UnicodeString[c+1];
			for(int f=0;f<=c;f++) {
				_fieldString((UDebugEnumType)t, f, strs[t][f]);
			}
		}
		strs[UDBG_ENUM_COUNT] = new UnicodeString[1]; // empty string
	}
