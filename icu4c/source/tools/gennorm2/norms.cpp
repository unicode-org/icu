...
Norms::Norms(UErrorCode &errorCode) {
    normTrie = utrie3bld_open(0, 0, &errorCode);
...
Norm *Norms::getNorm(UChar32 c) {
    uint32_t i = utrie3bld_get(normTrie, c);
    if(i==0) {
        return nullptr;
    }
    return norms+i;
...
const Norm &Norms::getNormRef(UChar32 c) const {
    return norms[utrie3bld_get(normTrie, c)];
}
...
Norm *Norms::createNorm(UChar32 c) {
    ...
    if(i!=0) {
        ...
        utrie3bld_set(normTrie, c, (uint32_t)(p - norms), errorCode);
        return p;
    }
}
...
void Norms::enumRanges(Enumerator &e) {
    UChar32 start = 0, end;
    uint32_t i;
    while ((end = utrie3bld_getRange(normTrie, start, nullptr, nullptr, &i)) >= 0) {
        if (i > 0) {
