...
/**
 * Read a tab separated data file generated by the CLDR-to-ICU conversion tool
 * (see ICU_ZONE_REGIONS, ICU_METAZONE_OFFSETS).  Blank lines and lines starting
 * with '#' are ignored; every other line must have exactly the given number of
 * fields.
 */
vector<vector<string> > readDataFile(const string& filepath, size_t numFields) {
    ...
    while (getline(in, line)) {
        lineno++;
        // Tolerate CRLF line endings and a UTF-8 BOM.
        ...
        if (fields.size() != numFields) {
            ostringstream os;
            os << filepath << ":" << lineno << ": expected " << numFields
...
// Mapping from a TZ database zone ID to its CLDR region, where "001"
// (UN M.49 - World) means the zone is not associated with a region.
typedef map<string, string> ZoneRegionMap;

/**
 * Read the zone/region mapping from the file generated by the CLDR-to-ICU
 * conversion tool (see ICU_ZONE_REGIONS).  Each data line is tab separated:
 *
 *   <zone id> <region>
 *
 * where <region> is an ISO 3166 2-letter code, or "001" for zones which CLDR
 * does not associate with a region, such as "Etc/GMT+1".
 */
void readZoneRegions(const string& filepath, ZoneRegionMap& zoneRegions) {
    vector<vector<string> > rows = readDataFile(filepath, 2);
    for (vector<vector<string> >::const_iterator i = rows.begin(); i != rows.end(); ++i) {
        zoneRegions[(*i)[0]] = (*i)[1];
    }
}

/**
 * Read the metazone offset overrides from the file generated by the CLDR-to-ICU
 * conversion tool (see ICU_METAZONE_OFFSETS).  Each data line is tab separated:
 *
 *   <zone id> <from> <to> <stdOffset> <dstOffset>
 *
 * where <from> (inclusive) and <to> (exclusive) are UTC date-times formatted as
 * "yyyy-MM-dd HH:mm", and the offsets are in the CLDR "[+-]HH[:mm]" format.
 */
void readMetazoneOffsets(const string& filepath, CldrMetazoneMap& metazoneRules) {
    vector<vector<string> > rows = readDataFile(filepath, 5);
    for (vector<vector<string> >::const_iterator i = rows.begin(); i != rows.end(); ++i) {
        const vector<string>& fields = *i;
        CldrMetazoneRule rule;
        rule.from = parseUtcDateTime(fields[1]);
        rule.to = parseUtcDateTime(fields[2]);
        rule.stdOffset = parseOffset(fields[3]);
        rule.dstOffset = parseOffset(fields[4]);
        // Offsets from metaZones.xml should only be taken into account after 1970-01-01
        if (rule.to <= 0) {
            continue;
        }
        if (rule.from < 0) {
        ...
        metazoneRules[fields[0]].push_back(rule);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (!validArgs) {
        cout << "Usage: tz2icu <dir> <tzver> [--old]" << endl
    ...
    ZoneRegionMap zoneRegions;
    try {
        readZoneRegions(ICU_ZONE_REGIONS, zoneRegions);
        cout << "Finished reading " << zoneRegions.size()
             << " zone/region mappings from " ICU_ZONE_REGIONS << endl;
    } catch (const exception& error) {
        cerr << "Error: While reading " ICU_ZONE_REGIONS ": " << error.what() << endl;
        return 1;
    }
    if (zoneRegions.empty()) {
        cerr << "Error: No zone/region mappings found in " ICU_ZONE_REGIONS << endl;
        return 1;
    }

    CldrMetazoneMap metazoneRules;
    try {
        readMetazoneOffsets(ICU_METAZONE_OFFSETS, metazoneRules);
        cout << "Finished reading " << metazoneRules.size()
             << " timezone rules with offsets from " ICU_METAZONE_OFFSETS << endl;
    } catch (const exception& error) {
        cerr << "Error: While reading " ICU_METAZONE_OFFSETS ": " << error.what() << endl;
        return 1;
    }
    if (metazoneRules.empty()) {
        cerr << "Error: No metazone offset rules found in " ICU_METAZONE_OFFSETS << endl;
        return 1;
    }
    ...
    // Create the country map from the CLDR zone/region data
    map<string, set<string> > countryMap;  // country -> set of zones
    map<string, string> reverseCountryMap; // zone -> country

    int32_t n = 0;
    for (ZoneRegionMap::const_iterator i = zoneRegions.begin();
         i != zoneRegions.end(); ++i) {
        const string& zone(i->first);
        const string& country(i->second);
        if (ZONEINFO.find(zone) == ZONEINFO.end()) {
            // CLDR may know zones which are not (or no longer) in the
            // TZ database, such as zones removed from a newer tzdata.
            cout << "Warning: CLDR zone " << zone
                 << " is not in the TZ database, ignoring" << endl;
            continue;
        }
        if (country == "001") {
            // The zone is not associated with any region, for example
            // "Etc/GMT+1". Such zones are emitted as "001" below.
            continue;
        }
    ...
    cout << "Finished mapping " << n << " zones to regions" << endl;
    ...
    // Aliases which CLDR knows, but deliberately does not associate with
    // a region, such as "Etc/GMT+1", are left without a region as well.
    // This only affects ICU-specific zones, such as "PST" or "SystemV/PST8".
    for (map<string,set<string> >::const_iterator i = links.begin();
         i!=links.end(); ++i) {
        const string& olson(i->first);
        ...
        for (set<string>::const_iterator j=aliases.begin();
            ...
            if (reverseCountryMap.find(*j) == reverseCountryMap.end()
                && zoneRegions.find(*j) == zoneRegions.end()) {
