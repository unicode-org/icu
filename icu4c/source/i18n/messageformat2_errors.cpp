...
namespace message2 {
    ...
    void DynamicErrors::setSoftBadOption(const FunctionName& formatterName, UErrorCode& status) {
        addError(DynamicError(DynamicErrorType::SoftBadOptionError, formatterName), status);
    }
    ...
    void DynamicErrors::checkErrors(UErrorCode& status) const {
        ...
        if (U_FAILURE(status)) {
            ...
            case DynamicErrorType::SoftBadOptionError: {
