...
{
    if (!mRuleText.isEmpty()) {
        parseRuleDescriptor(mRuleText, status);
    }
}
                             ...
                             UErrorCode& status)
{
    ...
    int32_t pluralRuleStart = mRuleText.indexOf(gDollarOpenParenthesis, -1, 0);
    int32_t pluralRuleEnd = (pluralRuleStart >= 0 ? mRuleText.indexOf(gClosedParenthesisDollar, -1, pluralRuleStart) : -1);
    if (pluralRuleEnd >= 0) {
        int32_t endType = mRuleText.indexOf(gComma, pluralRuleStart);
        if (endType < 0) {
            status = U_PARSE_ERROR;
            return;
        }
        UnicodeString type(mRuleText.tempSubString(pluralRuleStart + 2, endType - pluralRuleStart - 2));
        ...
        rulePatternFormat = formatter->createPluralFormat(pluralType,
                mRuleText.tempSubString(endType + 1, pluralRuleEnd - endType - 1), status);
    }
}
                            ...
                            UErrorCode& status)
{
    ...
    if (mRuleText.indexOf(gGreaterGreaterGreater, 3, 0) == subStart) {
        subEnd = subStart + 2;

        // otherwise the substitution token ends with the same character
        // it began with
    } else {
        UChar c = mRuleText.charAt(subStart);
        subEnd = mRuleText.indexOf(c, subStart + 1);
        // special case for '<%foo<<'
        if (c == gLessThan && subEnd != -1 && subEnd < mRuleText.length() - 1 && mRuleText.charAt(subEnd+1) == c) {
    ...
    subToken.setTo(mRuleText, subStart, subEnd + 1 - subStart);
...
NFRule::indexOfAnyRulePrefix() const
{
    ...
    for (int i = 0; RULE_PREFIXES[i]; i++) {
        int32_t pos = mRuleText.indexOf(*RULE_PREFIXES[i]);
...
NFRule::_appendRuleText(UnicodeString& result) const
{
    ...
    if (mRuleText.charAt(0) == gSpace && (sub1 == NULL || sub1->getPos() != 0)) {
        result.append(gTick);
    }
...
NFRule::doFormat(int64_t number, UnicodeString& toInsertInto, int32_t pos, int32_t recursionCount, UErrorCode& status) const
{
    ...
    // messed up)
    int32_t pluralRuleStart = mRuleText.length();
    int32_t lengthOffset = 0;
    if (!rulePatternFormat) {
        toInsertInto.insert(pos, mRuleText);
    }
    else {
        pluralRuleStart = mRuleText.indexOf(gDollarOpenParenthesis, -1, 0);
        int pluralRuleEnd = mRuleText.indexOf(gClosedParenthesisDollar, -1, pluralRuleStart);
        int initialLength = toInsertInto.length();
        if (pluralRuleEnd < mRuleText.length() - 1) {
            toInsertInto.insert(pos, mRuleText.tempSubString(pluralRuleEnd + 2));
        }
        toInsertInto.insert(pos,
            rulePatternFormat->format((int32_t)(number/util64_pow(radix, exponent)), status));
        if (pluralRuleStart > 0) {
            toInsertInto.insert(pos, mRuleText.tempSubString(0, pluralRuleStart));
        }
        lengthOffset = mRuleText.length() - (toInsertInto.length() - initialLength);
    }
                ...
                Formattable& resVal) const
{
    ...
    int32_t sub1Pos = sub1 != NULL ? sub1->getPos() : mRuleText.length();
    int32_t sub2Pos = sub2 != NULL ? sub2->getPos() : mRuleText.length();
    ...
    prefix.setTo(mRuleText, 0, sub1Pos);
    ...
    do {
        ...
        temp.setTo(mRuleText, sub1Pos, sub2Pos - sub1Pos);
        ...
        if (pp.getIndex() != 0 || sub1 == NULL) {
            ...
            // a real result
            temp.setTo(mRuleText, sub2Pos, mRuleText.length() - sub2Pos);
                 ...
                 int32_t* length) const
{
    if (rulePatternFormat) {
        ...
        if (start >= 0) {
            int32_t pluralRuleStart = mRuleText.indexOf(gDollarOpenParenthesis, -1, 0);
            int32_t pluralRuleSuffix = mRuleText.indexOf(gClosedParenthesisDollar, -1, pluralRuleStart) + 2;
            int32_t matchLen = position.getEndIndex() - start;
            UnicodeString prefix(mRuleText.tempSubString(0, pluralRuleStart));
            UnicodeString suffix(mRuleText.tempSubString(pluralRuleSuffix));
