...
PrioritizedVariant::~PrioritizedVariant() {}
    ...
    void InternalValue::forceSelection(DynamicErrors& errs,
        ...
        while (std::holds_alternative<InternalValue*>(p->argument)) {
            ...
            if (p != this &&
                !p->options.getStringFunctionOption(UnicodeString("select")).isEmpty()
                && (selectorName == UnicodeString("number") || selectorName == UnicodeString("integer"))) {
        ...
        // The checking of the "soft" error has to be done here; otherwise,
        ...
        if (errorCode == U_MF_SELECTOR_ERROR) {
            ...
            errs.setSoftBadOption(selectorName, errorCode);
    ...
    FormattedPlaceholder InternalValue::forceFormatting(DynamicErrors& errs, UErrorCode& errorCode) {
        ...
        if (badSelect) {
            ...
            errs.setSoftBadOption(name, errorCode);
        }
        return result;
    }
