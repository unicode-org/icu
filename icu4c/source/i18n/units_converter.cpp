...
UBool checkSimpleUnit(const MeasureUnitImpl &unit, UErrorCode &status) {
    ...
    if (singleUnit.dimensionality != 1 || singleUnit.unitPrefix != UMEASURE_SI_PREFIX_ONE) {
        return false;
    }
...
void loadConversionRate(ConversionRate &conversionRate, const MeasureUnitImpl &source,
    ...
    // TODO: should we consider failure if there's an offset for a
    // not-simple-unit? What about kilokelvin / kilocelsius?
